...
void RegisterReduceL1L2Cases(std::vector<TestCase> &registry, const std::string &op_type,
  ...
  EmitReduceL1L2DefaultAxesCase<Kernel>(
      registry, op_type, "test_cc_" + name_prefix + "_default_axes_keepdims", shape, values,
      /*keepdims=*/true);
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_keepdims", shape,
                             values, {1}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_do_not_keepdims",
                             shape, values, {1}, /*keepdims=*/false,
                             /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(
      registry, op_type, "test_cc_" + name_prefix + "_negative_axes_keepdims", shape, values, {-2},
      /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_empty_axes_input_noop",
                             shape, values, {},
  ...
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_empty_set",
                             /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                             /*noop_with_empty_axes=*/false);
  // Non-reduced axis of size 0: the result preserves the empty dimension.
  EmitReduceL1L2Case<Kernel>(registry, op_type,
...
void RegisterReduceL1L2OnnxCases(std::vector<TestCase> &registry, const std::string &op_type,
  ...
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_" + onnx_prefix + "_keep_dims_example", shape,
                             example_values, {2},
                             /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_" + onnx_prefix + "_keep_dims_random", shape,
                             random_values, {2},
                             /*keepdims=*/true, /*noop_with_empty_axes=*/false);

  // do_not_keepdims (axis = 2, keepdims = 0)
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_" + onnx_prefix + "_do_not_keepdims_example",
                             shape, example_values, {2}, /*keepdims=*/false,
                             /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_" + onnx_prefix + "_do_not_keepdims_random",
  ...
  EmitReduceL1L2Case<Kernel>(
      registry, op_type, "test_" + onnx_prefix + "_negative_axes_keep_dims_example", shape,
      example_values, {-1}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceL1L2Case<Kernel>(
      registry, op_type, "test_" + onnx_prefix + "_negative_axes_keep_dims_random", shape,
      random_values, {-1}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);

  // empty_set (data shape [2, 0, 4], axis = 1, keepdims = 1)
  EmitReduceL1L2Case<Kernel>(registry, op_type, "test_" + onnx_prefix + "_empty_set",
                             /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                             /*noop_with_empty_axes=*/false);
}
...
void RegisterReduceL1Cases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducel1_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             const KernelContext reduce_l1_kernel_ctx{DefaultOpset(18)};
             const onnx_kernels::kernel::ReduceL1 reduce_l1_kernel{reduce_l1_kernel_ctx};
  ...
  RegisterReduceL1L2Cases<onnx_kernels::kernel::ReduceL1>(registry, "ReduceL1", "reducel1");
  RegisterReduceL1L2OnnxCases<onnx_kernels::kernel::ReduceL1>(registry, "ReduceL1", "reduce_l1");
}
...
void RegisterReduceL2Cases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducel2_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             const KernelContext reduce_l2_kernel_ctx{DefaultOpset(18)};
             const onnx_kernels::kernel::ReduceL2 reduce_l2_kernel{reduce_l2_kernel_ctx};
  ...
  RegisterReduceL1L2Cases<onnx_kernels::kernel::ReduceL2>(registry, "ReduceL2", "reducel2");
  RegisterReduceL1L2OnnxCases<onnx_kernels::kernel::ReduceL2>(registry, "ReduceL2", "reduce_l2");
}
