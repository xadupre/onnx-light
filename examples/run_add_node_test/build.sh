...
#   bash examples/run_add_node_test/build.sh [install-prefix] [lib-build-dir] [example-build-dir]
...
INSTALL_PREFIX="${1:-${REPO_ROOT}/build/install-run-add-node-test}"
LIB_BUILD_DIR="${2:-${REPO_ROOT}/build/run-add-node-test-lib}"
EXAMPLE_BUILD_DIR="${3:-${REPO_ROOT}/build/run-add-node-test-example}"
...
echo
echo "Example binary:"
echo "  ${EXAMPLE_BUILD_DIR}/run_add_node_test"
