...
public:
  OrtAffineGridKernel(const OrtApi *, const OrtKernelInfo *info)
      : kernel_(runtime::KernelContext{runtime::DefaultOpset(20)}) {
    try {
      align_corners_ = Ort::ConstKernelInfo(info).GetAttribute<int64_t>("align_corners");
  ...
  Ort::Status Compute(OrtKernelContext *context, const Ort::Custom::Tensor<float> &theta,
    ...
    try {
      ...
      std::unique_ptr<runtime::CpuExecutor> executor = runtime::CpuExecutor::CreateExternal(
          ExternalParticipantLimit(), context, &DispatchWithOrt);
      ...
      return Ort::Status(exception.what(), ORT_INVALID_ARGUMENT);
    } catch (const Ort::Exception &exception) {
      return Ort::Status(exception.what(), exception.GetOrtErrorCode());
    } catch (const std::exception &exception) {
      return Ort::Status(exception.what(), ORT_RUNTIME_EXCEPTION);
    }
  }
