    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if ((nullptr != value) && (nullptr != sparse_value))
            fail_shape_inference("Only one of the attributes 'value' or 'sparse_value' must be "
                                 "specified for a Constant node.");
          ...
          fail_shape_inference("One of the attributes 'value' or 'sparse_value' must be specified "
                               "for a Constant node.");
        }));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("value",
              ...
              "Should be a one-element tensor. If not specified, it defaults to a tensor of value "
        ...
        .TypeConstraint("T2",
                        {"tensor(float16)",        "tensor(float)",          "tensor(double)",
                         ...
                         "tensor(uint32)",         "tensor(uint64)",         "tensor(uint4)",
                         "tensor(int4)",           "tensor(bool)",           "tensor(bfloat16)",
                         "tensor(float8e4m3fn)",   "tensor(float8e4m3fnuz)", "tensor(float8e5m2)",
                         "tensor(float8e5m2fnuz)", "tensor(float4e2m1)",     "tensor(float8e8m0)"},
    ...
    OpSchema()
        ...
        .TypeConstraint("T2",
                        {"tensor(float16)",        "tensor(float)",          "tensor(double)",
                         ...
                         "tensor(int4)",           "tensor(bool)",           "tensor(bfloat16)",
                         "tensor(float8e4m3fn)",   "tensor(float8e4m3fnuz)", "tensor(float8e5m2)",
                         "tensor(float8e5m2fnuz)", "tensor(float4e2m1)"},
                        "Constrain output types to be numerics or boolean.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          if (ctx.getAttribute("value") != nullptr) {
            propagateElemTypeFromDtypeToOutput(ctx, ctx.getAttribute("value"), 0);
