...
Ceil::Ceil(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}

void Ceil::RegisterTuningSchemas() {
  tuning::RegisterParallelTuningSchemas("Ceil", kSupportedElementTypes, kPortableParallelMinimum,
                                        kTuningAbi);
}

KernelTuningKey Ceil::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Ceil", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Ceil::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Ceil", parameters, tuning_, kTuningAbi);
}
