...
bool AnalyzeGatherToSlice(core::builder::GraphGraph &graph, const NodeProto &candidate,
  ...
  std::optional<ProducerPlan> PlanTranspose(const NodeProto &producer, int64_t axis,
                                            bool is_scalar) {
    if (is_scalar) {
  ...
  std::optional<ProducerPlan> PlanReshape(core::builder::GraphGraph & graph,
                                          const NodeProto &producer, int64_t axis, bool is_scalar,
                                          const SymShape &output_shape, int64_t new_length) {
    ...
    if (axis < input_rank && input_shape[static_cast<std::size_t>(axis)] ==
                                 output_shape[static_cast<std::size_t>(axis)]) {
      reshape_input_axis = axis;
    } else {
      const int64_t mirrored = axis + (input_rank - output_rank);
      if (mirrored >= 0 && mirrored < input_rank &&
          input_shape[static_cast<std::size_t>(mirrored)] ==
              output_shape[static_cast<std::size_t>(axis)]) {
        reshape_input_axis = mirrored;
      }
    }
  ...
  std::optional<ProducerPlan> PlanMatMul(core::builder::GraphGraph & graph,
                                         const NodeProto &producer, int64_t axis, bool is_scalar,
                                         const SymShape &output_shape) {
    if (producer.input_size() < 2) {
      return std::nullopt;
  ...
  std::optional<ProducerPlan> PlanProducer(core::builder::GraphGraph & graph,
    ...
    return PlanReductionAxisOp(producer, axis, is_scalar,
                               static_cast<int64_t>(output_shape.Rank()));
  }
