 ...
 * printing or event logging). This is the "kernel initialization" step: the
 * ``(domain, op_type)`` resolution against the model-local function registry,
 * the control-flow handlers, the user-registered custom kernels and the static
 * :cpp:func:`KernelDispatchTable` is performed once here so that a caller (e.g.
 * :cpp:class:`RuntimeSession`) can prepare every node's kernel up front and
 * then invoke it repeatedly without redoing the lookup.
 *
...
/**
 * Invokes an already-resolved kernel for ``node``, wrapping the call with the
 * verbose progress line and (when enabled) the per-node timing event. Shared by
 * :cpp:func:`RunNode` and :cpp:class:`RuntimeSession` so both the
