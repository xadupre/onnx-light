...
TEST(BackendTestCase, AdamCasesArePresent) {
  ...
  // {X_new, V_new, H_new}.
  {
    ...
    EXPECT_EQ(ds.inputs[0].data_type,
              static_cast<int32_t>(core::runtime::DataType::FLOAT)); // R
    EXPECT_EQ(ds.inputs[1].data_type,
              static_cast<int32_t>(core::runtime::DataType::INT64)); // T
    for (size_t i = 2; i < 6; ++i) {
      EXPECT_EQ(ds.inputs[i].data_type, static_cast<int32_t>(core::runtime::DataType::FLOAT));
      EXPECT_EQ(ds.inputs[i].shape, (std::vector<int64_t>{3}));
    }
    for (const core::runtime::Tensor &t : ds.outputs) {
