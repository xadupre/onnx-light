 ...
 * @brief Registers every ``onnx_optim`` ``ComputeShape*`` shape function
 ...
 * ``onnx_optim``, one per ``onnx_optim/shapes/<domain>/shape_<domain>.cc``
 ...
 * the ``onnx_optim`` -> ``onnx_core`` dependency direction: ``onnx_core``
 * never needs to know about ``onnx_optim``'s operator implementations.
 */
 ...
 * this: ``onnx_core`` must not depend on or call into ``onnx_optim``.
 * ``lib_onnx_optim`` is also a plain static archive, so a file-scope static
