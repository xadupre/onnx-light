...
// depend on `onnx_optim`, so it cannot self-populate with the built-in
// `onnx_optim` shape functions. Any test in this binary that exercises
...
// `onnx_optim::RegisterShapeFunctions()` to have run first. Registering it
...
public:
  void SetUp() override { ::onnx_light::onnx_optim::RegisterShapeFunctions(); }
};
