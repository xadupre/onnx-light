...
/// Allocating half-precision binary comparison kernel. Decodes half inputs to
/// float32, applies a comparison op, and produces a BOOL output tensor.
template <typename Op>
Tensor BinaryHalfCompareElementwiseAlloc(const char *op_name, const char *dtype_name, int32_t dtype,
  ...
  const uint16_t *px = reinterpret_cast<const uint16_t *>(x.bytes());
  const uint16_t *py = reinterpret_cast<const uint16_t *>(y.bytes());
  uint8_t *pz = reinterpret_cast<uint8_t *>(z.data.data());

  if (x.shape == y.shape) {
    for (int64_t i = 0; i < bi.element_count; ++i) {
      pz[static_cast<size_t>(i)] = op(decode(px[i]), decode(py[i])) ? 1 : 0;
    }
  } else if (bi.nx == 1 || bi.ny == 1) {
    for (int64_t i = 0; i < bi.element_count; ++i) {
      const float a = bi.nx == 1 ? decode(px[0]) : decode(px[i]);
      const float b = bi.ny == 1 ? decode(py[0]) : decode(py[i]);
      pz[static_cast<size_t>(i)] = op(a, b) ? 1 : 0;
    }
  } else {
    const size_t rank = bi.shape.size();
    std::vector<int64_t> idx(rank, 0);
    for (int64_t flat = 0; flat < bi.element_count; ++flat) {
      ...
      pz[static_cast<size_t>(flat)] = op(decode(px[ox]), decode(py[oy])) ? 1 : 0;
      for (size_t d = rank; d-- > 0;) {
        if (++idx[d] < bi.shape[d]) {
          break;
        }
...
/// In-place half-precision binary comparison kernel. Decodes half inputs to
...
void BinaryHalfCompareElementwise(const char *op_name, const char *dtype_name, int32_t dtype,
  ...
  const uint16_t *px = reinterpret_cast<const uint16_t *>(x.bytes());
  const uint16_t *py = reinterpret_cast<const uint16_t *>(y.bytes());
  uint8_t *pz = reinterpret_cast<uint8_t *>(output.data.data());
