...
# Seed models for fuzz_shape_inference / fuzz_optim_shape_inference.
# Both harnesses read a trailing toggle byte from the input; bit 0x04
# selects the structured-model path and the remaining bits are reserved.
# Seeds use 0x00, which selects the raw-bytes path. Each seed is a valid
# serialized ModelProto + one trailing 0x00 byte so the harness slices
# the toggle off and passes the rest to onnx_light.onnx.load.
...
# Text-format seeds for fuzz_parser. Each string is a valid input to
...
def main() -> int:
    ...
    shape_inference_out = sys.argv[3]
    ...
    # Seed models for fuzz_shape_inference / fuzz_optim_shape_inference
    # covering both the per-op dispatch table (Concat / MatMul / Reshape data
    # propagation, unary chains) and the recursive subgraph visitor (If /
    # Loop). Each seed is a serialized ModelProto with a trailing 0x00
    # toggle byte, which the harness reads as the raw-bytes path and slices
    # off before parsing.
