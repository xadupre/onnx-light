...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axis",
              "(Optional) The axis of the dequantizing dimension of the input tensor. Used only "
              "for per-axis and blocked "
              "quantization. Negative value means counting dimensions from the back. Accepted "
              "range is `[-r, r-1]` "
              "where `r = rank(input)`. When the rank of the input is 1, per-tensor quantization "
              "is applied, "
        ...
        .Attr("block_size",
              ...
              "replicated). Used only for "
              "blocked quantization. The block size is a positive integer. Given `x` shape `(D0, "
              "..., Di, ..., Dn)`, "
        ...
        .Attr(
            ...
            "from `y_zero_point` data type (`T3`). "
            ...
            AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("precision",
              "(Optional) The precision of the division operation between `x` and `y_scale`. If "
              "not provided, "
        ...
        .SetDoc(kDoc_QuantizeLinear_ver25)
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(
            ...
            "the same shape as the input, except for one dimension in which blocking is performed.",
            "T2")
        ...
        .Output(0, "y",
                ...
                "type is specified by the `output_dtype` attribute or, in its absence, the type "
                "of `x_scale`.",
                "T3")
        .Attr("axis",
              "(Optional) The axis of the dequantizing dimension of the input tensor. Used for "
              "per-axis and blocked quantization. Negative value means counting dimensions from "
        ...
        .Attr("block_size",
              ...
              "integer. Given `x` shape `(D0, ..., Di, ..., Dn)`, `y_scale` shape `(S0, ... Si, "
              "...Sn)` and `axis=i`, the accepted range is `[ceil(Di/Si), ceil(Di/(Si-1))-1]`",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("output_dtype",
              "(Optional) The output data type. If not supplied, the output data type is inferred "
    ...
    OpSchema()
        ...
        .TypeConstraint("T3",
                        {"tensor(int8)", "tensor(uint8)", "tensor(int16)", "tensor(uint16)",
                         ...
                         "tensor(float4e2m1)"},
                        "The type of the input `y_zero_point` and the output `y`.")
        .SetDoc(kDoc_QuantizeLinear_ver24)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(
            "output_dtype",
            "(Optional) The output data type. If not supplied, the output data type is inferred "
            "from `y_zero_point` data type (`T2`). "
            "If neither `output_dtype` nor `y_zero_point` are supplied, output data type is uint8. "
        ...
        .TypeConstraint("T2",
                        {"tensor(int8)", "tensor(uint8)", "tensor(int16)", "tensor(uint16)",
                         ...
                         "tensor(float8e5m2fnuz)", "tensor(uint4)", "tensor(int4)"},
                        "The type of the input `y_zero_point` and the output `y`.")
        .SetDoc(kDoc_QuantizeLinear_ver21)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          // The type of y_zero_point is known only if it is present as an input and its type could
          // be inferred (e.g. it is not simply an unresolved formal parameter of an enclosing
          // function).
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .SetDoc(kDoc_DequantizeLinear_ver21)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          propagateElemTypeFromInputToOutput(ctx, 1, 0);
          if (!hasInputShape(ctx, 0)) {
            return;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "y", "N-D quantized output tensor. It has same shape as input 'x'.", "T2")
        .Attr("axis",
              "(Optional) The axis of the quantization dimension of the input tensor. Ignored for "
              "per-tensor quantization. Negative value means counting dimensions from the back. "
        ...
        .SetDoc(kDoc_QuantizeLinear_ver19)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          if (ctx.hasInput(2)) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(
            "axis",
            "(Optional) The axis of the dequantizing dimension of the input tensor. Used only for "
            "per-axis quantization. "
            "Negative value means counting dimensions from the back. Accepted range is `[-r, r-1]` "
            "where `r = rank(input)`. When the rank of the input is 1, per-tensor quantization is "
            "applied, "
            "rendering the axis unnecessary in this scenario.",
            AttributeProto::INT, static_cast<int64_t>(1))
        .TypeConstraint("T1",
                        {"tensor(int8)", "tensor(uint8)", "tensor(int32)", "tensor(float8e4m3fn)",
                         "tensor(float8e4m3fnuz)", "tensor(float8e5m2)", "tensor(float8e5m2fnuz)"},
                        "Constrain 'x_zero_point' and 'x' to 8-bit integer or float, or /32-bit "
                        "integer tensor.")
        .TypeConstraint("T2", {"tensor(float)", "tensor(float16)", "tensor(bfloat16)"},
                        "'x_scale' determines the output type.")
        .SetDoc(kDoc_DequantizeLinear_ver19)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          propagateElemTypeFromInputToOutput(ctx, 1, 0);
          if (!hasInputShape(ctx, 0)) {
            return;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "y", "N-D full precision output tensor. It has same shape as input 'x'.",
                "tensor(float)")
        .Attr("axis",
              "(Optional) The axis of the dequantizing dimension of the input tensor. Ignored for "
              "per-tensor quantization. Negative value means counting dimensions from the back. "
              "Accepted range is [-r, r-1] where r = rank(input).",
              AttributeProto::INT, static_cast<int64_t>(1))
        .TypeConstraint("T", {"tensor(int8)", "tensor(uint8)", "tensor(int32)"},
                        "Constrain 'x_zero_point' and 'x' to 8-bit/32-bit integer tensor.")
        .SetDoc(kDoc_DequantizeLinear_ver13)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          auto y_type = ctx.getOutputType(0);
          // only float is supported
          y_type->mutable_tensor_type()->set_elem_type(ONNX_LIGHT_NAMESPACE::TensorProto::FLOAT);
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T2", {"tensor(int8)", "tensor(uint8)"},
                        "Constrain 'y_zero_point' and 'y' to 8-bit integer tensor.")
        .SetDoc(kDoc_QuantizeLinear_ver10)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          if (ctx.hasInput(2)) {
            if (ctx.getInputType(2) != nullptr) {
              propagateElemTypeFromInputToOutput(ctx, 2, 0);
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"tensor(int8)", "tensor(uint8)", "tensor(int32)"},
                        "Constrain 'x_zero_point' and 'x' to 8-bit/32-bit integer tensor.")
        .SetDoc(kDoc_DequantizeLinear_ver10)
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          auto y_type = ctx.getOutputType(0);
          // only float is supported
          y_type->mutable_tensor_type()->set_elem_type(ONNX_LIGHT_NAMESPACE::TensorProto::FLOAT);
