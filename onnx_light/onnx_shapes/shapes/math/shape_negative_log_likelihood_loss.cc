...
void ComputeShapeNegativeLogLikelihoodLoss(ShapesContext &ctx, const NodeProto &node,
  ...
  const SymTensor &target_tensor = ctx.Get(target);
  const SymShape &target_shape = target_tensor.Shape();
  ...
  if (weight != nullptr && weight[0] != '\0') {
    const SymTensor &weight_tensor = ctx.Get(weight);
  ...
  if (reduction == "none") {
    ...
    ctx.Set(node.output(0), SymTensor(nullptr, input_tensor.Dtype(), target_shape));
  } else {
    // Scalar output (rank 0).
    ctx.Set(node.output(0), SymTensor(nullptr, input_tensor.Dtype(), SymShape()));
  }
}
