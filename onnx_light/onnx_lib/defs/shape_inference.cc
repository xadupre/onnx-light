...
void propagateElemTypeFromTensorInputToOutput(InferenceContext &ctx, size_t inputIndex,
                                              size_t outputIndex) {
  ...
  if (input_value_case != TypeProto::kTensorType &&
      input_value_case != TypeProto::kSparseTensorType) {
    fail_type_inference("Input ", inputIndex,
                        " expected to have tensor or sparse tensor type. Got: ", input_value_case);
  }
  ...
  if (output_value_case == TypeProto::kTensorType ||
    ...
    fail_type_inference("Output ", outputIndex,
                        " expected to have tensor or sparse tensor type. Got: ", output_value_case);
  }
}

static void propagateElemTypeFromSequenceInputToOutput(InferenceContext &ctx, size_t inputIndex,
                                                       size_t outputIndex) {
  const auto *const input_type = ctx.getInputType(inputIndex);
...
static void propagateElemTypeFromOptionalInputToOutput(InferenceContext &ctx, size_t inputIndex,
                                                       size_t outputIndex) {
  const auto *const input_type = ctx.getInputType(inputIndex);
...
static void propagateElemTypeFromMapInputToOutput(InferenceContext &ctx, size_t inputIndex,
                                                  size_t outputIndex) {
  const auto *const input_type = ctx.getInputType(inputIndex);
...
static void mergeInShapeInfo(const TensorShapeProto &source, TensorShapeProto &target) {
  ...
  if (num_source_dims != num_target_dims) {
    fail_shape_inference("Mismatch between number of inferred and declared dimensions. inferred=",
                         num_source_dims, " declared=", num_target_dims);
  }
...
static void UnionShapeInfoForTensor(const TensorShapeProto &source_shape,
...
static void UnionShapeInfo(const TypeProto_SparseTensor &source_type,
                           TypeProto_SparseTensor &target_type) {
  // The union of a tensor of unknown rank and a tensor of known rank is a tensor of unknown rank.
  // Hence, if the source_type had unknown rank, we clear the shape of the target_type.
  // Otherwise, UnionShapeInfoForTensor handles the rest.
  if (source_type.has_shape()) {
...
void UnionTypeInfo(const TypeProto &source_type, TypeProto &target_type) {
  if (source_type.value_case() != target_type.value_case()) {
    fail_type_inference("Mismatched type:", " inferred=", source_type.value_case(),
                        " declared=", target_type.value_case());
  }
  ...
  if (target_case == TypeProto::ValueCase::kTensorType) {
    ...
    if (source_elem_type != target_elem_type) {
      fail_type_inference("Mismatched tensor element type:", " inferred=",
                          Utils::DataTypeUtils::ToDataTypeString(source_elem_type),
                          " declared=", Utils::DataTypeUtils::ToDataTypeString(target_elem_type));
    }
    ...
    if (source_elem_type != target_elem_type) {
      fail_type_inference("Mismatched sparse tensor element type:", " inferred=",
                          Utils::DataTypeUtils::ToDataTypeString(source_elem_type),
                          " declared=", Utils::DataTypeUtils::ToDataTypeString(target_elem_type));
    }
    ...
    UnionTypeInfo(source_type.optional_type().elem_type(),
                  *target_type.mutable_optional_type()->mutable_elem_type());
    ...
    if (source_key_type != target_key_type) {
      fail_type_inference("Mismatched map tensor key type:", " inferred=",
                          Utils::DataTypeUtils::ToDataTypeString(source_key_type),
                          " declared=", Utils::DataTypeUtils::ToDataTypeString(target_key_type));
    }
    ...
    UnionTypeInfo(source_type.map_type().value_type(),
                  *target_type.mutable_map_type()->mutable_value_type());
  }
}
...
static void propagateTensorElemTypeWithValidation(const TypeProto *input_type,
  ...
  if (input_value_case == TypeProto::kTensorType ||
    ...
    fail_type_inference("Input was expected to have tensor or sparse tensor type. Got ",
                        input_value_case);
  }
  ...
  if (output_value_case == TypeProto::VALUE_NOT_SET) {
    setTensorElementType(input_elem_type, input_value_case, *output_type);
  } else if (output_value_case == TypeProto::kTensorType ||
             output_value_case == TypeProto::kSparseTensorType) {
    ...
    if (output_elem_type != TensorProto::UNDEFINED) {
      if (input_elem_type != output_elem_type) {
        fail_type_inference("Input element type of ", input_elem_type,
                            " does not match existing output type of ", output_elem_type);
      }
    } else {
      setTensorElementType(input_elem_type, output_value_case, *output_type);
    }
...
static void propagateSequenceElemTypeWithValidation(const TypeProto *input_type,
                                                    TypeProto *output_type) {
  if (nullptr == input_type) {
    fail_type_inference("Input type was null");
  }
  ...
  if (input_seq_type.has_elem_type()) {
    propagateElemTypeWithValidation(&input_seq_type.elem_type(),
...
static void propagateOptionalElemTypeWithValidation(const TypeProto *input_type,
                                                    TypeProto *output_type) {
  if (nullptr == input_type) {
    fail_type_inference("Input type was null");
  }
  ...
  if (input_opt_type.has_elem_type()) {
    propagateElemTypeWithValidation(&input_opt_type.elem_type(),
                                    output_type->mutable_optional_type()->mutable_elem_type());
  } else {
    fail_type_inference("Element type of optional input was unknown");
  }
}

static void propagateMapElemTypeWithValidation(const TypeProto *input_type,
                                               TypeProto *output_type) {
  if (nullptr == input_type) {
    fail_type_inference("Input type was null");
  }
  ...
  output_type->mutable_map_type()->set_key_type(input_map_type.key_type());
  propagateElemTypeWithValidation(&input_map_type.value_type(),
                                  output_type->mutable_map_type()->mutable_value_type());
}
...
std::pair<int, int> getAttributeProtoElemTypeAndLength(const AttributeProto *attr_proto) {
  if (attr_proto->ints_size()) {
    return {static_cast<int>(TensorProto_DataType_INT64),
            static_cast<int>(attr_proto->ints_size())};
  } else if (attr_proto->floats_size()) {
    return {static_cast<int>(TensorProto_DataType_FLOAT),
            static_cast<int>(attr_proto->floats_size())};
  } else if (attr_proto->strings_size()) {
    return {static_cast<int>(TensorProto_DataType_STRING),
            static_cast<int>(attr_proto->strings_size())};
  ...
  for (const auto &attribute : attribute_names) {
    ...
    if (attr_proto != nullptr) {
      if (elem_type != TensorProto::UNDEFINED) {
        ...
        fail_shape_inference("One and only one attribute must be set out of ",
