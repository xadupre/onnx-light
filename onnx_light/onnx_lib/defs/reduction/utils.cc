...
static std::vector<std::string> GetSupportedDataTypesForReductionOps(bool supports8bit,
                                                                     bool supports_bool) {
...
to `False` instead of `True`.)DOC";
    ...
    if (axes_input) {
      schema.Attr("noop_with_empty_axes",
      ...
      schema.Input(1, "axes",
                   ...
                   "When axes is empty (either not provided or explicitly empty), behavior depends "
                   "on 'noop_with_empty_axes': "
    ...
    schema.TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
      ...
      for (int64_t &axe : axes) {
        if (axe < -input_ndim || axe >= input_ndim) {
          fail_shape_inference("axis=", axe, " must be in [-rank, rank-1] (rank=", input_ndim, ") (3).");
        }
        if (axe < 0)
          axe += input_ndim;
      }
