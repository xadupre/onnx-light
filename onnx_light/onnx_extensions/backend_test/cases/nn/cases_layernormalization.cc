...
void RegisterCase(std::vector<TestCase> &registry, const OpsetId &opset, const std::string &base,
  ...
  Expect(registry, std::move(node), "test_cc_" + base, {opset},
         [opset, x_shape, normalized_shape, axis, epsilon]() -> IoData {
           const KernelContext ctx{opset};
           const onnx_kernels::kernel::LayerNormalization kernel{ctx};
...
void RegisterLayerNormalizationCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_layer_normalization_2d_axis0_benchmark", {opset},
           {x_count, x_count, x_count}, {x_count, 1, 1}, [x_shape, normalized_shape]() -> IoData {
             const OpsetId opset = DefaultOpset(17);

             const KernelContext layernorm_kernel_ctx{opset};
             const onnx_kernels::kernel::LayerNormalization layernorm_kernel{layernorm_kernel_ctx};
  ...
  RegisterCase(registry, opset, "layer_normalization_2d_axis1", {3, 4},
               /*axis=*/1, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_2d_axis_negative_1", {3, 4},
               /*axis=*/-1, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_2d_axis_negative_2", {3, 4},
               /*axis=*/-2, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);

  // 3D cases (shape [2, 3, 5]) with explicit epsilon.
  RegisterCase(registry, opset, "layer_normalization_3d_axis0_epsilon", {2, 3, 5},
               /*axis=*/0, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);
  RegisterCase(registry, opset, "layer_normalization_3d_axis1_epsilon", {2, 3, 5},
               /*axis=*/1, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);
  RegisterCase(registry, opset, "layer_normalization_3d_axis2_epsilon", {2, 3, 5},
               /*axis=*/2, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);
  RegisterCase(registry, opset, "layer_normalization_3d_axis_negative_1_epsilon", {2, 3, 5},
               /*axis=*/-1, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);
  RegisterCase(registry, opset, "layer_normalization_3d_axis_negative_2_epsilon", {2, 3, 5},
               /*axis=*/-2, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);
  RegisterCase(registry, opset, "layer_normalization_3d_axis_negative_3_epsilon", {2, 3, 5},
               /*axis=*/-3, /*include_axis_attr=*/true, kAltEpsilon,
               /*include_epsilon_attr=*/true);

  // 4D cases (shape [2, 3, 4, 5]).
  RegisterCase(registry, opset, "layer_normalization_4d_axis0", {2, 3, 4, 5},
               /*axis=*/0, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis1", {2, 3, 4, 5},
               /*axis=*/1, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis2", {2, 3, 4, 5},
               /*axis=*/2, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis3", {2, 3, 4, 5},
               /*axis=*/3, /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis_negative_1", {2, 3, 4, 5}, /*axis=*/-1,
               /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis_negative_2", {2, 3, 4, 5}, /*axis=*/-2,
               /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis_negative_3", {2, 3, 4, 5}, /*axis=*/-3,
               /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);
  RegisterCase(registry, opset, "layer_normalization_4d_axis_negative_4", {2, 3, 4, 5}, /*axis=*/-4,
               /*include_axis_attr=*/true, kDefaultEpsilon,
               /*include_epsilon_attr=*/false);

  // Default axis (-1) with default epsilon; no attributes set.
  RegisterCase(registry, opset, "layer_normalization_default_axis", {2, 3, 4, 5},
