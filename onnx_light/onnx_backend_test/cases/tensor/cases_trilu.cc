...
void RegisterTriluCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    RegisterLazyBenchmarkCase(registry, std::move(node), "test_cc_trilu_upper_default_benchmark",
                              {opset}, {4194304}, {4194304}, [trilu_kernel]() -> IoData {
                                Tensor x = Tensor::FromFloat("X", {2048, 2048},
