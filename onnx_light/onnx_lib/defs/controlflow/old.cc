...
#include "onnx_lib/defs/controlflow/utils.h"
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("else_branch",
              ...
              AttributeProto::GRAPH)
        .TypeConstraint("V", control_flow_types_ir12(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("else_branch",
              ...
              " the number of outputs in the then_branch.",
              AttributeProto::GRAPH)
        .TypeConstraint("V", control_flow_types_ir11(),
                        "All Tensor, Sequence(Tensor), Optional(Tensor), and "
                        "Optional(Sequence(Tensor)) types up to IRv11.")
        .TypeConstraint("B", {"tensor(bool)"}, "Only bool")
        .TypeAndShapeInferenceFunction(IfInferenceFunction));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("else_branch",
              ...
              " the number of outputs in the then_branch.",
              AttributeProto::GRAPH)
        .TypeConstraint("V", control_flow_types_ir10(),
                        "All Tensor, Sequence(Tensor), Optional(Tensor), and "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("else_branch",
              ...
              " the number of outputs in the then_branch.",
              AttributeProto::GRAPH)
        .TypeConstraint("V", control_flow_types_ir9(),
                        "All Tensor, Sequence(Tensor), Optional(Tensor), and "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("else_branch",
              ...
              " the number of outputs in the then_branch.",
              AttributeProto::GRAPH)
        .TypeConstraint("V", control_flow_types_ir4(),
                        "All Tensor, Sequence(Tensor), Optional(Tensor), and "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "M",
               ...
               "I", OpSchema::Optional)
        .Input(1, "cond", "A boolean termination condition. Optional. Pass empty string to skip.",
               "B", OpSchema::Optional)
        .Input(2, "v_initial",
               "The initial values of any loop-carried dependencies (values that "
               "change across loop iterations)",
               "V", OpSchema::Variadic, false, 0)
        .Output(0, "v_final_and_scan_outputs",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .SetDoc(kDoc_scan_24)
        .Input(0, "initial_state_and_scan_inputs",
               "Initial values of the loop's N state variables followed by M scan_inputs", "V",
               OpSchema::Variadic, false)
        .Output(0, "final_state_and_scan_outputs",
        ...
        .Attr("body",
              ...
              AttributeProto::GRAPH, true)
        .Attr("num_scan_inputs", "An attribute specifying the number of scan_inputs M. ",
        ...
        .TypeAndShapeInferenceFunction(
            ScanInferenceFunction)); // Shares same shape inference as opset 11
...
void ScanInferenceFunction_opset8(InferenceContext &ctx) {
  ...
  if (!output_types.empty()) {
    ...
    if (output_types.size() != num_outputs) {
      fail_type_inference("Graph attribute inferencing returned type information for ",
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      if (!subgraph_output_type->has_tensor_type()) {
        fail_type_inference("Scan 'body' subgraph outputs should all be tensors but output ", i,
                            " was not");
      }
...
static void ScanInferenceFunction_opset9(InferenceContext &ctx) {
  ...
  if (getRepeatedAttribute(ctx, "scan_input_axes", axes)) {
    if (axes.size() != num_scan_inputs) {
      fail_shape_inference("Number of scan input axes specified (", axes.size(),
  ...
  if (getRepeatedAttribute(ctx, "scan_output_axes", output_axes)) {
    if (output_axes.size() != num_scan_outputs) {
      fail_shape_inference("Number of scan output axes specified (", output_axes.size(),
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      if (is_loop_state_var) {
        ...
        if (subgraph_output_tensor_type.has_shape()) {
          ...
          output_axis =
              handle_negative_axis_validate_opset9("scan_output_axes", output_axis, output_rank);
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "sequence_lens",
               ...
               "I", OpSchema::Optional)
        .Input(1, "initial_state_and_scan_inputs",
               "Initial values of the loop's N state variables followed by M scan_inputs", "V",
               OpSchema::Variadic, false)
        .Output(0, "final_state_and_scan_outputs",
                "Final values of the loop's N state variables followed by K scan_outputs", "V",
...
static void LoopInferenceFunction_opset8(InferenceContext &ctx) {
  ...
  if (!subgraph_output_types.empty()) {
    ...
    if (subgraph_output_types.size() != num_outputs + 1) {
      fail_type_inference("Graph attribute inferencing returned type information for ",
                          subgraph_output_types.size(), " outputs. Expected ", num_outputs + 1);
    }
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      if (!subgraph_output_type->has_tensor_type()) {
        fail_type_inference("Loop 'body' subgraph outputs should all be tensors but output ", i,
                            " was ", subgraph_output_type->value_case());
      }
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "v_initial",
               ...
               "V", OpSchema::Variadic, false)
        .Output(0, "v_final_and_scan_outputs",
                "Final N loop carried dependency values then K scan_outputs", "V",
                OpSchema::Variadic, false)
        .Attr("body",
              "The graph run each iteration. It has 2+N inputs: (iteration_num, "
              "condition, loop carried dependencies...). It has 1+N+K outputs: "
...
static void IfInferenceFunction_opset1(InferenceContext &ctx) {
  ...
  if (num_then_outputs != num_else_outputs) {
    fail_type_inference("then_branch and else_branch produce different number of outputs. ",
                        num_then_outputs, " != ", num_else_outputs);
  }
  ...
  for (size_t i = 0, end = then_output_types.size(); i < end; ++i) {
    ...
    if (then_output->value_case() != else_output->value_case()) {
      fail_type_inference("Mismatched type for output ", i, " then=", then_output->value_case(),
                          " else=", else_output->value_case());
    }
    ...
    *if_output = *then_output;
    ...
    if (then_output->has_tensor_type()) {
      ...
      if (then_elem_type != else_elem_type) {
        fail_type_inference("Mismatched tensor element type for output ", i,
                            " then=", then_elem_type, " else=", else_elem_type);
      }
  ...
  if (!subgraph_output_types.empty()) {
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      if (!subgraph_output_type->has_tensor_type() && !subgraph_output_type->has_sequence_type()) {
        fail_type_inference(
            "Loop 'body' subgraph outputs should all be tensors or sequences but output ", i,
            " was ", subgraph_output_type->value_case());
      }

      if (!is_loop_state_var && !subgraph_output_type->has_tensor_type()) {
        fail_type_inference("Loop 'body' subgraph scan outputs should all be tensors but output ",
                            i, " was ", subgraph_output_type->value_case());
      }
