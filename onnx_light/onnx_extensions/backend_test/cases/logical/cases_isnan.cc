...
void RegisterIsNaNCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_isnan_benchmark", {opset}, {count}, {count},
           []() -> IoData {
             ...
             const KernelContext isnan_kernel_ctx{opset};
             const onnx_kernels::kernel::IsNaN isnan_kernel{isnan_kernel_ctx};
  ...
  {
    NodeProto node = MakeNode("IsNaN", {"x"}, {"y"});
    Expect(registry, std::move(node), "test_cc_isnan", {opset}, [nan_v]() -> IoData {
      const OpsetId opset = DefaultOpset(20);
  ...
  {
    NodeProto node = MakeNode("IsNaN", {"x"}, {"y"});
    Expect(registry, std::move(node), "test_isnan", {opset}, [nan_v, inf_v]() -> IoData {
      const OpsetId opset = DefaultOpset(20);
  ...
  {
    NodeProto node = MakeNode("IsNaN", {"x"}, {"y"});
    Expect(registry, std::move(node), "test_isnan_float16", {opset}, [nan_v, inf_v]() -> IoData {
      const OpsetId opset = DefaultOpset(20);
  ...
  {
    NodeProto node = MakeNode("IsNaN", {"x"}, {"y"});
    Expect(registry, std::move(node), "test_cc_isnan_bfloat16", {opset},
           [nan_v, inf_v]() -> IoData {
             const OpsetId opset = DefaultOpset(20);
