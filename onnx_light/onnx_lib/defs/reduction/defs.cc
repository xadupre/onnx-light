...
ONNX_OPERATOR_SET_SCHEMA(ReduceMax, 20,
                         OpSchema().FillUsing(ReduceOpGenerator("max", EMPTY_MIN, true, true,
                                                                nullptr, nullptr, true)));

ONNX_OPERATOR_SET_SCHEMA(ReduceMin, 20,
                         OpSchema().FillUsing(ReduceOpGenerator("min", EMPTY_MAX, true, true,
                                                                nullptr, nullptr, true)));

ONNX_OPERATOR_SET_SCHEMA(ReduceSum, 13,
                         OpSchema().FillUsing(ReduceOpDynamicAxes("sum", EMPTY_ZERO)));
...
ONNX_OPERATOR_SET_SCHEMA(ReduceSumSquare, 18,
                         OpSchema().FillUsing(ReduceFunctionOp("sum square", EMPTY_ZERO,
                                                               reduce_sum_square_func_body)));

ONNX_OPERATOR_SET_SCHEMA(ReduceMean, 18,
                         OpSchema().FillUsing(ReduceOpDynamicAxes("mean", EMPTY_UNDEFINED)));

ONNX_OPERATOR_SET_SCHEMA(ReduceProd, 18,
                         OpSchema().FillUsing(ReduceOpDynamicAxes("product", EMPTY_ONE)));

ONNX_OPERATOR_SET_SCHEMA(ReduceLogSum, 28,
                         OpSchema().FillUsing(ReduceFunctionOpFloatOnly("log sum", EMPTY_MINUS_INF,
                                                                        reduce_log_sum_func_body)));

ONNX_OPERATOR_SET_SCHEMA(ReduceLogSumExp, 28,
                         OpSchema().FillUsing(ReduceFunctionOpFloatOnly(
                             "log sum exponent", EMPTY_MINUS_INF, reduce_log_sum_exp_func_body)));
...
ONNX_OPERATOR_SET_SCHEMA(ReduceL1, 18,
                         OpSchema().FillUsing(ReduceFunctionOp("L1 norm", EMPTY_ZERO,
                                                               reduce_l1_func_body)));
...
ONNX_OPERATOR_SET_SCHEMA(ReduceL2, 18,
                         OpSchema().FillUsing(ReduceFunctionOp("L2 norm", EMPTY_ZERO,
                                                               reduce_l2_func_body)));
...
The type of the output tensor is integer.)DOC";
    ...
    schema.Attr("select_last_index",
                ...
                AttributeProto::INT, static_cast<int64_t>(0));
    schema.Input(0, "data", "An input tensor.", "T", OpSchema::Single, true, 1,
