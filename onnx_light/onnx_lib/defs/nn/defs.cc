...
#include "onnx_lib/defs/nn/utils.h"
...
static constexpr const char *pads_doc =
    ...
    "auto_pad attribute. If not present, the padding defaults to 0 along start and end of each "
    "spatial axis.";
...
ONNX_API void convPoolShapeInference(InferenceContext &ctx, bool use_dilation,
                                     bool require_kernel_shape, int input1Idx, int input2Idx) {
  ...
  std::vector<int64_t> strides = getConvPoolStrides(ctx, n_input_dims);
  ...
  for (int i = 0; i < kernel_shape_size; ++i) {
    ...
    int64_t effective_input_size =
        checkedAdd(checkedAdd(input_size, pads[i]), pads[i + kernel_shape_size]);
    ...
    if (ceil_mode == 1 && checkedMultiply(checkedSubtract(output_size, 1), strides[i]) >=
                              checkedAdd(input_size, pads[i])) {
      // we need to match pytorch's behavior of "Sliding windows that would start in the right
      // padded region are ignored."
      // (https://pytorch.org/docs/stable/generated/torch.nn.MaxPool1d.html#maxpool1d). this code
      // follows the same logic as PyTorch's C++ implementation:
...
static std::function<void(OpSchema &)> PoolOpSchemaGenerator(const char *name, const char *opName,
 ...
 {additionalDescription}
                        ...
                        ReplaceAll(doc, "{name}", name); ReplaceAll(doc, "{opName}", opName);
                        ReplaceAll(doc, "{additionalDescription}", additionalDescription);
                        ReplaceAll(doc, "{kernelSpatialShape}",
                                   use_dilation
    ...
    schema.Attr("ceil_mode", "Whether to use ceil or floor (default) to compute the output shape.",
    ...
    schema.Output(0, "Y",
                  ...
                  "T", OpSchema::Single, true, 1, OpSchema::Differentiable);
    schema.TypeConstraint("T", GetSupportedDataTypesForPoolingOps(supports8bit),
                          supports8bit
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .FillUsing(PoolOpSchemaGenerator(
            ...
            "when attribute count_include_pad is zero).",
        ...
        .Attr("count_include_pad",
              "Whether include pad pixels when calculating values for the edges. Default is 0, "
              "doesn't count include pad.",
              AttributeProto::INT, static_cast<int64_t>(0)));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .FillUsing(PoolOpSchemaGenerator(
            "MaxPool", "max",
            "The output of each pooling window is maximum number of elements exclude pad. ", true,
            true))
        .Attr("storage_order",
        ...
        .Output(1, "Indices",
                ...
                "I", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
...
static void maxUnpoolShapeInference(InferenceContext &ctx) {
  ...
  *final_output_shape->add_dim() = ctx.getInputType(1)->tensor_type().shape().dim(
  ...
  for (int i = 0; i < kernel_shape_size; ++i) {
    ...
    int64_t newdim_value =
        checkedMultiply(strides[i], checkedSubtract(input_shape.dim(2 + i).dim_value(), 1));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               ...
               "data tensor to arrive with the dimension denotation "
               "of [DATA_BATCH, DATA_CHANNEL, DATA_FEATURE, DATA_FEATURE ...].",
               "T1", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(
            1, "I",
...
static std::function<void(OpSchema &)> LpPoolOpSchemaGenerator(const char *name) {
 ...
 ```)DOC";
    ...
    schema.Attr("auto_pad", conv_auto_pad_doc, AttributeProto::STRING, std::string("NOTSET"));
    schema.Attr("pads", pads_doc, AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("p", "p value of the Lp norm used to pool over the input data.",
...
static std::function<void(OpSchema &)> RoiPoolOpSchemaGenerator(const char *name) {
 ...
 (num_rois, channels, pooled_shape[0], pooled_shape[1]).)DOC";
    ...
    schema.Input(1, "rois",
                 ...
                 "T", OpSchema::Single, true, 1, OpSchema::NonDifferentiable);
    schema.Output(0, "Y",
...
computes the output.)DOC";
    ...
    schema.Input(1, "W",
                 ...
                 "groups G.",
                 "T", OpSchema::Single, true, 1, OpSchema::Differentiable);
    schema.Input(2, "B", "Optional 1D bias to be added to the convolution, has size of M.", "T",
    ...
    schema.Attr("auto_pad", conv_auto_pad_doc, AttributeProto::STRING, std::string("NOTSET"));
    schema.Attr("pads", pads_doc, AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("group", "number of groups input channels and output channels are divided into.",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("kernel_shape",
              "The shape of the convolution kernel. If not present, should be inferred from input "
              "'w'.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("dilations",
        ...
        .Attr("pads",
              ...
              "xi_begin the number of"
              "pixels added at the beginning of axis `i` and xi_end, the number of pixels added at "
              "the end of axis `i`."
              "This attribute cannot be used simultaneously with auto_pad attribute. If not "
              "present, the padding defaults"
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (nullptr == x_zero_point_type ||
            ...
            fail_type_inference("input and zero_point pair is expected to have be same type in ",
                                ctx.getDisplayName(), ".");
          }
          ...
          if (nullptr == w_zero_point_type ||
            ...
            fail_type_inference("weight and zero_point pair is expected to have same type in ",
                                ctx.getDisplayName(), ".");
          }
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T3", {"tensor(int32)"},
...
ONNX_API void convTransposeShapeInference(InferenceContext &ctx) {
  ...
  *final_output_shape->add_dim() = input_shape.dim(0);
  *final_output_shape->add_dim() = ctx.getInputType(1)->tensor_type().shape().dim(1) *
...
output_shape can also be explicitly specified in which case pads values are auto generated using these equations:
  ...
  Else: pads[start_i] = total_padding[i] - (total_padding[i]/2); pads[end_i] = (total_padding[i]/2).
    ...
    schema.Attr("output_shape",
                "The shape of the output can be explicitly set which will cause pads values to be "
                "auto generated. If output_shape is specified "
                "pads values are ignored. See doc for details for equations to generate pads. Note "
                "that the output_shape attribute value "
                "should not include dimensions for batch size and channels, which are "
                "automatically inferred.",
                AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr(
        "output_padding",
        "Additional elements added to the side with higher coordinate indices in the output. "
        "Each padding value in \"output_padding\" must be less than the corresponding "
    ...
    schema.Attr("strides",
                ...
                "each spatial axis.",
                AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("auto_pad", conv_transpose_auto_pad_doc, AttributeProto::STRING,
                std::string("NOTSET"));
    schema.Attr("pads", pads_doc, AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("group", "number of groups input channels and output channels are divided into.",
                AttributeProto::INT, static_cast<int64_t>(1));
...
ONNX_OPERATOR_SET_SCHEMA(ConvTranspose, 22,
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("dilations",
              ...
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("group",
              "Number of groups the input and output channels, C and oC, are divided into. C and "
              "oC must both "
              "be divisible by group. Default is 1.",
              AttributeProto::INT, static_cast<int64_t>(1))
        .Attr("kernel_shape",
              "Shape of the convolution kernel. If not present, it is inferred from the shape of "
              "input W.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("offset_group",
              ...
              AttributeProto::INT, static_cast<int64_t>(1))
        .Attr("pads",
              "Padding for the beginning and end along each spatial axis. The values represent the "
              "number of pixels "
              "added to the beginning and end of the corresponding axis and can take any "
              "nonnegative value. "
              "The format should be as follows: [x1_begin, x2_begin, ..., x1_end, x2_end, ...], "
              "where xi_begin "
              "is the number of pixels added at the beginning of axis `i` and xi_end is the number "
              "of pixels "
              "added at the end of axis `i`. Default is 0 along each axis.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("strides", "Stride along each spatial axis. Default is 1 along each axis.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateElemTypeFromInputToOutput(ctx, 0, 0);
          convPoolShapeInference(ctx, true, false, 0, 1);
...
static std::function<void(OpSchema &)> GlobalPoolingOpSchemaGenerator(const char *op_type,
                                                                      const char *op) {
  return [=](OpSchema &schema) {
    std::string doc;
    POPULATE_OP_DOC_STR(doc = R"DOC(
 ...
 equal to the spatial dimension of input tensor.)DOC";
                        ReplaceAll(doc, "{op_type}", op_type); ReplaceAll(doc, "{op}", op););
    schema.SetDoc(doc);
    schema.Input(0, "X",
                 "Input data tensor from the previous operator; "
                 "dimensions for image case are (N x C x H x W), "
...
ONNX_OPERATOR_SET_SCHEMA(GlobalAveragePool, 22,
                         OpSchema().FillUsing(GlobalPoolingOpSchemaGenerator("AveragePool",
                                                                             "average")));
ONNX_OPERATOR_SET_SCHEMA(GlobalMaxPool, 22,
                         OpSchema().FillUsing(GlobalPoolingOpSchemaGenerator("MaxPool", "max")));

static std::function<void(OpSchema &)> GlobalLpPoolingOpSchemaGenerator(const char *op_type,
                                                                        const char *op) {
  return [=](OpSchema &schema) {
    std::string doc;
    POPULATE_OP_DOC_STR(doc = R"DOC(
...
ONNX_OPERATOR_SET_SCHEMA(GlobalLpPool, 22,
                         OpSchema().FillUsing(GlobalLpPoolingOpSchemaGenerator("LpPool",
                                                                               "lp pool")));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("training_mode",
              "If set to true, it indicates BatchNormalization is being used for training, and "
              "outputs 1 "
        ...
        .Input(0, "X",
               ...
               "The op also accepts single dimension input of size N in which case C is assumed to "
               "be 1",
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(1, "scale", "Scale tensor of shape (C).", "T1", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(2, "B", "Bias tensor of shape (C).", "T1", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(3, "input_mean",
               "running (training) or estimated (testing) mean tensor of shape (C).", "T2",
               OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(4, "input_var",
               "running (training) or estimated (testing) variance tensor of shape (C).", "T2",
               OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Output(0, "Y", "The output tensor of the same shape as X", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .Output(1, "running_mean", "The running mean after the BatchNormalization operator.", "T2",
                OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(2, "running_var",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (ctx.getAttribute("training_mode") &&
            ...
            if (ctx.getNumOutputs() != 3)
              fail_shape_inference("This number of op outputs should be 3 when Training_mode = "
                                   "True, but it is not.");
          } else {
            if (ctx.getNumOutputs() != 1)
              fail_shape_inference("This number of op outputs should be 1 when Training_mode = "
                                   "False, but it is not.");
          }
...
ONNX_OPERATOR_SET_SCHEMA(InstanceNormalization, 22,
                         OpSchema()
                             ...
                             .Input(2, "B", "The input 1-dimensional bias tensor of size C.", "T",
                                    OpSchema::Single, true, 1, OpSchema::Differentiable)
                             .Output(0, "output", "The output tensor of the same shape as input.",
                                     "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
                             .TypeConstraint("T", OpSchema::all_float_types_ir4(),
                                             "Constrain input and output types to float tensors.")
                             .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output", "Matrix after normalization", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_float_types_ir4(),
                        "Constrain input and output types to float tensors.")
        .SetDoc(kDoc_LpNormalization_ver1)
        .Attr("axis", "The axis on which to apply normalization, -1 mean last axis.",
              AttributeProto::INT, static_cast<int64_t>(-1))
        .Attr("p", "The order of the normalization, only 1 or 2 are supported.",
              AttributeProto::INT, static_cast<int64_t>(2))
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateShapeAndTypeFromFirstInput(ctx);
        }));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "The input data as Tensor.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(1, "ratio",
        ...
        .Input(2, "training_mode",
               ...
               "T2", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(0, "output", "The output.", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .Output(1, "mask", "The output mask.", "T2", OpSchema::Optional, true, 1,
                OpSchema::NonDifferentiable)
        .TypeConstraint("T", OpSchema::all_float_types_ir10(),
                        "Constrain input and output types to float tensors.")
        .TypeConstraint("T1", OpSchema::all_float_types_ir10(),
...
ONNX_OPERATOR_SET_SCHEMA(Shrink, 9,
                         OpSchema()
                             .SetDoc(Shrink_ver9_doc)
                             .Attr("lambd",
                                   "The lambd value for the Shrink formulation. Default is 0.5.",
                                   AttributeProto::FLOAT, 0.5f)
                             .Attr("bias", "The bias value added to output. Default is 0.",
                                   AttributeProto::FLOAT, 0.0f)
                             .Input(0, "input", "The input data as Tensor.", "T", OpSchema::Single,
                                    true, 1, OpSchema::Differentiable)
                             .Output(0, "output", "The output.", "T", OpSchema::Single, true, 1,
                                     OpSchema::Differentiable)
                             .TypeConstraint("T", OpSchema::all_numeric_types(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input", "A tensor of rank >= axis.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Output(0, "output",
                ...
                "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir13(),
...
ONNX_OPERATOR_SET_SCHEMA(
    TfIdfVectorizer, 9,
    OpSchema()
        .Input(0, "X", "Input for n-gram extraction", "T", OpSchema::Single, true, 1,
               OpSchema::NonDifferentiable)
        .Output(0, "Y", "Ngram results", "T1", OpSchema::Single, true, 1,
                OpSchema::NonDifferentiable)
        .TypeConstraint("T", {"tensor(string)", "tensor(int32)", "tensor(int64)"},
                        "Input is ether string UTF-8 or int32/int64")
        .TypeConstraint("T1", {"tensor(float)"}, "1-D tensor of floats")
        .Attr("max_gram_length",
              "Maximum n-gram length. If this value is 3, 3-grams will be used to generate the "
              "output.",
              AttributeProto::INT)
        .Attr("min_gram_length",
        ...
        .Attr("max_skip_count",
              ...
              "n-gram from X. "
              "If max_skip_count=1, min_gram_length=2, max_gram_length=3, this operator may "
              "generate 2-grams "
        ...
        .Attr("pool_strings",
              ...
              "It's an 1-D tensor starting with the collections of all 1-grams and ending with the "
              "collections of n-grams. "
              "The i-th element in pool stores the n-gram that should be mapped to coordinate "
              "ngram_indexes[i] in the output vector.",
        ...
        .Attr("ngram_counts",
              ...
              "It is useful when determining the boundary between two consecutive collections of "
              "n-grams. "
              "For example, if ngram_counts is [0, 17, 36], the first index (zero-based) of "
        ...
        .Attr("ngram_indexes",
              ...
              "'pool_*' attribute. "
              "The i-th element in ngram_indexes indicate the coordinate of the i-th n-gram in the "
              "output tensor.",
              AttributeProto::INTS)
        .Attr("weights",
              "list of floats. This attribute stores the weight of each n-gram in pool. The i-th "
              "element in weights "
              "is the weight of the i-th n-gram in pool. Its length equals to the size of "
              "ngram_indexes. "
              "By default, weights is an all-one tensor.This attribute is used when mode is "
              "\"IDF\" or \"TFIDF\" "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axes",
              ...
              AttributeProto::INTS, mvn_default_axes)
        .TypeConstraint("T",
                        {"tensor(float16)", "tensor(float)", "tensor(double)", "tensor(bfloat16)"},
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "block_shape",
               ...
               "tensor(int64)", OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
        .Output(0, "output", "Output tensor produced by rearranging blocks into an image.", "T",
                OpSchema::Single, true, 1, OpSchema::Differentiable)
...
static bool BuildContextDependentFunctionBodyLayerNormalization(const FunctionBodyBuildContext &ctx,
  ...
  int64_t U =
      (type_attr != nullptr) ? type_attr->i() : static_cast<int64_t>(TensorProto_DataType_FLOAT);
  if ((U != TensorProto_DataType_FLOAT) && (U != TensorProto_DataType_BFLOAT16))
    return false; // Error
  ...
  auto mktensor = [](int64_t val) -> TensorProto {
    auto tp = ToTensor(std::vector<int64_t>{val});
    tp.add_dims(1);
    return tp;
  };
  ...
  builder.Const("FloatEpsilon", ToTensor<float>(epsilon))
      ...
      .Add("SuffixShape = ConstantOfShape (NumReducedAxes)", "value",
           mktensor(1)) // [1, ..., 1] for reduced axes
      .Add("ReducedShape = Concat <axis = 0> (PrefixShape, SuffixShape)") // [d[0], ..., d[axis-1],
                                                                          // 1, ..., 1]
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("stash_type",
              ...
              AttributeProto::INT, static_cast<int64_t>(TensorProto_DataType_FLOAT))
        ...
        .Output(1, "Mean", "Saved mean used during training to speed up gradient computation", "U",
                OpSchema::Optional)
        .Output(2, "InvStdDev",
        ...
        .TypeConstraint("U", {"tensor(float)", "tensor(bfloat16)"},
                        "Type of Mean and InvStdDev tensors.")
        .SetNodeDeterminism(OpSchema::NodeDeterminism::Deterministic)
        .SetContextDependentFunctionBodyBuilder(
            BuildContextDependentFunctionBodyLayerNormalizationVer17, 17)
        .SetContextDependentFunctionBodyBuilder(
            BuildContextDependentFunctionBodyLayerNormalizationVer18, 18)
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateShapeAndTypeFromFirstInput(ctx);
          auto stash_type = static_cast<int64_t>(TensorProto_DataType_FLOAT);
          ...
          if (axis < 0) {
            fail_shape_inference("Unexpected axis value (", axis, ") rank of first input is ",
                                 input_ndim, " in ", ctx.getDisplayName(), ".");
          }
          if (ctx.getNumOutputs() > 1) {
...
static void GroupNormalizationShapeInference(InferenceContext &ctx) {
  ...
  if (num_channels.has_dim_value() && num_channels.dim_value() % num_groups != 0) {
    fail_shape_inference("The number of channels must be divisible by num_groups. channels=",
                         num_channels.dim_value(), " num_groups=", num_groups, ".");
  }
}
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("num_groups",
              ...
              "`C`.",
              AttributeProto::INT, true)
        .Attr("stash_type", "The floating-point precision used in stage one of the computation.",
              AttributeProto::INT, static_cast<int64_t>(TensorProto_DataType_FLOAT))
        .Input(0, "X",
               ...
               "in the form of `(N x C x D1 x D2 ... Dn)`.",
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(1, "scale", "Scale tensor of shape `(C)`.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(2, "bias", "Bias tensor of shape `(C)`.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Output(0, "Y", "The output tensor of the same shape as `X`.", "T", OpSchema::Single, true,
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          if ((stash_type != TensorProto_DataType_FLOAT) &&
              (stash_type != TensorProto_DataType_BFLOAT16) &&
              (stash_type != TensorProto_DataType_FLOAT16) &&
    ...
    OpSchema()
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          int64_t U = (type_attr != nullptr) ? type_attr->i()
                                             : static_cast<int64_t>(TensorProto_DataType_FLOAT);
          if ((U != TensorProto_DataType_FLOAT) && (U != TensorProto_DataType_BFLOAT16) &&
              (U != TensorProto_DataType_FLOAT16) && (U != TensorProto_DataType_DOUBLE))
            return false; // Error
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("interleaved", "Rotate using interleaved pattern. Default value is 0 (False).",
              AttributeProto::INT, static_cast<int64_t>(0))
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("softmax_precision",
              ...
              "If softmax precision is not provided, the same precision as the input of softmax (Q "
              "and K) is used.",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("softcap", "Softcap value for attention weights. Default value is 0.",
        ...
        .Attr("qk_matmul_output_mode",
              ...
              "If set to `1`, qk_matmul_output is the output after the softcap operation (before "
              "mask addition). "
        ...
        .TypeAndShapeInferenceFunction(AttentionPropagateElemTypeFromInputToOutput)
        .SetNodeDeterminism(OpSchema::NodeDeterminism::Deterministic)
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
                                                   const OpSchema &schema,
                                                   FunctionProto &functionProto) {
          // ScaledDotProductAttention <scale, is_causal, q_num_heads, kv_numheads> (Q, K, V,
          // attn_mask, past_key, past_value) => (Y, present_key?, present_value?)
          int64_t int_type = TensorProto_DataType_INT64;
          int64_t float_type = TensorProto_DataType_FLOAT;
          ...
          int64_t softmax_precision =
              (softmax_precision_attr != nullptr) ? softmax_precision_attr->i() : T1;
          if ((softmax_precision != TensorProto_DataType_FLOAT) &&
              (softmax_precision != TensorProto_DataType_BFLOAT16) &&
              (softmax_precision != TensorProto_DataType_FLOAT16) &&
              (softmax_precision != TensorProto_DataType_DOUBLE))
            return false; // Error
          ...
          if (is_3d_input) {
            ...
            builder
                .Add("QIntermediateShape = Concat <axis = 0> (BatchSize, QSeqLen, QNumHeadsAttr, "
                     "NegOne)")
                .Add("KVIntermediateShape = Concat <axis = 0> (BatchSize, KVSeqLen, "
                     "KVNumHeadsAttr, NegOne)")
            ...
            // For 4D inputs: Already in desired shape [batch_size, num_heads, seq_length,
            // head_size]
          ...
          builder
              ...
              .Add(scale_attr != nullptr ? "ScaleFactor = Identity(ScaleF)"
          ...
          builder
              .Add("KExpandShape = Concat <axis = 0> (BatchSize, KVNumHeads, InterleaveDim, "
                   "NewKVSeqLen, QKHeadSize)")
              .Add("KExpanded = Expand(KUnsqueezed, KExpandShape)");
          builder
              .Add("VExpandShape = Concat <axis = 0> (BatchSize, KVNumHeads, InterleaveDim, "
                   "NewKVSeqLen, VHeadSize)")
              .Add("VExpanded = Expand(VUnsqueezed, VExpandShape)");

          // Reshape to [B, Hq, T, Dk] where Hq = Hk * repeats
          builder
              .Add("KAttentionShape = Concat <axis = 0> (BatchSize, QNumHeads, NewKVSeqLen, "
                   "QKHeadSize)")
              .Add("VAttentionShape = Concat <axis = 0> (BatchSize, QNumHeads, NewKVSeqLen, "
                   "VHeadSize)")
          ...
          int64_t qk_matmul_output_mode =
              (qk_matmul_output_mode_attr != nullptr) ? qk_matmul_output_mode_attr->i() : 0;
          if (ctx.hasOutput(3)) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(
            ...
            "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(2, "bias", "Optional per-channel bias with shape (channels).", "T",
               OpSchema::Optional, true, 1, OpSchema::Differentiable)
        .Input(3, "past_state",
               "Carry state from previous step with shape (batch_size, channels, k - 1). "
               "If not provided, padding is zero.",
               "T", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(0, "output", "Convolution output with same shape as input.", "T", OpSchema::Single,
                true, 1, OpSchema::Differentiable)
        .Output(1, "present_state",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (activation_attr != nullptr && activation_attr->has_s()) {
            const std::string activation =
                std::string(std::string(activation_attr->s().data(), activation_attr->s().size()));
            if (activation != "none" && activation != "silu" && activation != "swish") {
              fail_shape_inference("CausalConvWithState: unsupported activation value '",
                                   activation,
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          if (input_type->tensor_type().shape().dim(1).has_dim_value()) {
            ...
            if (weight_type == nullptr || !weight_type->has_tensor_type() ||
                !weight_type->tensor_type().has_shape() ||
                weight_type->tensor_type().shape().dim_size() < 1 ||
          ...
          std::string activation = (activation_attr != nullptr && activation_attr->has_s())
                                       ? std::string(std::string(activation_attr->s().data(),
                                                                 activation_attr->s().size()))
                                       : "none";
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "key",
               ...
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(2, "value", "Value vectors with 3D packed shape (B, T, H_kv * d_v).", "T",
               OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(3, "past_state",
               "Recurrent state from previous step with shape (B, H_kv, d_k, d_v). "
               "Always 4D. If not provided, defaults to zeros.",
               "S", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Input(4, "decay",
        ...
        .Input(5, "beta",
               ...
               "T", OpSchema::Optional, true, 1, OpSchema::Differentiable)
        .Output(0, "output", "Attention output with 3D packed shape (B, T, H_q * d_v).", "T",
                OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Output(1, "present_state",
                "Updated recurrent state with shape (B, H_kv, d_k, d_v). Always 4D.", "S",
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          std::string update_rule = (update_rule_attr != nullptr && update_rule_attr->has_s())
                                        ? std::string(std::string(update_rule_attr->s().data(),
                                                                  update_rule_attr->s().size()))
                                        : "gated_delta";
        ...
        .TypeAndShapeInferenceFunction([](ONNX_LIGHT_NAMESPACE::InferenceContext &ctx) {
          ...
          auto *kv_num_heads_attr = ctx.getAttribute("kv_num_heads");
          int64_t q_num_heads =
              (q_num_heads_attr && q_num_heads_attr->has_i()) ? q_num_heads_attr->i() : 0;
          int64_t kv_num_heads =
              (kv_num_heads_attr && kv_num_heads_attr->has_i()) ? kv_num_heads_attr->i() : 0;
          ...
          if (q_num_heads % kv_num_heads != 0) {
            fail_type_inference("q_num_heads (", q_num_heads,
                                ") must be a multiple of kv_num_heads (", kv_num_heads, ")");
          }

          // Read update_rule attribute with default
          auto *update_rule_attr = ctx.getAttribute("update_rule");
          std::string update_rule = (update_rule_attr != nullptr)
                                        ? std::string(std::string(update_rule_attr->s().data(),
                                                                  update_rule_attr->s().size()))
                                        : "gated_delta";
          ...
          propagateElemTypeFromInputToOutput(ctx, 0, 0);
          if (has_past_state && ctx.getInputType(3) != nullptr &&
          ...
          if (q_num_heads > 0 && QPack.has_dim_value()) {
            if (QPack.dim_value() % q_num_heads != 0) {
              fail_shape_inference("Packed query last dim (", QPack.dim_value(),
          ...
          if (kv_num_heads > 0 && KPack.has_dim_value()) {
            if (KPack.dim_value() % kv_num_heads != 0) {
              fail_shape_inference("Packed key last dim (", KPack.dim_value(),
          ...
          if (kv_num_heads > 0 && VPack.has_dim_value()) {
            if (VPack.dim_value() % kv_num_heads != 0) {
              fail_shape_inference("Packed value last dim (", VPack.dim_value(),
                                   ") must be divisible by kv_num_heads (", kv_num_heads, ")");
            }
          ...
          if (has_decay && kv_num_heads > 0) {
            ...
            if (decay_type != nullptr && decay_type->has_tensor_type() &&
                decay_type->tensor_type().has_shape() &&
              ...
              if (Dk.has_dim_value()) {
                ...
                if (decay_last != kv_num_heads && decay_last != kv_dk) {
                  fail_shape_inference("decay last dim (", decay_last, ") must be kv_num_heads (",
                                       kv_num_heads, ") or kv_num_heads * d_k (", kv_dk, ")");
                }
              } else if (decay_last != kv_num_heads && decay_last % kv_num_heads != 0) {
                fail_shape_inference("decay last dim (", decay_last, ") must be kv_num_heads (",
                                     kv_num_heads, ") or a multiple of kv_num_heads");
              }
            }
          }
          if (has_beta && kv_num_heads > 0) {
            auto *beta_type = ctx.getInputType(5);
            if (beta_type != nullptr && beta_type->has_tensor_type() &&
                beta_type->tensor_type().has_shape() &&
              ...
              if (beta_last != 1 && beta_last != kv_num_heads) {
                fail_shape_inference("beta last dim (", beta_last, ") must be 1 or kv_num_heads (",
                                     kv_num_heads, ")");
              }
            }
          }
