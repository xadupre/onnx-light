...
constexpr const char *kName = "kernel::Atan";
...
Atan::Atan(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}
...
KernelTuningKey Atan::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Atan", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Atan::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Atan", parameters, tuning_, kTuningAbi);
}
...
void Atan::operator()(const Tensor &x, Tensor &output) const {
