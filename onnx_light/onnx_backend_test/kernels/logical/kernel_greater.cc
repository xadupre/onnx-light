...
Tensor GreaterAlloc(const char *in_dtype_name, int32_t in_dtype, const Tensor &x, const Tensor &y) {
  return detail::BinaryElementwiseAllocInOut<TIn, uint8_t>(
      kGreaterName, in_dtype_name, in_dtype, kBoolName, TensorProto::DataType::BOOL, x, y,
...
void GreaterInPlace(const char *in_dtype_name, int32_t in_dtype, const Tensor &x, const Tensor &y,
  ...
  detail::BinaryElementwiseInOut<TIn, uint8_t>(
      kGreaterName, in_dtype_name, in_dtype, kBoolName, TensorProto::DataType::BOOL, x, y, output,
      [](TIn a, TIn b) -> uint8_t { return a > b ? 1 : 0; });
}
} // namespace
...
Tensor Greater::operator()(const Tensor &x, const Tensor &y) const {
  ...
  case TensorProto::DataType::FLOAT:
    return GreaterAlloc<float>("FLOAT", TensorProto::DataType::FLOAT, x, y);
  case TensorProto::DataType::INT8:
    return GreaterAlloc<int8_t>("INT8", TensorProto::DataType::INT8, x, y);
  case TensorProto::DataType::INT16:
    return GreaterAlloc<int16_t>("INT16", TensorProto::DataType::INT16, x, y);
  case TensorProto::DataType::UINT8:
    return GreaterAlloc<uint8_t>("UINT8", TensorProto::DataType::UINT8, x, y);
  case TensorProto::DataType::UINT16:
    return GreaterAlloc<uint16_t>("UINT16", TensorProto::DataType::UINT16, x, y);
  case TensorProto::DataType::UINT32:
    return GreaterAlloc<uint32_t>("UINT32", TensorProto::DataType::UINT32, x, y);
  case TensorProto::DataType::UINT64:
    return GreaterAlloc<uint64_t>("UINT64", TensorProto::DataType::UINT64, x, y);
  default:
...
void Greater::operator()(const Tensor &x, const Tensor &y, Tensor &output) const {
  switch (x.data_type) {
  case TensorProto::DataType::FLOAT:
    return GreaterInPlace<float>("FLOAT", TensorProto::DataType::FLOAT, x, y, output);
  case TensorProto::DataType::INT8:
    return GreaterInPlace<int8_t>("INT8", TensorProto::DataType::INT8, x, y, output);
  case TensorProto::DataType::INT16:
    return GreaterInPlace<int16_t>("INT16", TensorProto::DataType::INT16, x, y, output);
  case TensorProto::DataType::UINT8:
    return GreaterInPlace<uint8_t>("UINT8", TensorProto::DataType::UINT8, x, y, output);
  case TensorProto::DataType::UINT16:
    return GreaterInPlace<uint16_t>("UINT16", TensorProto::DataType::UINT16, x, y, output);
  case TensorProto::DataType::UINT32:
    return GreaterInPlace<uint32_t>("UINT32", TensorProto::DataType::UINT32, x, y, output);
  case TensorProto::DataType::UINT64:
    return GreaterInPlace<uint64_t>("UINT64", TensorProto::DataType::UINT64, x, y, output);
  default:
    throw std::invalid_argument(std::string(kGreaterName) +
                                " only supports FLOAT, INT8, INT16, UINT8, UINT16, UINT32 "
                                "and UINT64 inputs.");
