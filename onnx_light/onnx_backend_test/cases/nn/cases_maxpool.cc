...
void RegisterMaxPoolCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    RegisterLazyBenchmarkCase(registry, std::move(node), "test_cc_maxpool_1d_default_benchmark",
                              {opset}, {in_count}, {out_count}, [maxpool_kernel]() -> IoData {
                                Tensor x = Tensor::FromFloat("", {1, 32, 16384},
                                                             Randn<float>({1, 32, 16384}, 2301));
