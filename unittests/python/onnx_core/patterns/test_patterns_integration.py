...
def _make_model(nodes, inputs, outputs, initializers=(), *, opset=18):
    ...
    graph = helper.make_graph(nodes, "integration", inputs, outputs, list(initializers))
    return helper.make_model(graph, opset_imports=[helper.make_opsetid("", opset)], ir_version=10)
...
class TestPatternsIntegration(ExtTestCase):
    ...
    def test_bug_2of3_minimal_reconstruction(self):
        model = _make_model(
            [
                helper.make_node("Expand", ["X", "x_expanded_shape"], ["x_expanded"]),
                helper.make_node("Reshape", ["x_expanded", "x_reshape_shape"], ["x_reshaped"]),
                helper.make_node("Expand", ["Y", "y_expanded_shape"], ["y_expanded"]),
                helper.make_node("Reshape", ["y_expanded", "y_reshape_shape"], ["y_reshaped"]),
                helper.make_node("MatMul", ["x_reshaped", "y_reshaped"], ["product"]),
                helper.make_node("Reshape", ["product", "output_shape"], ["Z"]),
            ],
            [
                helper.make_tensor_value_info("X", TensorProto.FLOAT, [2, 3, 4, 5]),
                helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 3, 5, 6]),
            ],
            [helper.make_tensor_value_info("Z", TensorProto.FLOAT, [2, 3, 4, 6])],
            [
    ...
    def test_layernorm_data_minimal_reconstruction_prepares_gelu(self):
        model = _make_model(
            [
                ...
                helper.make_node("Sqrt", ["variance_epsilon"], ["deviation"]),
                helper.make_node("Div", ["centered", "deviation"], ["normalized"]),
                helper.make_node(
                    "Dropout", ["normalized", "ratio", "training"], ["dropped", "mask"]
                ),
                helper.make_node("Cast", ["dropped"], ["gelu_input"], to=TensorProto.FLOAT),
                helper.make_node("Pow", ["gelu_input", "three"], ["cube"]),
                helper.make_node("Mul", ["cube", "cubic_scale"], ["scaled_cube"]),
                helper.make_node("Add", ["gelu_input", "scaled_cube"], ["sum"]),
                helper.make_node("Mul", ["sum", "sqrt_two_over_pi"], ["scaled_sum"]),
                helper.make_node("Tanh", ["scaled_sum"], ["tanh"]),
                helper.make_node("Add", ["tanh", "one"], ["tanh_plus_one"]),
                helper.make_node("Mul", ["gelu_input", "half"], ["input_half"]),
                helper.make_node("Mul", ["input_half", "tanh_plus_one"], ["Y"]),
            ],
    ...
    def _make_partial_rotary_model(self):
        nodes = [
            helper.make_node("Split", ["X", "outer_split"], ["head", "tail"], axis=-1),
            helper.make_node("Split", ["head"], ["left", "right"], axis=-1, num_outputs=2),
            helper.make_node("Neg", ["right"], ["negative_right"]),
            helper.make_node("Concat", ["negative_right", "left"], ["rotated"], axis=-1),
            helper.make_node("Mul", ["rotated", "sin_cache"], ["rotated_sin"]),
            helper.make_node("Mul", ["head", "cos_cache"], ["source_cos"]),
            helper.make_node("Add", ["rotated_sin", "source_cos"], ["embedded"]),
            helper.make_node("Concat", ["embedded", "tail"], ["Y"], axis=-1),
        ]
        return _make_model(
            nodes,
            [
                helper.make_tensor_value_info("X", TensorProto.FLOAT, [2, 2, 3, 10]),
                helper.make_tensor_value_info("sin_cache", TensorProto.FLOAT, [1, 1, 3, 4]),
                helper.make_tensor_value_info("cos_cache", TensorProto.FLOAT, [1, 1, 3, 4]),
            ],
            [helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 2, 3, 10])],
    ...
    def test_rotary_prepass_fuses_two_shared_cache_branches(self):
        nodes = [
            helper.make_node("Sin", ["angles"], ["sin_angles"]),
            helper.make_node("Cos", ["angles"], ["cos_angles"]),
            helper.make_node("Unsqueeze", ["sin_angles", "head_axis"], ["sin_cache"]),
            helper.make_node("Unsqueeze", ["cos_angles", "head_axis"], ["cos_cache"]),
        ]
        for prefix in ("query", "key"):
            nodes.extend(
                [
                    ...
                    helper.make_node("Neg", [f"{prefix}_right"], [f"{prefix}_negative_right"]),
                    helper.make_node(
                        "Concat",
                    ...
                    helper.make_node("Mul", [prefix, "cos_cache"], [f"{prefix}_cos"]),
                    helper.make_node(
        ...
        model = _make_model(
            nodes,
            [
                helper.make_tensor_value_info("angles", TensorProto.FLOAT, [2, 3, 4]),
                helper.make_tensor_value_info("query", TensorProto.FLOAT, [2, 2, 3, 4]),
                helper.make_tensor_value_info("key", TensorProto.FLOAT, [2, 1, 3, 4]),
            ],
            [
                helper.make_tensor_value_info("query_embedded", TensorProto.FLOAT, [2, 2, 3, 4]),
                helper.make_tensor_value_info("key_embedded", TensorProto.FLOAT, [2, 1, 3, 4]),
            ],
    ...
    def test_swap_expand_reshape_prepares_rotary_prepass(self):
        model = _make_model(
            [
                helper.make_node("Expand", ["weights", "expanded_shape"], ["expanded"]),
                helper.make_node("Reshape", ["expanded", "reshape_shape"], ["reshaped"]),
                helper.make_node("Unsqueeze", ["reshaped", "cache_axis"], ["cos_cache"]),
                helper.make_node("Split", ["X"], ["left", "right"], axis=-1, num_outputs=2),
                helper.make_node("Neg", ["right"], ["negative_right"]),
                helper.make_node("Concat", ["negative_right", "left"], ["rotated"], axis=-1),
                helper.make_node("Mul", ["rotated", "sin_cache"], ["rotated_sin"]),
                helper.make_node("Mul", ["X", "cos_cache"], ["source_cos"]),
                helper.make_node("Add", ["rotated_sin", "source_cos"], ["Y"]),
            ],
            [
                helper.make_tensor_value_info("X", TensorProto.FLOAT, [2, 1, 1, 2]),
                helper.make_tensor_value_info("sin_cache", TensorProto.FLOAT, [2, 1, 1, 2]),
                helper.make_tensor_value_info("weights", TensorProto.FLOAT, [1, 2, 1]),
            ],
            [helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 1, 1, 2])],
            [
    ...
    def _make_attention_model_4d(self):
        return _make_model(
            [
                helper.make_node("Transpose", ["query"], ["transposed_query"], perm=[0, 2, 1, 3]),
                helper.make_node("Transpose", ["keys"], ["transposed_keys"], perm=[0, 2, 1, 3]),
                helper.make_node(
                    "Transpose", ["values"], ["transposed_values"], perm=[0, 2, 1, 3]
                ),
                helper.make_node(
                ...
                helper.make_node("Mul", ["transposed_query", "scale"], ["scaled_query"]),
                helper.make_node("Mul", ["concatenated_keys", "scale"], ["scaled_keys"]),
                helper.make_node(
                    "Transpose", ["scaled_keys"], ["transposed_scaled_keys"], perm=[0, 1, 3, 2]
                ),
                helper.make_node(
                    "MatMul", ["scaled_query", "transposed_scaled_keys"], ["scores"]
                ),
                helper.make_node("Where", ["mask", "zero", "negative_infinity"], ["bias"]),
                helper.make_node("Add", ["scores", "bias"], ["masked_scores"]),
                helper.make_node("Softmax", ["masked_scores"], ["probabilities"], axis=-1),
                helper.make_node("IsNaN", ["probabilities"], ["nan_mask"]),
                helper.make_node(
                ...
                helper.make_node("Transpose", ["context"], ["Y"], perm=[0, 2, 1, 3]),
            ],
            [
                helper.make_tensor_value_info("query", TensorProto.FLOAT, [2, 3, 2, 4]),
                helper.make_tensor_value_info("keys", TensorProto.FLOAT, [2, 3, 2, 4]),
                helper.make_tensor_value_info("values", TensorProto.FLOAT, [2, 3, 2, 4]),
                helper.make_tensor_value_info("past_keys", TensorProto.FLOAT, [2, 2, 3, 4]),
                helper.make_tensor_value_info("past_values", TensorProto.FLOAT, [2, 2, 3, 4]),
                helper.make_tensor_value_info("mask", TensorProto.BOOL, [2, 1, 3, 6]),
            ],
            [helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 3, 2, 4])],
            [
    ...
    def _make_attention_model_3d(self):
        ...
        return _make_model(
            [
                helper.make_node("MatMul", ["hidden", "query_weight"], ["query_3d"]),
                helper.make_node("Mul", ["query_3d", "scale"], ["scaled_query_3d"]),
                helper.make_node("Reshape", ["scaled_query_3d", "shape_4d"], ["query_4d"]),
                helper.make_node("Transpose", ["query_4d"], ["query"], perm=[0, 2, 1, 3]),
                helper.make_node("MatMul", ["hidden", "key_weight"], ["key_3d"]),
                helper.make_node("Mul", ["key_3d", "scale"], ["scaled_key_3d"]),
                helper.make_node("Reshape", ["scaled_key_3d", "shape_4d"], ["key_4d"]),
                helper.make_node("Transpose", ["key_4d"], ["key"], perm=[0, 2, 3, 1]),
                helper.make_node("MatMul", ["hidden", "value_weight"], ["value_3d"]),
                helper.make_node("Reshape", ["value_3d", "shape_4d"], ["value_4d"]),
                helper.make_node("Transpose", ["value_4d"], ["value"], perm=[0, 2, 1, 3]),
                helper.make_node("MatMul", ["query", "key"], ["scores"]),
                helper.make_node(
                    "Where", ["mask", "scores", "negative_infinity"], ["masked_scores"]
                ),
                helper.make_node("Softmax", ["masked_scores"], ["probabilities"], axis=-1),
                helper.make_node("IsNaN", ["probabilities"], ["nan_mask"]),
                helper.make_node(
                ...
                helper.make_node("MatMul", ["filtered_probabilities", "value"], ["context"]),
                helper.make_node(
                    "Transpose", ["context"], ["transposed_context"], perm=[0, 2, 1, 3]
                ),
                helper.make_node("Reshape", ["transposed_context", "shape_3d"], ["Y"]),
            ],
            [
                helper.make_tensor_value_info("hidden", TensorProto.FLOAT, [2, 3, hidden]),
                helper.make_tensor_value_info("mask", TensorProto.BOOL, [1, 1, 3, 3]),
            ],
