...
void RequireInputs(const NodeProto &node, int expected) {
  EXT_ENFORCE_INVALID(node.input_size() >= expected, "ComputeShapeNode: op '",
                      node.op_type(), "' expects at least ", std::to_string(expected),
  ...
  static const std::unordered_map<std::string, ComputeShapeFn> table = {
      ...
      {"ai.onnx:Attention",
       [](ShapesContext &ctx, const NodeProto &node) {
         ...
         const std::string past_k_name =
             node.input_size() > 4 ? std::string(node.input(4)) : std::string();
         const std::string past_v_name =
             node.input_size() > 5 ? std::string(node.input(5)) : std::string();
      ...
      {"ai.onnx:Dropout",
       [](ShapesContext &ctx, const NodeProto &node) {
        ...
        const std::string training_mode_name =
            node.input_size() >= 3 ? node.input(2) : std::string();
      ...
      {"ai.onnx:LinearAttention",
       [](ShapesContext &ctx, const NodeProto &node) {
         ...
         const std::string past_state_name =
             node.input_size() > 3 ? std::string(node.input(3)) : std::string();
