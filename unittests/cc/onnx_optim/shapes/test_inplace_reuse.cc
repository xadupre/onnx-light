...
TEST(OnnxOptimInPlaceReuse, WriteInPlaceReuseToMetadata) {
  ...
  *graph.add_node() = MakeNode("Abs", {"B"}, {"Y"});
  ...
  EXPECT_EQ(graph.node()[0].metadata_props()[0].key(),
            std::string(onnx_optim::annotations::kNotUsedAfterMetadataKey));
  EXPECT_EQ(graph.node()[0].metadata_props()[0].value(), std::string("X"));
  ASSERT_EQ(graph.node()[1].metadata_props().size(), 2);
  EXPECT_EQ(graph.node()[1].metadata_props()[0].key(),
            std::string(onnx_optim::annotations::kInPlaceReuseMetadataKey));
  EXPECT_EQ(graph.node()[1].metadata_props()[0].value(), std::string("0:0:equal"));
  EXPECT_EQ(graph.node()[1].metadata_props()[1].key(),
            std::string(onnx_optim::annotations::kReleaseAfterMetadataKey));
  EXPECT_EQ(graph.node()[1].metadata_props()[1].value(), std::string("A"));
  ASSERT_EQ(graph.node()[2].metadata_props().size(), 2);
  EXPECT_EQ(graph.node()[2].metadata_props()[0].value(), std::string("0:0:equal"));
  EXPECT_EQ(graph.node()[2].metadata_props()[1].key(),
            std::string(onnx_optim::annotations::kReleaseAfterMetadataKey));
  EXPECT_EQ(graph.node()[2].metadata_props()[1].value(), std::string("B"));
}
...
TEST(OnnxOptimInPlaceReuse, WriteInPlaceReuseToMetadataGreaterAndUpdate) {
  ...
  *graph.add_node() = MakeCastNode("A", "Y", TensorProto::DataType::INT32);
  ...
  EXPECT_EQ(graph.node()[0].metadata_props()[0].value(), std::string("X"));
  ASSERT_EQ(graph.node()[1].metadata_props().size(), 2);
  EXPECT_EQ(graph.node()[1].metadata_props()[0].value(), std::string("0:0:greater"));
  EXPECT_EQ(graph.node()[1].metadata_props()[1].key(),
            std::string(onnx_optim::annotations::kReleaseAfterMetadataKey));
  EXPECT_EQ(graph.node()[1].metadata_props()[1].value(), std::string("A"));
}
  ...
  *graph.add_node() = MakeNode("Abs", {"A"}, {"Y"});
  ...
  for (int n = 0; n < graph.node().size(); ++n) {
    for (int i = 0; i < graph.node()[n].metadata_props().size(); ++i) {
      EXPECT_NE(graph.node()[n].metadata_props()[i].key(),
                std::string(onnx_optim::annotations::kReleaseAfterShapeTagMetadataKey));
    }
  }
}
...
TEST(OnnxOptimInPlaceReuse, WriteInPlaceReuseToMetadataWithShapeTags) {
  ...
  bool found_shape_tag = false;
  for (int i = 0; i < graph.node()[1].metadata_props().size(); ++i) {
    if (graph.node()[1].metadata_props()[i].key() ==
        std::string(onnx_optim::annotations::kReleaseAfterShapeTagMetadataKey)) {
      EXPECT_EQ(graph.node()[1].metadata_props()[i].value(), std::string("S"));
      found_shape_tag = true;
    }
  }
  EXPECT_TRUE(found_shape_tag);
