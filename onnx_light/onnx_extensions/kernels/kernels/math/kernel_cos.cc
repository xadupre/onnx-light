...
constexpr const char *kName = "kernel::Cos";
...
Cos::Cos(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}

void Cos::RegisterTuningSchemas() {
  tuning::RegisterParallelTuningSchemas("Cos", kSupportedElementTypes, kPortableParallelMinimum,
                                        kTuningAbi);
}

KernelTuningKey Cos::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Cos", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Cos::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Cos", parameters, tuning_, kTuningAbi);
}
