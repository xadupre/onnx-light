...
void RegisterReduceLogSumOpCases(std::vector<TestCase> &registry, const std::string &op_type,
  ...
  EmitReduceLogSumDefaultAxesCase<Kernel>(registry, op_type,
                                          "test_cc_" + name_prefix + "_default_axes_keepdims",
                                          shape, values, /*keepdims=*/true);
  EmitReduceLogSumCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_keepdims", shape,
                               values, {1}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_do_not_keepdims",
                               shape, values, {1}, /*keepdims=*/false,
                               /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(
      registry, op_type, "test_cc_" + name_prefix + "_negative_axes_keepdims", shape, values, {-2},
      /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(
      registry, op_type, "test_cc_" + name_prefix + "_empty_axes_input_noop", shape, values, {},
      /*keepdims=*/true, /*noop_with_empty_axes=*/true);
  // Reducing over an axis of size 0: the empty-set identity is -inf
  // (``log(sum(empty)) == log(0)``).
  EmitReduceLogSumCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_empty_set",
                               /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
  // Non-reduced axis of size 0: the result preserves the empty dimension.
  EmitReduceLogSumCase<Kernel>(registry, op_type,
...
// ---------------------------------------------------------------------------
template <typename Kernel> void RegisterReduceLogSumExpOnnxCases(std::vector<TestCase> &registry) {
  const std::vector<int64_t> shape = {3, 2, 2};
  ...
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSumExp",
                               "test_reduce_log_sum_exp_keepdims_example", shape, example_values,
                               {1},
                               /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSumExp",
                               ...
                               /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(
      registry, "ReduceLogSumExp", "test_reduce_log_sum_exp_do_not_keepdims_example", shape,
      example_values, {1}, /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSumExp",
                               "test_reduce_log_sum_exp_do_not_keepdims_random", shape,
                               random_values, {1},
                               /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumDefaultAxesCase<Kernel>(registry, "ReduceLogSumExp",
                                          "test_reduce_log_sum_exp_default_axes_keepdims_example",
                                          shape, example_values, /*keepdims=*/true);
  EmitReduceLogSumDefaultAxesCase<Kernel>(registry, "ReduceLogSumExp",
                                          "test_reduce_log_sum_exp_default_axes_keepdims_random",
                                          shape, random_values, /*keepdims=*/true);
  EmitReduceLogSumCase<Kernel>(
      registry, "ReduceLogSumExp", "test_reduce_log_sum_exp_negative_axes_keepdims_example", shape,
      example_values, {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(
      registry, "ReduceLogSumExp", "test_reduce_log_sum_exp_negative_axes_keepdims_random", shape,
      random_values, {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSumExp", "test_reduce_log_sum_exp_empty_set",
                               /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
}
...
// ---------------------------------------------------------------------------
template <typename Kernel> void RegisterReduceLogSumOnnxCases(std::vector<TestCase> &registry) {
  ...
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSum", "test_reduce_log_sum_asc_axes",
                               shape_3_4_5, data_3_4_5, {0, 1}, /*keepdims=*/false,
                               /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSum", "test_reduce_log_sum_desc_axes",
                               shape_3_4_5, data_3_4_5, {2, 1}, /*keepdims=*/false,
                               /*noop_with_empty_axes=*/false);
  // ``_default``: empty axes, keepdims = 1 (the default).
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSum", "test_reduce_log_sum_default", shape_3_4_5,
                               data_3_4_5, {}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSum", "test_reduce_log_sum_negative_axes",
                               shape_3_4_5, data_3_4_5, {-2}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
  EmitReduceLogSumCase<Kernel>(registry, "ReduceLogSum", "test_reduce_log_sum_empty_set",
                               /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
}
...
void RegisterReduceLogSumCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducelogsum_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             ...
             const onnx_kernels::kernel::ReduceLogSum kernel{kernel_ctx};
  ...
  RegisterReduceLogSumOpCases<onnx_kernels::kernel::ReduceLogSum>(registry, "ReduceLogSum",
                                                                  "reducelogsum");
  RegisterReduceLogSumOnnxCases<onnx_kernels::kernel::ReduceLogSum>(registry);
...
void RegisterReduceLogSumExpCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducelogsumexp_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             const KernelContext kernel_ctx{DefaultOpset(18)};
             const onnx_kernels::kernel::ReduceLogSumExp kernel{kernel_ctx};
  ...
  RegisterReduceLogSumOpCases<onnx_kernels::kernel::ReduceLogSumExp>(registry, "ReduceLogSumExp",
                                                                     "reducelogsumexp");
  RegisterReduceLogSumExpOnnxCases<onnx_kernels::kernel::ReduceLogSumExp>(registry);
