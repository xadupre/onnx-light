...
GraphProto ExtractGraph(const GraphProto &graph, const std::vector<std::string> &input_names,
  ...
  const std::string graph_name =
      "Extracted from {" + (graph.has_name() ? std::string(graph.name()) : std::string()) + "}";
...
GraphProto MergeGraphs(const GraphProto &g1_in, const GraphProto &g2_in,
  ...
  if (!name.empty()) {
    ...
    const std::string n1 = g1.has_name() ? std::string(g1.name()) : std::string();
    const std::string n2 = g2.has_name() ? std::string(g2.name()) : std::string();
    g.set_name(n1 + "_" + n2);
  }
  ...
  if (!doc_string.empty()) {
    ...
    const std::string n1 = g1.has_name() ? std::string(g1.name()) : std::string();
    const std::string n2 = g2.has_name() ? std::string(g2.name()) : std::string();
    const std::string ds1 = g1.has_doc_string() ? std::string(g1.doc_string()) : std::string();
    const std::string ds2 = g2.has_doc_string() ? std::string(g2.doc_string()) : std::string();
...
GraphProto ExpandOutDimGraph(const GraphProto &graph, int64_t dim_idx, bool /*inplace*/) {
  ...
  const std::string expand_dim_k =
      (g.has_name() ? std::string(g.name()) : std::string()) + "_expand_out_dim_idx";
