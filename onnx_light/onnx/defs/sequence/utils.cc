...
std::function<void(OpSchema &)> SplitToSequenceOpGenerator(std::vector<std::string> input_types,
  ...
  return [input_types = std::move(input_types),
          output_types = std::move(output_types)](OpSchema &schema) {
    schema.Input(0, "input", "The tensor to split", "T")
        ...
        .TypeConstraint("I", {"tensor(int32)", "tensor(int64)"},
          ...
          if (num_inputs == 1) {
            ...
            splitSize = [&]() -> int64_t {
              ...
              if (splitShape.dim_size() == 0) {
                ...
                // split is 1-D tensor
                int64_t splitSizesSum =
                    std::accumulate(splitSizes.begin(), splitSizes.end(), static_cast<int64_t>(0));
                if (splitDimValue != splitSizesSum) {
                  fail_shape_inference("Sum of split values not equal to 'input' dim size on "
                                       "'axis'. 'axis' dim size=",
                                       splitDimValue, " sum of split values=", splitSizesSum);
                }
