...
public:
  explicit BroadcastBackwardCompatibility(const std::string &op_name, const OpSetID &initial,
                                          const OpSetID &target)
      : Adapter(op_name, initial, target) {}

  void adapt_broadcast_backward_compatibility(const std::shared_ptr<Graph> & /*unused*/,
                                              Node *node) const {
