...
void RegisterTinyLlmInlinedShapeInferenceCases(std::vector<TestCase> &registry) {
  ...
  // The 52 nodes are indexed 0-51 in AddNode insertion order.
  {
    ...
    //   [b,nh,s,hs]→[b,s,nh,hs]: same total byte size → kGreater.
    node_meta(28, ann::kNodeTagMetadataKey, "weight");
    node_meta(28, ann::kInPlaceReuseMetadataKey, "0:0:greater");
    node_meta(28, ann::kReleaseAfterMetadataKey, "context");
    // node[29] Reshape(context_t, merge_shape) → attn_out
    //   [b,s,nh,hs]→[b,s,h]: same total byte size → kGreater.
    node_meta(29, ann::kNodeTagMetadataKey, "weight");
    node_meta(29, ann::kInPlaceReuseMetadataKey, "0:0:greater");
