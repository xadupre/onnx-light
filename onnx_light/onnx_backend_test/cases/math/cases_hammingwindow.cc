...
// ---------------------------------------------------------------------------
// HammingWindow — generates a Hamming window of length ``size`` (since opset
...
void RegisterHammingWindowCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    RegisterLazyBenchmarkCase(registry, std::move(node), "test_cc_hammingwindow_benchmark", {opset},
                              {1}, {size}, [hamming_kernel, size]() -> IoData {
  ...
  // Default periodic variant.
  {
    ...
    Expect(node, {x}, {y}, "test_cc_hammingwindow", {opset}, "backend-test", registry);
  }
  ...
  // Symmetric variant (periodic = 0).
  {
    ...
    Expect(node, {x}, {y}, "test_cc_hammingwindow_symmetric", {opset}, "backend-test", registry);
  }
}
