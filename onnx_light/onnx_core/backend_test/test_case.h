...
// The blanket ``using namespace`` below is intentional and required: case
// files open ``namespace onnx_backend_test { ... }`` blocks and use names
// such as ``Tensor`` or ``TypeSpec`` without qualification.  Those names live
// transitively in ``core::runtime`` / ``core::backend_test`` and are only
// reachable via the unqualified-lookup chain that a namespace-scope
// ``using namespace`` establishes.
//
// The explicit using-declarations that follow are additionally needed for
// qualified access (``onnx_backend_test::TestCase`` etc.) from callers that do
// not open the namespace, because a ``using namespace`` directive does not
// affect qualified name lookup.
