...
struct ExtendSupportedTypes final : public Adapter {
  ...
  Node *create_cast_op(const std::shared_ptr<Graph> &graph, ArrayRef<Value *> inputs,
                       const int to_type, const std::vector<Dimension> &output_shape,
  ...
  void adapt_type_extension(const std::shared_ptr<Graph> &graph, Node *node) const {
    ...
    ONNX_ASSERTM(unsupported_version9_types.find(output_type) == unsupported_version9_types.end(),
