...
// ── test_add_concat_reshape_computed_shapes ─────────────────────────────────
//
// Translation of ``TestShapeBuilder.test_add_concat_reshape_computed_shapes``.
//
// Graph:
//   added       = Add(X, Y)
//   concat_out  = Concat(added, X, axis=2)
//   Z           = Reshape(concat_out, reshape_shape)   # reshape_shape=[0,0,-1]
//
// Inputs:
//   X, Y: float[batch, seq, d_model]
// Output:
//   Z: float[?, ?, ?]
//
// The Python ``BasicShapeBuilder`` tracks symbolic expressions so that:
//   added      -> (batch, seq, d_model)
//   concat_out -> (batch, seq, "2*d_model")
//   Z          -> (batch, seq, "2*d_model")
//
// In C++ the concat-axis dim is also recorded as the symbolic expression
// ``"2*d_model"`` (see test_compute_shape_tensor's
// ``RepeatedSymbolicAxisDimProducesScaledExpression``).  The Reshape with
// ``[0, 0, -1]`` keeps the leading dims as ``batch``/``seq`` and infers the
// last dim from the total element count; the resulting dim is symbolic.
TEST(OnnxOptimShapeBuilder, AddConcatReshapeComputesSymbolicShapes) {
  ModelProto model;
  model.set_ir_version(10);
  OperatorSetIdProto *osi = model.add_opset_import();
  osi->set_domain("");
  ...
  GraphProto *graph = model.add_graph();
  graph->set_name("add_concat_reshape");

  AddFloatInput(*graph, "X", {-1, -1, -1}, {"batch", "seq", "d_model"});
  AddFloatInput(*graph, "Y", {-1, -1, -1}, {"batch", "seq", "d_model"});
  AddFloatOutput(*graph, "Z", {-1, -1, -1}, {});

  AddInt64Initializer(*graph, "reshape_shape", {0, 0, -1});

  *graph->add_node() = MakeNode("Add", {"X", "Y"}, {"added"});
  {
    NodeProto concat_node = MakeNode("Concat", {"added", "X"}, {"concat_out"});
  ...
  *graph->add_node() = MakeNode("Reshape", {"concat_out", "reshape_shape"}, {"Z"});
  ...
  // added = Add(X, Y) has the same symbolic shape as X / Y.
  ASSERT_TRUE(ctx.Has("added"));
  ASSERT_EQ(ctx.Get("added").Shape().Rank(), 3u);
  EXPECT_EQ(ctx.Get("added").Dtype(), onnx_optim::TensorType::kFloat);
  CheckSymbolicDim(ctx, "added", 0, "batch");
  CheckSymbolicDim(ctx, "added", 1, "seq");
  CheckSymbolicDim(ctx, "added", 2, "d_model");

  // concat_out = Concat(added, X, axis=2): non-axis dims are merged from
  // both inputs ("batch", "seq"); the axis dim becomes "2*d_model" because
  // both inputs share the same symbolic "d_model".
  ASSERT_TRUE(ctx.Has("concat_out"));
  ASSERT_EQ(ctx.Get("concat_out").Shape().Rank(), 3u);
  EXPECT_EQ(ctx.Get("concat_out").Dtype(), onnx_optim::TensorType::kFloat);
  CheckSymbolicDim(ctx, "concat_out", 0, "batch");
  CheckSymbolicDim(ctx, "concat_out", 1, "seq");
  CheckSymbolicDim(ctx, "concat_out", 2, "2*d_model");

  // Z = Reshape(concat_out, [0, 0, -1]) keeps dims 0 and 1 from concat_out
  // and infers dim 2; the resulting tensor has rank 3 and float dtype.
  ASSERT_TRUE(ctx.Has("Z"));
  ASSERT_EQ(ctx.Get("Z").Shape().Rank(), 3u);
  EXPECT_EQ(ctx.Get("Z").Dtype(), onnx_optim::TensorType::kFloat);
  CheckSymbolicDim(ctx, "Z", 0, "batch");
