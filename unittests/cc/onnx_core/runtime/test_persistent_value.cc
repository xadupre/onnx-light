...
TEST(PersistentTensor, RetainsWithoutCopyAndDoesNotCertifyImportedStorage) {
  ...
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  auto reservation = retained.PrepareAppend().Reserve({3}, 0, 4, nullptr, &events);
  ASSERT_TRUE(reservation);
  ...
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 1u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_reuse_count), 0u);
}
...
TEST(PersistentTensor, ExplicitLeaseReservesInPlaceOnceAndPreservesCommittedPrefix) {
  ...
  EXPECT_THROW(lease.Reserve({3}, 0, 4, nullptr, &events), std::invalid_argument);
  auto reservation = moved.Reserve({3}, 0, 4, nullptr, &events);
  ASSERT_TRUE(reservation);
  WriteTail(*reservation, {3});
  ...
  EXPECT_THROW(moved.Reserve({3}, 0, 4, nullptr, &events), std::invalid_argument);
  EXPECT_FLOAT_EQ(candidate.value().AsFloat()[2], 3);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_reuse_count), 1u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 0u);
}
...
TEST(PersistentTensor, ViewsBlockInPlaceReservationsAndCannotRestoreCapacity) {
  ...
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  {
    ...
    auto reservation = cache.PrepareAppend().Reserve({3}, 0, 4, nullptr, &events);
    ASSERT_TRUE(reservation);
    WriteTail(*reservation, {3});
    const auto candidate = reservation->Commit(sizeof(float));
  ...
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 2u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_reuse_count), 1u);
}
...
TEST(PersistentTensor, ConcurrentConsumersRejectSecondUseOfOneLease) {
  ...
  const auto append = [&] {
    RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
    auto reservation = lease.Reserve({3}, 0, 4, nullptr, &events);
    EXT_ENFORCE(reservation.has_value(), "Expected a contiguous append reservation.");
    WriteTail(*reservation, {3});
    return std::pair{reservation->Commit(sizeof(float)), events.events()};
  };
  ...
  for (auto *attempt : {&first, &second}) {
    try {
      ...
      EXPECT_EQ(StorageTotal(statistics, &RuntimeEvent::storage_reuse_count), 1u);
      EXPECT_EQ(StorageTotal(statistics, &RuntimeEvent::storage_allocations), 0u);
...
TEST(PersistentTensor, FailedAppendDoesNotPublishAndRetryRewritesTail) {
  ...
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  {
    auto abandoned = cache.PrepareAppend().Reserve({3}, 0, 4, nullptr, &events);
  ...
  EXPECT_FLOAT_EQ(candidate.value().AsFloat()[2], 3);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 0u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_reuse_count), 2u);
}

TEST(PersistentTensor, GrowthRelocatesOnlyThePrefixAndWritesDirectlyIntoReservedCapacity) {
  auto cache = Cache();
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  auto reservation = cache.PrepareAppend().Reserve({5}, 0, 4, nullptr, &events);
  ASSERT_TRUE(reservation);
  ...
  auto next = candidate.PrepareAppend().Reserve({6}, 0, 4, nullptr, &events);
  ASSERT_TRUE(next);
  WriteTail(*next, {6});
  EXPECT_EQ(next->Commit(sizeof(float)).value().bytes(), candidate.value().bytes());
  const auto statistics = events.events();
  EXPECT_EQ(StorageTotal(statistics, &RuntimeEvent::storage_allocations), 1u);
  EXPECT_EQ(StorageTotal(statistics, &RuntimeEvent::storage_allocated_bytes), 8 * sizeof(float));
...
TEST(PersistentTensor, SupportsOtherElementTypesAxesAndEmptyTails) {
  ...
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  auto reservation = prefix.PrepareAppend().Reserve({1, 3}, 1, 4, nullptr, &events);
  ASSERT_TRUE(reservation);
  ...
  *reinterpret_cast<int64_t *>(reservation->writable_bytes().data()) = 30;
  ...
  auto empty = candidate.PrepareAppend().Reserve({1, 3}, 1, 4, nullptr, &events);
  ASSERT_TRUE(empty);
  EXPECT_TRUE(empty->writable_bytes().empty());
  EXPECT_EQ(empty->Commit(0).value().bytes(), candidate.value().bytes());
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_prefix_copied_bytes),
            2 * sizeof(int64_t));
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_append_copied_bytes), 0u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_reuse_count), 1u);
}
...
TEST(PersistentTensor, RejectsInvalidReservationsAndUnretainableAllocations) {
  auto cache = Cache();
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  EXPECT_THROW(cache.PrepareAppend().Reserve({1}, 0, 4, nullptr, &events), std::invalid_argument);
  EXPECT_THROW(cache.PrepareAppend().Reserve({3}, 1, 4, nullptr, &events), std::invalid_argument);
  EXPECT_THROW(cache.PrepareAppend().Reserve({1, 3}, 0, 4, nullptr, &events),
               std::invalid_argument);
  EXPECT_THROW(cache.PrepareAppend().Reserve({-1}, 0, 4, nullptr, &events), std::invalid_argument);
  EXPECT_THROW(
      cache.PrepareAppend().Reserve({std::numeric_limits<int64_t>::max()}, 0, 4, nullptr, &events),
      std::invalid_argument);
  EXPECT_THROW(
      cache.PrepareAppend().Reserve({5}, 0, std::numeric_limits<size_t>::max(), nullptr, &events),
      std::invalid_argument);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 0u);
  SimpleRawBufferAllocator arena(4);
  EXPECT_THROW(cache.PrepareAppend().Reserve({5}, 0, 4, &arena, &events), std::invalid_argument);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 1u);
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_prefix_copied_bytes), 0u);
  Tensor invalid = Tensor::FromFloat("", {2}, {1, 2});
  invalid.shape[0] = 3;
  PersistentTensor malformed(std::move(invalid));
  EXPECT_THROW(malformed.PrepareAppend().Reserve({4}, 0, 4, nullptr, &events),
               std::invalid_argument);
}
...
TEST(PersistentTensor, DeclinesLayoutsRequiringRepacking) {
  ...
  EXPECT_FALSE(slices.PrepareAppend().Reserve({2, 2}, 1, 4, nullptr, &events));
  PersistentTensor zero_width(Tensor::FromFloat("", {1, 0}, {}));
  EXPECT_FALSE(zero_width.PrepareAppend().Reserve({2, 0}, 0, 4, nullptr, &events));
  PersistentTensor packed(Tensor("", DataType::UINT4, {2}, std::vector<uint8_t>{0x21}));
  EXPECT_FALSE(packed.PrepareAppend().Reserve({4}, 0, 4, nullptr, &events));
  PersistentTensor packed_six(Tensor("", DataType::FLOAT6E2M3, {2}, std::vector<uint8_t>{0, 0}));
  EXPECT_FALSE(packed_six.PrepareAppend().Reserve({4}, 0, 4, nullptr, &events));
  EXPECT_EQ(StorageTotal(events.events(), &RuntimeEvent::storage_allocations), 0u);
}
...
TEST(PersistentTensor, AllocatorExhaustionPreservesTheCommittedPrefix) {
  ...
  RuntimeContext events(RuntimeContextOptions{.events_enabled = true});
  PersistentTensor empty(Tensor::FromFloat("", {0}, {}));
  auto reservation = empty.PrepareAppend().Reserve({2}, 0, 2, arena.get(), &events);
  ASSERT_TRUE(reservation);
  WriteTail(*reservation, {1, 2});
  auto cache = reservation->Commit(2 * sizeof(float));
  const auto *bytes = cache.value().bytes();
  EXPECT_THROW(cache.PrepareAppend().Reserve({3}, 0, 2, arena.get(), &events), std::bad_alloc);
