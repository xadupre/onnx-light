...
static std::function<void(OpSchema &)> GlobalLpPoolingOpSchemaGenerator_opset2(const char *op_type,
                                                                               const char *op) {
 ...
 equal to the spatial dimension of input tensor.)DOC";
    ...
    schema.TypeAndShapeInferenceFunction(
        [](InferenceContext &ctx) { globalPoolTypeShapeInference_opset2(ctx); });
  };
}

ONNX_OPERATOR_SET_SCHEMA(GlobalLpPool, 2,
                         OpSchema().FillUsing(GlobalLpPoolingOpSchemaGenerator_opset2("LpPool",
                                                                                      "lp pool")));
...
static void convPoolShapeInference_opset19(InferenceContext &ctx, bool use_dilation,
  ...
  for (int i = 0; i < kernel_shape_size; ++i) {
    ...
    int64_t output_size = checkedAdd(1, strided_kernel_positions);
    if (ceil_mode == 1 && checkedMultiply(checkedSubtract(output_size, 1), strides[i]) >=
                              checkedAdd(input_shape.dim(2 + i).dim_value(), pads[i])) {
...
ONNX_OPERATOR_SET_SCHEMA(InstanceNormalization, 6,
                         OpSchema()
...
static std::function<void(OpSchema &)> GlobalPoolingOpSchemaGenerator_opset1(const char *op_type,
                                                                             const char *op) {
  return [=](OpSchema &schema) {
    std::string doc;
    POPULATE_OP_DOC_STR(doc = R"DOC(
 ...
 equal to the spatial dimension of input tensor.)DOC";
    ...
    schema.TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                          "Constrain input and output types to float tensors.");
    schema.TypeAndShapeInferenceFunction(
        [](InferenceContext &ctx) { globalPoolTypeShapeInference_opset1(ctx); });
  };
}
ONNX_OPERATOR_SET_SCHEMA(GlobalAveragePool, 1,
                         OpSchema().FillUsing(GlobalPoolingOpSchemaGenerator_opset1("AveragePool",
                                                                                    "average")));
ONNX_OPERATOR_SET_SCHEMA(GlobalMaxPool, 1,
                         OpSchema().FillUsing(GlobalPoolingOpSchemaGenerator_opset1("MaxPool",
                                                                                    "max")));
...
ConvTransposeOpSchemaGenerator_opset11(const char *filter_desc) {
  return [=](OpSchema &schema) {
    std::string doc;
    POPULATE_OP_DOC_STR(doc = R"DOC(
...
output_shape can also be explicitly specified in which case pads values are auto generated using these equations:
  ...
  Else: pads[start_i] = total_padding[i] - (total_padding[i]/2); pads[end_i] = (total_padding[i]/2).
    ...
    schema.Attr("strides",
                ...
                "each spatial axis.",
                AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("auto_pad", conv_transpose_auto_pad_doc_opset19, AttributeProto::STRING,
                std::string("NOTSET"));
    ...
    schema.Attr("group", "number of groups input channels and output channels are divided into.",
                AttributeProto::INT, static_cast<int64_t>(1));
    schema.TypeAndShapeInferenceFunction(
        [](InferenceContext &ctx) { convTransposeShapeInference_opset11(ctx); });
  };
}

ONNX_OPERATOR_SET_SCHEMA(ConvTranspose, 11,
                         OpSchema().FillUsing(ConvTransposeOpSchemaGenerator_opset11("a filter")));
...
computes the output.)DOC";
    ...
    schema.Attr("strides",
                ...
                "each spatial axis.",
                AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("auto_pad", conv_auto_pad_doc_opset19, AttributeProto::STRING,
                std::string("NOTSET"));
    schema.Attr("pads", pads_doc_opset11, AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("group", "number of groups input channels and output channels are divided into.",
                AttributeProto::INT, static_cast<int64_t>(1));
...
static std::function<void(OpSchema &)> RoiPoolOpSchemaGenerator_opset1(const char *name) {
 ...
 (num_rois, channels, pooled_shape[0], pooled_shape[1]).)DOC";
    ...
    schema.TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                          "Constrain input and output types to float tensors.");
    schema.TypeAndShapeInferenceFunction(
        [](InferenceContext &ctx) { roiPoolTypeShapeInference_opset1(ctx); });
  };
}

ONNX_OPERATOR_SET_SCHEMA(MaxRoiPool, 1,
                         OpSchema().FillUsing(RoiPoolOpSchemaGenerator_opset1("max")));
...
ONNX_OPERATOR_SET_SCHEMA(LpPool, 18,
                         OpSchema().FillUsing(LpPoolOpSchemaGenerator_opset18("LpPool")));
...
static std::function<void(OpSchema &)>
PoolOpSchemaGenerator_opset19(const char *name, const char *opName,
                              const char *additionalDescription, bool use_dilation,
 ...
 {additionalDescription}
    ...
    schema.Output(0, "Y",
                  ...
                  "T", OpSchema::Single, true, 1, OpSchema::Differentiable);
    schema.TypeConstraint("T", GetSupportedDataTypesForPoolingOps_opset19(supports8bit),
                          supports8bit
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir12(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "dimension of the output.",
                "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir11(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "dimension of the output.",
                "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir10(),
                        "Constrain input and output to all tensor types up to IRv10.")
        .Attr("axis",
              "Indicate up to which input dimensions "
              "(exclusive) should be flattened to the outer dimension of the output. "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axes",
              ...
              AttributeProto::INTS, old_mvn_default_axes)
        .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
...
static void convPoolShapeInference_opset1_to_11(InferenceContext &ctx, bool use_dilation,
                                                bool require_kernel_shape, int input1Idx,
                                                int input2Idx) {
  // we need the first input shape for this inference.
  if (!hasInputShape(ctx, input1Idx)) {
...
static std::function<void(OpSchema &)>
PoolOpSchemaGenerator_opset1_to_8(const char *name, const char *opName,
...
static std::function<void(OpSchema &)>
PoolOpSchemaGenerator_opset10_to_11(const char *name, const char *opName,
                                    const char *additionalDescription, bool use_dilation,
                                    int opsetNum) {
  return [=](OpSchema &schema) {
    std::string doc;
 ...
 * pad_shape[i] is sum of pads along axis i
 ...
 {additionalDescription}
    ...
    schema.Attr("kernel_shape", "The size of the kernel along each axis.", AttributeProto::INTS);
    schema.Attr("strides",
                opsetNum == 11 ? "Stride along each spatial axis. If not present, the stride "
                                 "defaults to 1 along each spatial axis."
...
static std::function<void(OpSchema &)>
PoolOpSchemaGenerator_opset11(const char *name, const char *opName,
                              const char *additionalDescription, bool use_dilation,
                              bool supports8bit = false) {
  return [=](OpSchema &schema) {
    std::string doc;
...
or when ceil_mode is disabled:
 ...
 {additionalDescription}
    ...
    schema.Output(0, "Y",
                  ...
                  "the dimension is used",
                  "T", OpSchema::Single, true, 1, OpSchema::Differentiable);
    schema.TypeConstraint("T", GetSupportedDataTypesForPoolingOps_opset11(supports8bit),
                          supports8bit
                              ? "Constrain input and output types to float and 8 bit tensors."
                              : "Constrain input and output types to float tensors.");
    schema.TypeAndShapeInferenceFunction([use_dilation](InferenceContext &ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(AveragePool, 10,
                         OpSchema()
                             .FillUsing(PoolOpSchemaGenerator_opset10_to_11(
                                 "AveragePool", "average",
                                 "The output of each pooling window is divided by the number of "
                                 "elements (exclude pad when attribute count_include_pad is zero).",
                                 false, 10))
                             .Attr("count_include_pad",
...
ONNX_OPERATOR_SET_SCHEMA(AveragePool, 11,
                         OpSchema()
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("storage_order",
              "The storage order of the tensor. 0 is row major, and 1 is column major.",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("dilations", "Dilation value along each spatial axis of filter.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Output(1, "Indices",
                "Indices tensor from max pooling across the input tensor. "
                "The dimensions of indices are the same as output tensor. "
...
ONNX_OPERATOR_SET_SCHEMA(LpPool, 11,
                         OpSchema().FillUsing(LpPoolOpSchemaGenerator_opset11("LpPool")));
...
computes the output.)DOC";
    ...
    schema.Attr(
        ...
        "The shape of the convolution kernel. If not present, should be inferred from input W.",
        AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("dilations", "dilation value along each spatial axis of the filter.",
                AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("strides", "Stride along each spatial axis.", AttributeProto::INTS, OPTIONAL_VALUE);
    schema.Attr("auto_pad", auto_pad_doc2, AttributeProto::STRING, std::string("NOTSET"));
    schema.Attr("pads", pads_doc2, AttributeProto::INTS, OPTIONAL_VALUE);
...
static std::function<void(OpSchema &)>
ConvTransposeOpSchemaGenerator_opset1(const char *filter_desc) {
  return [=](OpSchema &schema) {
    std::string doc;
    POPULATE_OP_DOC_STR(doc = R"DOC(
The convolution transpose operator consumes an input tensor and {filter_desc},
...
output_shape can also be explicitly specified in which case pads values are auto generated using these equations:
  ...
  Else: pads[start_i] = total_padding[i] - (total_padding[i]/2); pads[end_i] = (total_padding[i]/2).
    ...
    schema.Attr("group", "number of groups input channels and output channels are divided into.",
                AttributeProto::INT, static_cast<int64_t>(1));
    schema.TypeAndShapeInferenceFunction(
        [](InferenceContext &ctx) { convTransposeShapeInference_opset1(ctx); });
  };
}

ONNX_OPERATOR_SET_SCHEMA(ConvTranspose, 1,
                         OpSchema().FillUsing(ConvTransposeOpSchemaGenerator_opset1("a filter")));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("is_test",
              ...
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("epsilon", "The epsilon value to use to avoid division by zero, default is 1e-5f.",
              AttributeProto::FLOAT, 1e-5f)
        .Attr("momentum",
              "Factor used in computing the running mean and variance."
              "e.g., running_mean = running_mean * momentum + mean * (1 - momentum), default is "
              "0.9f.",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "scale", "Scale tensor of shape (C).", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(2, "B", "Bias tensor of shape (C).", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(3, "mean", "running (training) or estimated (testing) mean tensor of shape (C).",
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(4, "var", "running (training) or estimated (testing) variance tensor of shape (C).",
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Output(0, "Y", "The output tensor of the same shape as X", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .Output(1, "mean", "The running mean after the BatchNormalization operator.", "T",
                OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(2, "var", "The running variance after the BatchNormalization operator.", "T",
                OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(3, "saved_mean",
                "Saved mean used during training to speed up gradient "
                "computation.",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "B", "Bias tensor of shape (C).", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(3, "input_mean",
               "running (training) or estimated (testing) mean tensor of shape (C).", "U",
               OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(4, "input_var",
               "running (training) or estimated (testing) variance tensor of shape (C).", "U",
               OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Output(0, "Y", "The output tensor of the same shape as X", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .Output(1, "running_mean", "The running mean after the BatchNormalization operator.", "U",
                OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(2, "running_var",
                ...
                "U", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .TypeConstraint("T",
                        {"tensor(float16)", "tensor(float)", "tensor(double)", "tensor(bfloat16)"},
                        "Constrain input and output types to float tensors.")
        .TypeConstraint(
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "The input data as Tensor.", "T")
        .Output(0, "output", "The output.", "T")
        .Output(1, "mask", "The output mask. If is_test is nonzero, this output is not filled.",
                "T", OpSchema::Optional)
        .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                        "Constrain input and output types to float tensors."));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               ...
               "in the form of `(N x C x D1 x D2 ... Dn)`.",
               "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .Input(1, "scale", "Scale tensor of shape `(num_groups)`.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(2, "bias", "Bias tensor of shape `(num_groups)`.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
