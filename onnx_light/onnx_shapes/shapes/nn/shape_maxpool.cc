...
void ComputeShapeMaxPool(ShapesContext &ctx, const NodeProto &node, const char *x) {
  ...
  EXT_ENFORCE_INVALID(GetAttributeInts(node, "kernel_shape", kernel_shape),
                      "ComputeShapeMaxPool: required attribute 'kernel_shape' is missing.");
  EXT_ENFORCE_INVALID(
      kernel_shape.size() == n_input_dims,
      "ComputeShapeMaxPool: attribute 'kernel_shape' size must match input rank - 2.");

  std::vector<int64_t> strides;
  if (GetAttributeInts(node, "strides", strides)) {
    EXT_ENFORCE_INVALID(strides.size() == n_input_dims,
                        "ComputeShapeMaxPool: attribute 'strides' size must match input rank - 2.");
  } else {
  ...
  if (GetAttributeInts(node, "dilations", dilations)) {
    EXT_ENFORCE_INVALID(
        ...
        "ComputeShapeMaxPool: attribute 'dilations' size must match input rank - 2.");
  } else {
  ...
  if (GetAttributeInts(node, "pads", pads)) {
    EXT_ENFORCE_INVALID(pads.size() == 2 * n_input_dims,
                        "ComputeShapeMaxPool: attribute 'pads' size must be 2 * (input rank - 2).");
  } else {
