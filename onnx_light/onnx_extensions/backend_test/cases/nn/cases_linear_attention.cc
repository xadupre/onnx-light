...
void RegisterLinearAttentionCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_linear_attention_linear_benchmark", {opset},
           {qkv_count, qkv_count, qkv_count}, {qkv_count, state_count}, []() -> IoData {
             const OpsetId opset = DefaultOpset(27);

             const KernelContext kernel_ctx{opset};
             const onnx_kernels::kernel::LinearAttention kernel{kernel_ctx};
  ...
  // Case 1: linear update rule
  {
    ...
    Expect(
        registry, std::move(node), "test_cc_linear_attention_linear", {opset}, [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
          Tensor key =
              Tensor::FromFloat("", {1, 2, 4}, {1.0f, 0.0f, 0.5f, 0.5f, 0.0f, 1.0f, -0.5f, 0.5f});
          Tensor value =
              Tensor::FromFloat("", {1, 2, 4}, {1.0f, 2.0f, 0.5f, -0.5f, 3.0f, 4.0f, -1.0f, 1.0f});
  ...
  // Case 2: gated update rule
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_gated", {opset}, [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
          Tensor key =
  ...
  // Case 3: delta update rule
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_delta", {opset}, [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
          Tensor key =
  ...
  // Case 4: gated_delta update rule (default)
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_gated_delta", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 5: with past_state
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_with_past_state", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
          ...
          return IoData{{std::move(query), std::move(key), std::move(value), std::move(past_state)},
                        {std::move(result.output), std::move(result.present_state)}};
        });
  }
  ...
  // Case 6: GQA — q_num_heads=4, kv_num_heads=2
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(registry, std::move(node), "test_cc_linear_attention_gqa", {opset}, [attrs]() -> IoData {
      Tensor key =
          Tensor::FromFloat("", {1, 2, 4}, {1.0f, 0.0f, 0.5f, 0.5f, 0.0f, 1.0f, -0.5f, 0.5f});
      Tensor value =
          Tensor::FromFloat("", {1, 2, 4}, {1.0f, 2.0f, 0.5f, -0.5f, 3.0f, 4.0f, -1.0f, 1.0f});
  ...
  // Case 7: gated with per-key-dim decay (decay_last = H_kv * d_k)
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_gated_perdim_decay", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 8: explicit scale attribute
  {
    ...
    Expect(
        registry, std::move(node), "test_cc_linear_attention_explicit_scale", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 9: gated_per_head_decay — gated rule with per-head scalar decay (last dim = H_kv).
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_gated_per_head_decay", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 10: gated_delta_beta_scalar — gated_delta with beta last dim = 1.
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_gated_delta_beta_scalar", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 11: gated_delta_gqa — q_num_heads=4, kv_num_heads=2 with gated_delta rule.
  {
    ...
    Expect(registry, std::move(node), "test_cc_linear_attention_gated_delta_gqa", {opset},
           [attrs]() -> IoData {
             Tensor key = Tensor::FromFloat("", {1, 2, 4},
                                            {1.0f, 0.0f, 0.5f, 0.5f, 0.0f, 1.0f, -0.5f, 0.5f});
             Tensor value = Tensor::FromFloat("", {1, 2, 4},
                                              {1.0f, 2.0f, 0.5f, -0.5f, 3.0f, 4.0f, -1.0f, 1.0f});
             Tensor q_gqa = Tensor::FromFloat("", {1, 2, 8},
                                              {1.0f, 0.0f, 0.5f, 0.5f, -1.0f, 1.0f, 0.2f, 0.3f,
                                               0.0f, 1.0f, 1.0f, -1.0f, 0.5f, 0.5f, -0.5f, 0.5f});
             Tensor decay = Tensor::FromFloat(
                 "", {1, 2, 4}, {-0.1f, -0.2f, -0.3f, -0.4f, -0.05f, -0.1f, -0.15f, -0.2f});
             Tensor beta = Tensor::FromFloat("", {1, 2, 2}, {0.8f, 0.9f, 0.7f, 0.6f});
  ...
  // Case 13: decode_step — T=1 with past_state (gated_delta).
  {
    ...
    Expect(registry, std::move(node), "test_cc_linear_attention_decode_step", {opset},
           [attrs]() -> IoData {
             ...
             Tensor k1 = Tensor::FromFloat("", {1, 1, 4}, {1.0f, 0.0f, 0.5f, 0.5f});
             Tensor v1 = Tensor::FromFloat("", {1, 1, 4}, {1.0f, 2.0f, 0.5f, -0.5f});
             Tensor past_state = Tensor::FromFloat(
  ...
  // Case 14: prefill_with_past — T>1 with non-zero past_state (gated_delta).
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_prefill_with_past", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
  ...
  // Case 15: no_past_explicit_zeros — past_state provided but filled with zeros (gated_delta).
  {
    ...
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(
        registry, std::move(node), "test_cc_linear_attention_no_past_explicit_zeros", {opset},
        [attrs]() -> IoData {
          Tensor query = Tensor::FromFloat("", {1, 2, 4},
                                           {1.0f, 0.0f, 0.5f, 0.5f, // t=0: h0=[1,0], h1=[0.5,0.5]
                                            0.0f, 1.0f, 1.0f, -1.0f});
          ...
          return IoData{{std::move(query), std::move(key), std::move(value), std::move(past_zeros),
                         std::move(decay), std::move(beta)},
                        {std::move(result.output), std::move(result.present_state)}};
        });
  }
  ...
  // FP16-rounded inputs in float32 and rounded back to FLOAT16.
  {
    ...
    AddAttribute<int64_t>(node, "q_num_heads", 2);
    AddAttribute<int64_t>(node, "kv_num_heads", 2);
    Expect(registry, std::move(node), "test_cc_linear_attention_fp16", {opset},
           [attrs, query, key, value, decay32, beta32]() -> IoData {
             Tensor q16 = RoundToFloat16(query);
             Tensor k16 = RoundToFloat16(key);
             Tensor v16 = RoundToFloat16(value);
             Tensor decay16 = RoundToFloat16(decay32);
