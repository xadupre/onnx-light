...
EinsumPlan BuildPlan(const std::vector<Tensor> &inputs, const std::string &raw_equation) {
  ...
  EXT_ENFORCE_INVALID(!(equation.empty()),
                      std::string(kEinsumName) + ": equation must not be empty.");
  ...
  EXT_ENFORCE_INVALID(input_terms.size() == inputs.size(),
                      std::string(kEinsumName) + ": number of input terms in the equation (" +
                          std::to_string(input_terms.size()) +
                          ") does not match number of inputs (" + std::to_string(inputs.size()) +
                          ").");
  ...
  for (std::size_t i = 0; i < input_terms.size(); ++i) {
    ...
    if (dots == std::string::npos) {
      ...
      EXT_ENFORCE_INVALID(term.size() == inputs[i].shape.size(),
                          std::string(kEinsumName) + ": term '" + term + "' has " +
    ...
    // dimensions.
    EXT_ENFORCE_INVALID(!(term.size() - 3 > inputs[i].shape.size()),
                        std::string(kEinsumName) + ": term '" + term +
  ...
  for (std::size_t i = 0; i < plan.input_labels.size(); ++i) {
    ...
    EXT_ENFORCE_INVALID(labels.size() == shape.size(),
                        std::string(kEinsumName) + ": expanded term '" + labels + "' has " +
  ...
  for (char c : plan.output_labels) {
    EXT_ENFORCE_INVALID(label_size_map.find(c) != label_size_map.end(),
                        std::string(kEinsumName) + ": output label '" + std::string(1, c) +
...
void EinsumInPlace(const std::vector<Tensor> &inputs, const std::string &equation, int32_t dtype,
  ...
  EXT_ENFORCE_INVALID(output.data_type == dtype,
                      std::string(kEinsumName) + ": output dtype mismatch.");
  EXT_ENFORCE_INVALID(output.shape == plan.output_shape,
                      std::string(kEinsumName) + ": output shape mismatch.");
  EXT_ENFORCE_INVALID(!(output.data.size() != static_cast<std::size_t>(out_count) * sizeof(T)),
                      std::string(kEinsumName) + ": output buffer size mismatch.");
...
void RequireHomogeneous(const std::vector<Tensor> &inputs) {
  EXT_ENFORCE_INVALID(!(inputs.empty()),
                      std::string(kEinsumName) + " requires at least one input.");
  const int32_t dtype = inputs[0].data_type;
  for (std::size_t i = 1; i < inputs.size(); ++i) {
    EXT_ENFORCE_INVALID(inputs[i].data_type == dtype,
                        std::string(kEinsumName) + ": all inputs must share the same dtype.");
  }
}
