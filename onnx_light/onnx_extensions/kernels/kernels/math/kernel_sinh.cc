...
constexpr const char *kName = "kernel::Sinh";
...
Sinh::Sinh(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}

void Sinh::RegisterTuningSchemas() {
  tuning::RegisterParallelTuningSchemas("Sinh", kSupportedElementTypes, kPortableParallelMinimum,
                                        kTuningAbi);
}

KernelTuningKey Sinh::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Sinh", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Sinh::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Sinh", parameters, tuning_, kTuningAbi);
}
...
void Sinh::operator()(const Tensor &x, Tensor &output) const {
  ...
  case DataType::FLOAT: {
    ...
    ParallelFor(n, tuning_.parallel_minimum_elements, [px, py](int64_t begin, int64_t end) {
      for (int64_t i = begin; i < end; ++i) {
        py[static_cast<size_t>(i)] = std::sinh(px[i]);
      }
    });
    return;
  }
...
void Sinh::Run(RuntimeContext &rt) {
