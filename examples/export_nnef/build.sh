...
# export_nnef example against that install.
#
# Usage (run from the repository root or from this directory):
#   bash examples/export_nnef/build.sh [install-prefix] [lib-build-dir] [example-build-dir]
...
INSTALL_PREFIX="${1:-${REPO_ROOT}/build/install-export-nnef}"
LIB_BUILD_DIR="${2:-${REPO_ROOT}/build/export-nnef-lib}"
EXAMPLE_BUILD_DIR="${3:-${REPO_ROOT}/build/export-nnef-example}"
...
echo "=== Step 2: configure and build export_nnef (${BUILD_TYPE}) ==="
...
echo
echo "Example binary:"
echo "  ${EXAMPLE_BUILD_DIR}/export_nnef"
