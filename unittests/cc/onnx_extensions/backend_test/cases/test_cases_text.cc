...
TEST(BackendTestCase, StringConcatCaseIsPresent) {
  ...
  // element-wise concatenated values.
  {
    ...
    EXPECT_EQ(ds.outputs[0].data_type, static_cast<int32_t>(core::runtime::DataType::STRING));
...
TEST(BackendTestCase, StringSplitCaseIsPresent) {
...

  {
    ...
    EXPECT_EQ(ds.outputs[0].data_type, static_cast<int32_t>(core::runtime::DataType::STRING));
    EXPECT_EQ(ds.outputs[1].data_type, static_cast<int32_t>(core::runtime::DataType::INT64));
