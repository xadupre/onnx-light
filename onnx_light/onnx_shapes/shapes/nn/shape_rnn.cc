...
SymDim NumDirectionsDim(const std::string &op_type, const std::string &direction) {
  if (direction == "forward" || direction == "reverse") {
    return SymDim(static_cast<int64_t>(1));
  }
  if (direction == "bidirectional") {
    return SymDim(static_cast<int64_t>(2));
  }
  return SymDim(op_type + ".num_directions(" + direction + ")");
}
...
SymDim HiddenSizeDim(const std::string &op_type, const NodeProto &node, const ShapesContext &ctx,
                     const char *r) {
  const int64_t hidden_size = GetAttributeOr<int64_t>(node, "hidden_size", -1);
  if (hidden_size > 0) {
    return SymDim(hidden_size);
  }
  if (r != nullptr && ctx.Has(r)) {
    const SymShape &r_shape = ctx.Get(r).Shape();
    if (r_shape.Rank() == 3u) {
      return r_shape[2];
    }
  }
  return SymDim(op_type + ".hidden_size()");
}
...
void ComputeShapeRNN(ShapesContext &ctx, const NodeProto &node, const char *x, const char *r) {
  ...
  const SymDim num_directions = NumDirectionsDim(op_type, direction);
  const SymDim hidden_size = HiddenSizeDim(op_type, node, ctx, r);
  const SymDim seq_length = layout == 0 ? x_shape[0] : x_shape[1];
  const SymDim batch_size = layout == 0 ? x_shape[1] : x_shape[0];
  ...
  if (!y_name.empty()) {
    ...
    ctx.Set(y_name, SymTensor(nullptr, input.Dtype(), std::move(y_shape)));
  }
  ...
  for (int i = 1; i < node.output_size(); ++i) {
    ...
    ctx.Set(name, SymTensor(nullptr, input.Dtype(), std::move(h_shape)));
  }
}
