...
void RegisterSoftsignCases(std::vector<TestCase> &registry) {
  ...
  // Mirrors upstream onnx ``test_softsign_example``: 1-D input [-1, 0, 1].
  {
    NodeProto node;
    node.set_op_type("Softsign");
    node.add_input("x");
    ...
    Tensor y = softsign_kernel(x);
    Expect(node, {x}, {y}, "test_cc_softsign_example", {opset}, "backend-test", registry);
  }
  // FLOAT16
  {
    NodeProto node;
    ...
    Tensor y = softsign_kernel(x);
    Expect(node, {x}, {y}, "test_cc_softsign_float16", {opset}, "backend-test", registry);
  }
  ...
  // BFLOAT16
  {
    ...
    Tensor y = softsign_kernel(x);
    Expect(node, {x}, {y}, "test_cc_softsign_bfloat16", {opset}, "backend-test", registry);
  }
}
