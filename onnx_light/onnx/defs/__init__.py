...
def _attr_default_value_repr(attr) -> str:  # type: ignore[no-untyped-def]
    """Returns a canonical string representation of an ``Attribute`` default value.

    Returns:
        A string encoding the attribute type and value suitable for equality
        comparison between two default values.
    """
...
class ConstraintDiff:
    ...
    name: str
    """Type parameter string (e.g. ``'T'``, ``'T1'``)."""
    kind: str
    """Type of difference: ``'added'``, ``'removed'``, or ``'changed'``."""
    added_types: list[str] = field(default_factory=list)
    """Types present in the new schema but not in the old one."""
    removed_types: list[str] = field(default_factory=list)
    """Types present in the old schema but not in the new one."""
    details: list[str] = field(default_factory=list)
    """Human-readable description of what changed."""
    is_breaking: bool = False
    """Indicates whether the difference is a breaking change."""
...
def _compare_parameters(
    ...
    Returns:
        A list of :class:`ParameterDiff` items describing additions, removals,
        and changes.
    """
    ...
    for name, (idx, p_new) in new_by_name.items():
        if name not in old_by_name:
            is_optional = p_new.option == OpSchema.FormalParameterOption.Optional
            diffs.append(
                ParameterDiff(
                    name=name,
                    kind="added",
    ...
    for name in old_by_name:
        ...
        if p_old.option != p_new.option:
            ...
            if p_new.option != OpSchema.FormalParameterOption.Optional:
                breaking = True
        ...
        if changes:
            diffs.append(
                ParameterDiff(name=name, kind="changed", details=changes, is_breaking=breaking)
            )
...
def _compare_attributes(old_attrs: dict, new_attrs: dict) -> list[AttributeDiff]:  # type: ignore[type-arg]
    ...
    Returns:
        A list of :class:`AttributeDiff` items describing additions, removals,
        and changes.
    """
...
def _compare_constraints(
    ...
    Returns:
        A list of :class:`ConstraintDiff` items describing additions, removals,
        and changes.
    """
...
def compare_schemas(schema_old: "OpSchema", schema_new: "OpSchema") -> SchemaDiff:  # type: ignore[name-defined]
    ...
    Args:
        ...
        schema_new: The schema to compare against (typically newer).

    Returns:
        A :class:`SchemaDiff` instance summarising all detected differences.
    ...
    Example::
        ...
        print(diff)
    """
    input_diffs = _compare_parameters(list(schema_old.inputs), list(schema_new.inputs), "input")
    output_diffs = _compare_parameters(
    ...
    if (
        ...
        old_max_str = (
            "∞" if OpSchema.is_infinite(schema_old.max_input) else str(schema_old.max_input)
        )
        new_max_str = (
            "∞" if OpSchema.is_infinite(schema_new.max_input) else str(schema_new.max_input)
        )
        breaking = (schema_new.min_input > schema_old.min_input) or (
            not OpSchema.is_infinite(schema_new.max_input)
            and (
                OpSchema.is_infinite(schema_old.max_input)
    ...
    if (
        ...
        old_max_str = (
            "∞" if OpSchema.is_infinite(schema_old.max_output) else str(schema_old.max_output)
        )
        new_max_str = (
            "∞" if OpSchema.is_infinite(schema_new.max_output) else str(schema_new.max_output)
        )
        breaking = (schema_new.min_output > schema_old.min_output) or (
            not OpSchema.is_infinite(schema_new.max_output)
            and (
                OpSchema.is_infinite(schema_old.max_output)
