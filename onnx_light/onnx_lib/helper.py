...
_ML_DTYPES_TENSOR_TYPE_MAP: dict[int, TensorDtypeMap] = {
    ...
    int(TensorProto.FLOAT8E4M3FNUZ): TensorDtypeMap(
        np.dtype(_ml_dtypes.float8_e4m3fnuz), int(TensorProto.INT32), "TensorProto.FLOAT8E4M3FNUZ"
    ),
    ...
    int(TensorProto.FLOAT8E5M2FNUZ): TensorDtypeMap(
        np.dtype(_ml_dtypes.float8_e5m2fnuz), int(TensorProto.INT32), "TensorProto.FLOAT8E5M2FNUZ"
    ),
...
def make_attribute(
    key: str, value: Any, doc_string: str | None = None, attr_type: int | None = None
) -> AttributeProto:
    ...
    # Iterable cases
        ...
        if len(value) == 0 and attr_type is None:
            raise ValueError(f"Could not infer attribute {key!r} type from empty iterator")
...
def make_tensor(
    ...
    if raw:
        ...
        if isinstance(vals, np.ndarray):
            ...
            raise TypeError(f"Raw data must be bytes or numpy.ndarray, but got {type(vals)}.")
    ...
    if data_type == TensorProto.COMPLEX128:
        vals = vals.view(np.float64)  # type: ignore
    elif data_type == TensorProto.COMPLEX64:
        vals = vals.view(np.float32)  # type: ignore
    elif data_type in {TensorProto.BFLOAT16, TensorProto.FLOAT16}:
        vals = vals.view(np.uint16)  # type: ignore
    elif data_type in {
...
def make_optional(name: str, elem_type: OptionalProto.DataType, value) -> OptionalProto:
...
def make_sequence(
    name: str, elem_type: SequenceProto.DataType, values: Sequence[Any]
) -> SequenceProto:
