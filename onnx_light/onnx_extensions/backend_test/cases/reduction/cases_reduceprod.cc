...
void EmitReduceProdCase(std::vector<TestCase> &registry, const std::string &case_name,
...
void EmitReduceProdDefaultAxesCase(std::vector<TestCase> &registry, const std::string &case_name,
                                   const std::vector<int64_t> &data_shape,
...
void RegisterReduceProdCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reduceprod_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             ...
             const onnx_kernels::kernel::ReduceProd kernel{kernel_ctx};
  ...
  EmitReduceProdDefaultAxesCase(registry, "test_cc_reduceprod_default_axes_keepdims", shape, values,
                                /*keepdims=*/true);
  EmitReduceProdCase(registry, "test_cc_reduceprod_keepdims", shape, values, {1},
                     /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_cc_reduceprod_do_not_keepdims", shape, values, {1},
                     /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_cc_reduceprod_negative_axes_keepdims", shape, values, {-2},
                     /*keepdims=*/true,
                     /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_cc_reduceprod_empty_axes_input_noop", shape, values, {},
                     /*keepdims=*/true,
                     /*noop_with_empty_axes=*/true);
  // Reducing over an axis of size 0: the product identity is 1, so the result
  // is a one-filled tensor.
  EmitReduceProdCase(registry, "test_cc_reduceprod_empty_set",
  ...
  EmitReduceProdCase(registry, "test_reduce_prod_keepdims_example", shape, values, {1},
                     /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_reduce_prod_keepdims_random", shape, random_values, {1},
                     /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_reduce_prod_do_not_keepdims_example", shape, values, {1},
                     /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_reduce_prod_do_not_keepdims_random", shape, random_values, {1},
                     /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceProdDefaultAxesCase(registry, "test_reduce_prod_default_axes_keepdims_example", shape,
                                values, /*keepdims=*/true);
  EmitReduceProdDefaultAxesCase(registry, "test_reduce_prod_default_axes_keepdims_random", shape,
                                random_values, /*keepdims=*/true);
  EmitReduceProdCase(registry, "test_reduce_prod_negative_axes_keepdims_example", shape, values,
                     {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_reduce_prod_negative_axes_keepdims_random", shape,
                     random_values, {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceProdCase(registry, "test_reduce_prod_empty_set",
                     /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                     /*noop_with_empty_axes=*/false);
}
