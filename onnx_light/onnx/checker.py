...
# : Raised when a model or proto fails validation.
    ...
    : ""
      ...
      Returns : None.""
                ...
                _checker.check_model(model)
                    ...
                    def check_attribute(attribute : _C.AttributeProto)
                              ->None
    : ""
    ...
    : raise ValidationError(
            f "The attribute {attribute.name!r} has no value: {attribute}") if sum (oneof) !=
      1 : raise ValidationError(
              f "The attribute {attribute.name!r} has more than one value: {attribute}")

              def check_sparse_tensor(sparse_tensor : _C.SparseTensorProto)
                  ->None
    : ""
      "Checks a sparse tensor and raises checker.ValidationError on invalid content.

      Raises : ValidationError : If the sparse tensor is invalid.""
                                                                 "
                                 if len (sparse_tensor.dims) !=
      2 : raise ValidationError(f "Only 2D sparse tensors are allowed: {tuple(sparse_tensor.dims)}")

              def check_graph(graph : _C.GraphProto)
                  ->None : ""
    ...
    : raise ValidationError(f "Expected a GraphProto, got {type(graph)}")
          ...
          def check_function_call_cycles(model : _C.ModelProto)
                  ->None : ""
                           "Checks for cycles in model-local function call graph.

                           Raises : ValidationError
    : If the model contains cyclic function references
                  .""
                   "
