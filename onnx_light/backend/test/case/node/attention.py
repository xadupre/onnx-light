...
#
# This file mirrors ``onnx.backend.test.case.node.attention`` so that
# :func:`onnx_light.backend.test.case.base.collect_test_case` registers the
# upstream node backend tests for the ``Attention`` operator. The reference
# implementation used to compute expected outputs is the one shipped with
# the ``onnx`` package (a dev dependency of ``onnx_light``); only the
# ``Base``/``expect`` helpers are wired through ``onnx_light``'s own test
# infrastructure so that the cases are merged with the C++-generated ones.
...
from onnx_light.backend.test.case.base import Base, expect

# Use a fixed seed so the inputs (and therefore the expected outputs computed
# by ``_compute_attention``) are deterministic across runs. The reference
# implementation from ``onnx`` is used to derive the outputs from the inputs,
# so any seed yields a self-consistent test case.
np.random.seed(0)
