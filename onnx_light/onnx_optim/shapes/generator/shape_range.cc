...
void ComputeShapeRange(ShapesContext &ctx, const NodeProto &node) {
  ...
  const OptimTensor &start = ctx.Get(std::string(node.input(0)));
  const OptimTensor &limit = ctx.Get(std::string(node.input(1)));
  const OptimTensor &delta = ctx.Get(std::string(node.input(2)));
