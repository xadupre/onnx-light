...
// ---------------------------------------------------------------------------
// ``qwen3_4_layers_like_fused`` — the fused counterpart of
// ``qwen3_4_layers_like``.
//
// This case reproduces the exact same 4-layer Qwen3-style causal language
// model as :cpp:func:`RegisterQwen3_4LayersLikeShapeInferenceCases`, but the
// two building blocks that the sibling case spells out as explicit subgraphs
// are expressed with their fused ONNX operators instead:
//
//   * every manual RMSNorm (``Cast + Pow + ReduceMean + Add + Sqrt +
//     Reciprocal + Mul + Cast + Mul``) collapses to a single
//     ``RMSNormalization`` node — this covers the pre-attention norm, the
//     per-head Q/K norms, the post-attention norm and the final norm;
...
// operator equivalent. The projection weights, RoPE tables and per-layer norm
// scales are shared with the inlined case (same names, shapes and dtypes).
//
// The fused operators require opset 23; the graph is otherwise identical in
// signature to the inlined case:
//
//   Inputs:
...
// ---------------------------------------------------------------------------
void RegisterQwen3_4LayersLikeFusedShapeInferenceCases(std::vector<TestCase> &registry,
                                                       TestMode /*mode*/) {
  const OpsetId opset = DefaultOpset(23);

  const std::string name = "test_cc_shape_inference_big_qwen3_4_layers_like_fused";
