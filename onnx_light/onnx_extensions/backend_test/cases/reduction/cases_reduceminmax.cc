...
void RegisterReduceMinMaxCases(std::vector<TestCase> &registry, const std::string &op_type,
  ...
  EmitReduceMinMaxDefaultAxesCase<Kernel>(
      registry, op_type, "test_cc_" + name_prefix + "_default_axes_keepdims", shape, values,
      /*keepdims=*/true);
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_keepdims", shape,
                               values, {1}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_do_not_keepdims",
                               shape, values, {1}, /*keepdims=*/false,
                               /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(
  ...
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_cc_" + name_prefix + "_empty_set",
  ...
  EmitReduceMinMaxCase<Kernel>(registry, op_type,
...
void RegisterReduceMinMaxOnnxCases(std::vector<TestCase> &registry, const std::string &op_type,
  ...
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_" + onnx_prefix + "_keepdims_example",
                               shape, example_values, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_" + onnx_prefix + "_keepdims_random", shape,
                               random_values, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(
      registry, op_type, "test_" + onnx_prefix + "_do_not_keepdims_example", shape, example_values,
      {1}, /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_" + onnx_prefix + "_do_not_keepdims_random",
                               shape, random_values, {1},
                               /*keepdims=*/false, /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxDefaultAxesCase<Kernel>(registry, op_type,
                                          "test_" + onnx_prefix + "_default_axes_keepdims_random",
                                          shape, random_values, /*keepdims=*/true);
  EmitReduceMinMaxCase<Kernel>(
      registry, op_type, "test_" + onnx_prefix + "_negative_axes_keepdims_example", shape,
      example_values, {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(
      registry, op_type, "test_" + onnx_prefix + "_negative_axes_keepdims_random", shape,
      random_values, {-2}, /*keepdims=*/true, /*noop_with_empty_axes=*/false);
  EmitReduceMinMaxCase<Kernel>(registry, op_type, "test_" + onnx_prefix + "_empty_set",
                               /*data_shape=*/{2, 0, 4}, /*data_values=*/{}, {1}, /*keepdims=*/true,
                               /*noop_with_empty_axes=*/false);
}
...
void RegisterReduceMaxCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducemax_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             const KernelContext reduce_max_kernel_ctx{DefaultOpset(18)};
             const onnx_kernels::kernel::ReduceMax reduce_max_kernel{reduce_max_kernel_ctx};
  ...
  RegisterReduceMinMaxCases<onnx_kernels::kernel::ReduceMax>(registry, "ReduceMax", "reducemax");
  RegisterReduceMinMaxOnnxCases<onnx_kernels::kernel::ReduceMax>(registry, "ReduceMax",
                                                                 "reduce_max");
  ...
  EmitReduceMinMaxDefaultAxesCase<onnx_kernels::kernel::ReduceMax>(
      registry, "ReduceMax", "test_reduce_max_default_axes_keepdim_example",
      /*data_shape=*/{3, 2, 2},
...
void RegisterReduceMinCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_cc_reducemin_default_axes_keepdims_benchmark",
           {DefaultOpset(18)}, {256 * 256 * 16}, {1}, []() -> IoData {
             const KernelContext reduce_min_kernel_ctx{DefaultOpset(18)};
             const onnx_kernels::kernel::ReduceMin reduce_min_kernel{reduce_min_kernel_ctx};
  ...
  RegisterReduceMinMaxCases<onnx_kernels::kernel::ReduceMin>(registry, "ReduceMin", "reducemin");
  RegisterReduceMinMaxOnnxCases<onnx_kernels::kernel::ReduceMin>(registry, "ReduceMin",
                                                                 "reduce_min");
  EmitReduceMinMaxDefaultAxesCase<onnx_kernels::kernel::ReduceMin>(
      registry, "ReduceMin", "test_reduce_min_default_axes_keepdims_example",
      /*data_shape=*/{3, 2, 2},
      {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f},
      /*keepdims=*/true);
