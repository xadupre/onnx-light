...
#include "onnx_lib/defs/operator_sets_preview.h"
#include "onnx_lib/defs/operator_sets_training.h"

#ifdef ONNX_ML
#include "onnx_lib/defs/operator_sets_ml.h"
#endif
...
OpSchema &OpSchema::SetDoc(const std::string &doc [[maybe_unused]]) {
#ifndef __ONNX_NO_DOC_STRINGS
  doc_ = doc;
#endif
  return *this;
...
bool OpSchema::HasContextDependentFunction() const {
  return !opset_version_to_function_builder_.empty();
}

bool OpSchema::HasContextDependentFunctionWithOpsetVersion(int opset_version) const {
...
void OpSchema::CheckInputOutputType(struct InferenceContext &ctx) const {
  ...
  for (size_t in_idx = 0; in_idx < ctx.getNumInputs(); ++in_idx) {
    // If the last input is Variadic by definition, checker still needs to check the rest of actual
    // input's type
    ...
    if (nullptr == param_type || !param_type->is_set()) {
      continue;
    } else if (!all_types.empty() &&
               all_types.find(Utils::DataTypeUtils::ToType(*param_type)) == all_types.end()) {
      fail_check(param.GetName(), " typestr: ", type_str,
                 ", has unsupported type: ", *Utils::DataTypeUtils::ToType(*param_type));
    }
  ...
  for (size_t out_idx = 0; out_idx < ctx.getNumOutputs(); ++out_idx) {
    // If the last output is Variadic by definition, checker still needs to check the rest of actual
    // output's type
...
void OpSchema::Verify(const NodeProto &node) const {
  ...
  for (int in_idx = 0; in_idx < node.input_size(); ++in_idx) {
    if (in_idx >= static_cast<int>(inputs_.size())) {
      if (!inputs_.empty() && Variadic == inputs_.back().GetOption()) {
        ...
        fail_check("Node (", node.name(), ") has more inputs (", node.input_size(),
                   ") than declared (", inputs_.size(), ") in op definition.");
      }
    }
    if (node.input(in_idx).empty() && (Single == inputs_[in_idx].GetOption())) {
      fail_check("Node (", node.name(), ")'s input ", in_idx,
  ...
  for (int out_idx = 0; out_idx < node.output_size(); ++out_idx) {
    if (out_idx >= static_cast<int>(outputs_.size())) {
      if (!outputs_.empty() && Variadic == outputs_.back().GetOption()) {
        ...
        fail_check("Node (", node.name(), ") has more outputs (", node.output_size(),
                   ") than declared (", outputs_.size(), ") in op definition.");
      }
    }

    if (node.output(out_idx).empty() && (Single == outputs_[out_idx].GetOption())) {
      fail_check("Node (", node.name(), ")'s output ", out_idx,
                 " is marked single but has an empty string in the graph");
    }
  }
  ...
  for (const auto &attr_proto : node.attribute()) {
    ...
    if (attr_proto.type() != expected_type) {
      fail_check("Mismatched attribute type in '", node.name(), " : ", name, "'. Expected: '",
                 AttributeProto_AttributeType_Name(expected_type), "', actual: '",
...
void OpSchema::VerifyInputNum(int input_num, std::string_view node_name) const {
  if (input_num < min_input_ || input_num > max_input_) {
    fail_check(VerifyFailPrefix(node_name), " has input size ", input_num,
...
void OpSchema::VerifyOutputNum(int output_num, std::string_view node_name) const {
  if (output_num < min_output_ || output_num > max_output_) {
    fail_check(VerifyFailPrefix(node_name), " has output size ", output_num,
...
OpSchema &OpSchema::SinceVersion(OperatorSetVersion v) {
  ...
  // default opset_version is for a FunctionProto of the same opset_version as the op's
  // since_version_. It is indexed with -1 so we need to reindex it with since_version_.
  //
  // FunctionProtos of non-default opset_versions are for models whose opset version is higher than
  // the op's opset version such that ops used in the default function_proto are no longer valid.
  // For example: A model of opset version 18 contains a LayerNormalization op. LayerNormalization
  // is function op whese function body uses ReduceMean op. LayerNormalization's since_version is 17
  // thus it is good for the model of opset 18. however, if a runtime needs to inline
  // LayerNormalization, the inlined model has a ReduceMean op. ReduceMean in opset 18 is different
  // from opset 17. This requires us to define more than one function body
  ...
  if (it_function_body != opset_version_to_function_body_.end()) {
    ...
    UpdateFunctionProtoOpsetImportVersion(*opset_version_to_function_body_[since_version_],
                                          since_version_);
...
OpSchema &OpSchema::Attr(const char *name, const char *description,
                         AttributeProto::AttributeType type, const char *defaultValue) {
...
OpSchema &OpSchema::Input(int n, std::string name, const std::string &description,
  ...
  return Input(n, FormalParameter(std::move(name),
#ifndef __ONNX_NO_DOC_STRINGS
                                  description,
#else
                                  std::string(),
#endif
                                  std::move(type_str), param_option, is_homogeneous, min_arity,
...
OpSchema &OpSchema::Input(int n, const char *name, const char *description, const char *type_str,
  ...
  return Input(n, std::string(name),
               ...
               std::string(),
#endif
               std::string(type_str), param_option, is_homogeneous, min_arity,
               differentiation_category);
}
...
OpSchema &OpSchema::Output(int n, std::string name, const std::string &description,
                           ...
                           bool is_homogeneous, int min_arity,
                           DifferentiationCategory differentiation_category) {
  return Output(n, FormalParameter(std::move(name),
#ifndef __ONNX_NO_DOC_STRINGS
                                   description,
#else
                                   std::string(),
...
bool OpSchema::BuildContextDependentFunction(const FunctionBodyBuildContext &ctx,
  ...
  if (opset_version_to_function_builder_.empty() ||
      it == opset_version_to_function_builder_.begin()) {
    ONNX_THROW_EX(std::out_of_range(
        std::string("Cannot find a function builder that satisfies the requested opset version: "
                    "op_type = ") +
...
// A function of a schema (either stored in opset_version_to_function_body_ or built with one of
// function builder in opset_version_to_function_builder_) has predefined opset_imports. Before
// returning the function, we shall update the predefined opset_imports so that it is consistent
// with the requested version. Note that this call only update opset_import of the default domain.
// TODO(ONNX): extend this call to work for no-default domains.
void OpSchema::UpdateFunctionProtoOpsetImportVersion(FunctionProto &function_proto,
                                                     int requested_opset_version) const {
...
const FunctionProto *OpSchema::GetFunction(int requested_opset_version, bool validate) const {
  ...
  if (it != opset_version_to_function_body_.begin()) {
    ...
    if (!validate || ValidateReferencedOpsInFunction(function, requested_opset_version,
                                                     function_since_version)) {
      return function;
    }
  }
  return nullptr;
...
// are not updated between function_since_version and requested_opset_version (include
// requested_opset_version). this call only validate ops in the default domain.
// TODO(ONNX): validate ops in other domains.
bool OpSchema::ValidateReferencedOpsInFunction(const FunctionProto *function,
  ...
  for (const auto &node : function->node()) {
    if (node.domain().empty() || node.domain() == "ai.onnx") {
      const OpSchema *op1 = OpSchemaRegistry::Instance()->GetSchema(
          node.op_type(), requested_opset_version, node.domain());
      const OpSchema *op2 = OpSchemaRegistry::Instance()->GetSchema(
...
const std::vector<std::string> &OpSchema::numeric_types_for_math_reduction_ir9() {
  static const std::vector<std::string> numeric_types_for_math_reduction_ir9 = {
      ...
      "tensor(double)",         "tensor(bfloat16)",   "tensor(float8e4m3fn)",
...
const std::vector<std::string> &OpSchema::all_numeric_sequence_types() {
  static const std::vector<std::string> all_numeric_sequence_types = {
      "seq(tensor(uint8))",   "seq(tensor(uint16))", "seq(tensor(uint32))", "seq(tensor(uint64))",
      "seq(tensor(int8))",    "seq(tensor(int16))",  "seq(tensor(int32))",  "seq(tensor(int64))",
...
const std::vector<std::string> &OpSchema::all_tensor_types() {
  static const std::vector<std::string> all_tensor_types = {
      "tensor(uint8)",   "tensor(uint16)",    "tensor(uint32)",    "tensor(uint64)",
      "tensor(int8)",    "tensor(int16)",     "tensor(int32)",     "tensor(int64)",
      "tensor(float16)", "tensor(float)",     "tensor(double)",    "tensor(string)",
...
const std::vector<std::string> &OpSchema::all_float_types_plus_Xint8_ir4() {
  static const std::vector<std::string> all_float_types_ir4 = {
      "tensor(bfloat16)", "tensor(float16)", "tensor(float)",
      "tensor(double)",   "tensor(int8)",    "tensor(uint8)"};
  return all_float_types_ir4;
}
...
const std::vector<std::string> &OpSchema::all_float_types_ir9() {
  static const std::vector<std::string> all_float_types_ir9 = {
      "tensor(bfloat16)",   "tensor(float16)",       "tensor(float)",
      "tensor(double)",     "tensor(float8e4m3fn)",  "tensor(float8e4m3fnuz)",
      "tensor(float8e5m2)", "tensor(float8e5m2fnuz)"};
...
#define ENFORCE(x)                                                                                 \
  do {                                                                                             \
    if (!(x))                                                                                      \
      ONNX_THROW_EX(                                                                               \
          std::logic_error("ONNX Schema " + name_ + ": failed validating the check: " + #x));      \
  } while (0)
  ...
  if (node_determinism_ == NodeDeterminism::Unknown) {
    ...
    if (HasContextDependentFunction()) {
      ...
      for (const auto &opset : func_proto->opset_import()) {
        domain_to_opset_version[std::string(opset.domain())] = opset.version();
      }
      for (const NodeProto &n : func_proto->node()) {
        const int64_t opset = domain_to_opset_version[std::string(n.domain())];
...
OpSchemaRegistry::DomainToVersionRange::LastReleaseVersionMap() const {
...
int OpSchemaRegistry::OpSchemaRegisterOnce::GetMaxRegisteredVerWithinTarget(
    const std::map<OperatorSetVersion, OpSchema> &m, int target_ver) {
...
void OpSchemaRegistry::OpSchemaRegisterOnce::CheckDomainAndVersionToRegister(
...
void OpSchemaRegistry::OpSchemaDeregister(const std::string &op_type, const int version,
...
int OpSchemaRegistry::GetLoadedSchemaVersion() { return loaded_schema_version; }
