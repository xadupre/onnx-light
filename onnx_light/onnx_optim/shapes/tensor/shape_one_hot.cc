...
void ComputeShapeOneHot(ShapesContext &ctx, const NodeProto &node) {
  ...
  const OptimTensor &indices = ctx.Get(std::string(node.input(0)));
  const OptimTensor &depth = ctx.Get(std::string(node.input(1)));
  const OptimTensor &values = ctx.Get(std::string(node.input(2)));
