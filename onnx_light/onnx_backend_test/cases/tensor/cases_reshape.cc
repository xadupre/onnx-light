...
void RegisterReshapeCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  {
    Expect(registry, MakeReshapeNode(), "test_cc_reshape_reordered", {opset}, [=]() -> IoData {
      const OpsetId opset = DefaultOpset(13);
      const kernel::KernelContext ctx{opset};
      const kernel::Reshape reshape_kernel{ctx};
  ...
  // expansion) so onnx-light covers the same set of named cases.
  {
    ...
    for (const auto &c : cases) {
      Expect(registry, MakeReshapeNode(), "test_cc_reshape_" + c.first, {opset},
             [reshape_kernel, shape_vec = c.second]() -> IoData {
