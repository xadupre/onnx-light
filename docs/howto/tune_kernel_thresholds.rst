...
Portable Gemm algorithm calibration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Gemm tuning ABI 2 adds the integer ``algorithm.configuration`` parameter.
It selects a finite configuration of the existing implementation, not a new
Gemm algorithm or unrestricted tile dimensions:

.. list-table::
   :header-rows: 1

   * - Configuration
     - Difference from portable defaults
   * - 0
     - None: tiles ``64 x 256 x 256``, packs B at 16384 elements
   * - 1
     - M tile 32
   * - 2
     - N tile 64
   ...
   * - 4
     - Always packs B
   * - 5
     - Disables dynamic B packing
   * - 6
     - Tiles ``32 x 64 x 128``

The skinny-M rule, reduction order, conversion threshold and work-unit settings
are unchanged. Prepared constant B remains packed independently of the dynamic
packing choice. Historical tile, packing, conversion and work-unit profile keys
are not restored. ABI 1 Gemm profiles are rejected as incompatible by the cache
loader with diagnostics; recalibrate rather than changing their ABI manually.
Other kernels' profiles are unaffected.

The default Gemm calibration still searches ``parallel.minimum_tasks``.
Request the algorithm search explicitly:

.. code-block:: bash

    python -m onnx_light kernel \
        --kernel Gemm --dtype FLOAT --impl portable --tune \
        --parameter algorithm.configuration=default,1,2,3,4,5,6 \
        --maximum-duration-ms 2000 --maximum-memory-mb 8

This search always includes portable configuration 0 as its reference. It
compares a fixed corpus, including skinny, square, rectangular, tile-tail and
transposed matrices, on the active session executor. It also works with one
participant. FLOAT, DOUBLE, FLOAT16 and BFLOAT16 are calibrated separately.
Every candidate must reproduce the reference output exactly before timing.
An incomplete corpus retains configuration 0 instead of selecting from partial
measurements. Timings select only a processor/execution-specific profile;
portable defaults are never promoted by calibration.
The algorithm search holds ``parallel.minimum_tasks`` at its portable value 2;
the threshold search holds configuration 0. Each publishes a complete profile,
so these two searches do not compose previously calibrated values. A different
joint setting needs its own measurement rather than combining unrelated wins.

The algorithm search requires a 5% improvement in the sum of case medians and
allows at most a 10% slowdown on any individual case. It uses five samples per
candidate/case with a rotating measurement order. The default limits are 1000 ms
and 64 MiB per dtype. Memory admission includes resident input/output tensors,
packing storage and half-precision promotion/demotion temporaries. Time checks
are cooperative before allocations, warmups and samples: an in-flight kernel
cannot be interrupted. If the deadline expires, the partial comparison is
reported but no partial winner is selected. These are calibration limits, not
limits on process memory, executor storage or later inference.
