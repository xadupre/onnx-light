...
static int32_t GetSoftmaxElementType(int32_t input_elem_type,
...
static void ValidateModifierGraph(const AttributeProto *attr, const std::string &attr_name,
  ...
  if (graph.input_size() != kModifierInputCount) {
    fail_shape_inference("Attribute ", attr_name, " expected ", kModifierInputCount,
                         " input but graph has ", graph.input_size(), ".");
  }
  ...
  auto validate_dtype = [&](const ValueInfoProto &vi, int32_t expected, const char *desc) {
    ...
    if (vi.type().tensor_type().elem_type() != expected) {
      fail_shape_inference("Attribute ", attr_name, " ", desc, " type mismatch. Expected ",
  ...
  if (graph.output(0).has_type()) {
    ...
    if (expected_output_elem_type.has_value() &&
      ...
      fail_shape_inference("Attribute ", attr_name, " output type mismatch. Expected ",
  ...
  if (graph.input(0).has_type() && graph.input(0).type().has_tensor_type()) {
    ...
    if (input_tensor.has_shape() && input_tensor.shape().dim_size() != 4) {
      fail_shape_inference("Attribute ", attr_name,
                           " input must be rank-4 tensor when shape is specified.");
    }
  }
  if (graph.output(0).has_type() && graph.output(0).type().has_tensor_type()) {
    ...
    if (output_tensor.has_shape() && output_tensor.shape().dim_size() != 4) {
      fail_shape_inference("Attribute ", attr_name,
                           " output must be rank-4 tensor when shape is specified.");
    }
  }
  if (graph.input(0).has_type() && graph.output(0).has_type() &&
      graph.input(0).type().has_tensor_type() && graph.output(0).type().has_tensor_type()) {
    ...
    for (int i = 0; i < dim_count; ++i) {
      if (in_shape.dim(i).has_dim_value() && out_shape.dim(i).has_dim_value() &&
        ...
        fail_shape_inference("Attribute ", attr_name,
                             " output shape must match input shape when specified.");
      }
    }
  }
}
...
static void FlexAttentionShapeInference(InferenceContext &ctx) {
  ...
  if (q_shape.dim_size() != 4 || k_shape.dim_size() != 4 || v_shape.dim_size() != 4) {
    fail_shape_inference("FlexAttention requires rank-4 inputs: "
...
//                   Y
//
static bool BuildFlexAttentionFunctionBody(const FunctionBodyBuildContext &ctx,
  ...
  const int32_t softmax_precision = GetSoftmaxElementType(static_cast<int32_t>(input_elem_type),
                                                          ctx.getAttribute("softmax_precision"));
  ...
  if (!ValidateModifierGraphForBuilder(score_mod_attr) ||
      !ValidateModifierGraphForBuilder(prob_mod_attr)) {
    return false;
  }
  ...
  builder.Add("QNumHeads = Shape <start = 1, end = 2> (Q)")
      .Add("KVNumHeads = Shape <start = 1, end = 2> (K)");
  ...
  builder.Add("KVRepeat = Div (QNumHeads, KVNumHeads)")
      ...
      .Add("KExpandShape = Concat <axis = 0> (BatchSize, KVNumHeads, KVRepeat, KVSeqLen, "
           "KHeadSize1D)")
      ...
      .Add("VExpandShape = Concat <axis = 0> (BatchSize, KVNumHeads, KVRepeat, KVSeqLen, "
           "VHeadSize1D)")
  ...
  // Apply score_mod.
  if (need_score_mod) {
    builder.AddInlinedCall({"ScoreAfterScoreMod"}, score_mod_attr->g(), {"SoftmaxCast"},
                           "ScoreMod_");
  } else {
...
ONNX_PREVIEW_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "Q", "Query tensor with shape `(batch_size, q_num_heads, q_seq_len, head_size)`.",
               "T1")
        .Input(1, "K", "Key tensor with shape `(batch_size, kv_num_heads, kv_seq_len, head_size)`.",
               "T1")
        .Input(2, "V",
        ...
        .Output(0, "Y",
                ...
                "T1")
        .Attr("scale", "Scaling factor for Q*K^T. Defaults to 1/sqrt(head_size).",
        ...
        .Attr("score_mod",
              "Optional score modifier subgraph with 1 rank-4 tensor input and 1 rank-4 tensor "
              "output "
        ...
        .Attr("prob_mod",
              "Optional probability modifier subgraph with 1 rank-4 tensor input and 1 rank-4 "
              "tensor output "
