...
void ComputeShapeEinsum(ShapesContext &ctx, const NodeProto &node) {
  ...
  EXT_ENFORCE_INVALID(!(n_inputs < 1), "ComputeShapeEinsum: Einsum requires at least one input.");
  ...
  EXT_ENFORCE_INVALID(!(equation.empty()), "ComputeShapeEinsum: 'equation' attribute is required.");
  ...
  for (int i = 1; i < n_inputs; ++i) {
    input_shapes.push_back(ctx.Get(node.input(i)).Shape());
  }
  ...
  for (int i = 0; i < n_inputs; ++i) {
    ...
    if (dots == std::string::npos) {
      EXT_ENFORCE_INVALID(term.size() == rank, "ComputeShapeEinsum: term '", term, "' has ",
                          term.size(), " labels but input ", i, " has rank ", rank, ".");
      continue;
    }
