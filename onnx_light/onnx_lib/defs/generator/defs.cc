...
#include "onnx_lib/defs/generator/utils.h"
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("sparse_value", "The value for the elements of the output tensor in sparse format.",
              AttributeProto::SPARSE_TENSOR, false)
        .Attr("value_int", "The value for the sole element for the scalar, int64, output tensor.",
              AttributeProto::INT, false)
        .Attr("value_ints", "The values for the elements for the 1D, int64, output tensor.",
              AttributeProto::INTS, false)
        .Attr("value_float",
              "The value for the sole element for the scalar, float32, output tensor.",
              AttributeProto::FLOAT, false)
        .Attr("value_floats", "The values for the elements for the 1D, float32, output tensor.",
              AttributeProto::FLOATS, false)
        .Attr("value_string",
              "The value for the sole element for the scalar, UTF-8 string, output tensor.",
              AttributeProto::STRING, false)
        .Attr("value_strings",
              "The values for the elements for the 1D, UTF-8 string, output tensor.",
              AttributeProto::STRINGS, false)
...
ONNX_OPERATOR_SET_SCHEMA(ConstantOfShape, 25,
                         OpSchema()
                             .SetDoc(kDoc_ConstantOfShape_ver24)
                             .Attr("value",
                                   "(Optional) The value of the output elements."
                                   "Should be a one-element tensor. If not specified, it defaults "
                                   "to a tensor of value 0 and datatype float32",
                                   AttributeProto::TENSOR, OPTIONAL_VALUE)
                             .Input(0, "input",
                                    "1D tensor. The shape of the expected output tensor. If empty "
                                    "tensor is given, the output would be a scalar."
                                    " All values must be >= 0.",
                                    "T1")
                             .Output(0, "output",
                                     "Output tensor of shape specified by 'input'."
                                     "If attribute 'value' is specified, the value and datatype of "
                                     "the output tensor is taken from 'value'."
                                     "If attribute 'value' is not specified, the value in the "
                                     "output defaults to 0, and the datatype "
                                     "defaults to float32.",
                                     "T2")
                             ...
                             .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
                               if (ctx.getAttribute("value") != nullptr) {
                                 propagateElemTypeFromDtypeToOutput(ctx, ctx.getAttribute("value"),
                                                                    0);
                               } else {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input", "2D input tensor to copy shape, and optionally, type information from.",
               "T1")
        .Output(0, "output", "Output tensor, same shape as input tensor T1.", "T2")
        .TypeConstraint("T1", OpSchema::all_non_complex_numeric_types_plus_bool_ir4(),
                        "Constrain input types. Strings and complex are not supported.")
        .TypeConstraint("T2", OpSchema::all_non_complex_numeric_types_plus_bool_ir4(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("dtype",
              ...
              "TensorProto::FLOAT.",
              AttributeProto::INT, static_cast<int64_t>(TensorProto::FLOAT))
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("scale", "The standard deviation of the normal distribution.", AttributeProto::FLOAT,
              1.0f)
        .Attr(
            "seed",
            "(Optional) Seed to the random generator, if not specified we will auto generate one.",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("sample_size", "Number of times to sample.", AttributeProto::INT,
        ...
        .Attr("dtype",
              "(Optional) The data type for the elements of the output tensor, if not specified, "
              "we will use int32.",
              AttributeProto::INT, static_cast<int64_t>(TensorProto::INT32))
        .Input(0, "input",
...
static bool BuildFunctionBodyRange27(const FunctionBodyBuildContext &ctx, const OpSchema &schema,
  ...
  if (needs_stash && stash_type != T) {
    ...
    builder.Add("sub_result = Sub (limit, start)")
        .Add("sub_result_casted = Cast (sub_result)", "to",
             static_cast<int64_t>(TensorProto_DataType_FLOAT))
        ...
        .Add("ceil_result_relu_int = Cast (ceil_result_relu)", "to",
             static_cast<int64_t>(TensorProto_DataType_INT64))
        .Add("ceil_result_relu_bool = Cast (ceil_result_relu)", "to",
             static_cast<int64_t>(TensorProto_DataType_BOOL))
        .Add(
            R"ONNX(variadic_output, output = Loop (ceil_result_relu_int, ceil_result_relu_bool, start)
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          FunctionBuilder builder(functionProto);
          builder
              .Add("X_random = RandomUniformLike <low = 0.0, high = 1.0, seed = @seed> (input)",
                   "dtype", static_cast<int64_t>(input_type))
