...
void PrintModelSummary(const ONNX_LIGHT_NAMESPACE::ModelProto &model) {
  ...
  if (model.has_producer_version()) {
    std::cout << "  Producer version : " << std::string(model.ref_producer_version()) << "\n";
  }
  if (model.has_graph()) {
...
// allocation keeps the freed blocks resident so the loop measures the
// serializer itself, mirroring the steady state of a long-running process.
