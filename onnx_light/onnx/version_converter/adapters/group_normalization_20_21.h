...
public:
  ...
  void transform_input(const std::shared_ptr<Graph> &graph, Node *node, int64_t input_id,
                       Value *reshape0_shape, Value *reshape1_shape, Value *expand_shape) const {
  ...
  void adapt_group_normalization_20_21(const std::shared_ptr<Graph> &graph, Node *node) const {
    ...
    //                                                           |
    // Reshape(input1/2, [-1, 1]) ----------- Expand(Reshape_out, [1, Div_out]) --
    // Reshape(Expand_out, [-1])
    //
    ...
    transform_input(graph, node, 1, constant_reshape0_shape->output(),
                    constant_reshape1_shape->output(), concat->output());
    transform_input(graph, node, 2, constant_reshape0_shape->output(),
                    constant_reshape1_shape->output(), concat->output());
