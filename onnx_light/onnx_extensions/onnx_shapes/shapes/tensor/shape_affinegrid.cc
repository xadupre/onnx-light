...
void ComputeShapeAffineGrid(ShapesContext &ctx, const NodeProto &node) {
  ...
  if (size_shape.Rank() != 1) {
    ...
    EXT_ENFORCE_INVALID(size_shape.Rank() == 0,
                        "ComputeShapeAffineGrid: size must be a 1-D tensor.");
  }
  if (size_shape.Rank() == 1 && size_shape[0].IsInt()) {
    ...
    if (mode == Mode::kUnknown) {
      ...
      EXT_THROW_INVALID("ComputeShapeAffineGrid: theta rank and size length disagree on 2D vs 3D.");
    }
  }
