...
bool Status::IsOK() const noexcept { return (state_ == nullptr); }

StatusCategory Status::Category() const noexcept {
  return IsOK() ? StatusCategory::NONE : state_->category;
}

StatusCode Status::Code() const noexcept { return IsOK() ? StatusCode::OK : state_->code; }

const std::string &Status::ErrorMessage() const { return IsOK() ? EmptyString() : state_->msg; }
