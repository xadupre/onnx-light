...
class TestRunNodesBindings(ExtTestCase):
    ...
    def test_runtime_session_parallel_region_report(self):
        ...
        self.assertIsNone(event.cpu_cycles)
        self.assertIsNone(event.retired_instructions)
        self.assertIsNone(event.llc_references)
        self.assertIsNone(event.llc_misses)
        self.assertIsNone(event.counter_time_enabled)
        self.assertIsNone(event.counter_time_running)
