...
:param opts: TensorBufferOptions (or a subclass such as SerializeOptions) that
  ...
  // Takes a FunctionProto (by reference), a list of serialised input TypeProtos,
  // and a list of serialised formal AttributeProtos. Returns a list of
  // serialised output TypeProtos, one per function output.
  shape_inference_mod.def(
      ...
      [](const FunctionProto &function, const std::vector<nb::bytes> &input_types_bytes,
        ...
        for (size_t ni = 0; ni < function.ref_node().size(); ++ni) {
          ...
          for (size_t oi = 0; oi < node.ref_output().size(); ++oi) {
            ...
            if (!out_name.empty() && oi < ctx.output_types_.size() &&
                ctx.output_types_[oi].has_type()) {
      ...
      "Infers output types of a FunctionProto given serialised input TypeProtos and "
