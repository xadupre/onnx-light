...
#include "onnx_lib/defs/data_propagators.h"
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("saturate",
              ...
              "(float8e4m3fn, float8e4m3fnuz, float8e5m2, float8e5m2fnuz, float8e8m0). It is true "
              "by default. All cases are fully described in the tables inserted in the operator "
              "description.",
        ...
        .Attr("round_mode",
              ...
              "`down`: round to nearest value towards zero, `nearest`: round to nearest value and "
              "ties round up.",
              AttributeProto::STRING, std::string("up"))
        .Input(0, "input", "Input tensor to be cast.", "T1", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Output(0, "output",
                "Output tensor with the same shape as input with type specified by the 'to' "
                "argument",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .Attr(
            ...
            "The \"linear\" mode includes linear and N-linear interpolation modes depending on the "
        ...
        .Attr("padding_mode",
              ...
              AttributeProto::STRING, std::string("zeros"))
        .Attr("align_corners",
        ...
        .Input(
            ...
            "Therefore, it should have most values in the range of [-1, 1]. If the grid has values "
            ...
            "the coordinates in the length-r location vector are listed from the innermost tensor "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "T2", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T1", OpSchema::all_non_complex_tensor_types_ir12(),
                        "Constrain input types. Casting from complex is not supported.")
        .TypeConstraint("T2", OpSchema::all_non_complex_tensor_types_ir12(),
                        "Constrain output types. Casting to complex is not supported.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateElemTypeFromAttributeToOutput(ctx, "to", 0);
          if (hasNInputShapes(ctx, 1)) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "specified by the 'to' argument",
                "T2", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T1", OpSchema::all_non_complex_tensor_types_ir11(),
                        "Constrain input types. Casting from complex is not supported.")
        .TypeConstraint("T2", OpSchema::all_non_complex_tensor_types_ir11(),
                        "Constrain output types. Casting to complex is not supported.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateElemTypeFromAttributeToOutput(ctx, "to", 0);
          if (hasNInputShapes(ctx, 1)) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .SetContextDependentFunctionBodyBuilder([](const FunctionBodyBuildContext &ctx,
          ...
          builder.Add(MakeString("output = Cast <to= ", static_cast<int64_t>(target_elt_type),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output",
                ...
                "the second input tensor.",
                "T2", OpSchema::Single, true, 1, OpSchema::Differentiable)
        .TypeConstraint("T1", OpSchema::all_non_complex_tensor_types_ir10(),
                        "Constrain input types. Casting from complex is not supported.")
        .TypeConstraint("T2", OpSchema::all_non_complex_tensor_types_ir10(),
                        "Constrain output types. Casting to complex is not supported.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          propagateElemTypeFromInputToOutput(ctx, 1, 0);
          if (hasNInputShapes(ctx, 1)) {
...
ONNX_OPERATOR_SET_SCHEMA(
    GridSample, 16,
    OpSchema()
        .Attr("mode", "Three interpolation modes: bilinear (default), nearest and bicubic.",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "An input tensor.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(1, "shape", "Specified shape for output.", "tensor(int64)", OpSchema::Single, true,
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "reshaped", "Reshaped data.", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir9(),
                        "Constrain input and output types to all tensor types.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          // Type inference
          propagateElemTypeFromInputToOutput(ctx, 0, 0);
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "shape", "Shape of the input tensor", "T1", OpSchema::Single, true, 1,
                OpSchema::NonDifferentiable)
        .Attr("start",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          int64_t rank =
              static_cast<int64_t>(ctx.getInputType(0)->tensor_type().shape().dim_size());
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "An input tensor.", "T", OpSchema::Single, true, 1,
               OpSchema::NonDifferentiable)
        .Output(0, "size", "Total number of elements of the input tensor", "T1", OpSchema::Single,
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          for (size_t i = 0; i < numInputs; i++) {
            ...
            if (shape.dim_size() != rank) {
              fail_shape_inference("All inputs to Concat must have same rank. Input ", i,
                                   " has rank ", shape.dim_size(), " != ", rank);
            }
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axis",
              ...
              "A negative value means counting dimensions from the back. Accepted range is [-rank, "
              "rank-1] "
              "where r = rank(input).",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("split", "length of each output. Values should be >= 0.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (getRepeatedAttribute(ctx, "split", split)) {
            if (split.size() != ctx.getNumOutputs()) {
              fail_shape_inference("Mismatch between number of splits (", split.size(),
                                   ") and outputs (", ctx.getNumOutputs(), ")");
            }
            int64_t total_dim = 0;
            for (int64_t d : split) {
              total_dim += d;
            ...
            if (total_dim != split_dim_value) {
              fail_shape_inference("Mismatch between the sum of 'split' (", total_dim,
                                   ") and the split dimension of the input (", split_dim_value,
                                   ")");
            }
          } else {
...
ONNX_OPERATOR_SET_SCHEMA(
    Split, 13,
    OpSchema()
        .Input(0, "input", "The tensor to split", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(1, "split",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "starts", "1-D tensor of starting indices of corresponding axis in `axes`",
               "Tind")
        .Input(2, "ends",
               "1-D tensor of ending indices (exclusive) of corresponding axis in `axes`", "Tind")
        .Input(3, "axes",
               "1-D tensor of axes that `starts` and `ends` apply to. Negative value means "
               "counting dimensions "
               "from the back. Accepted range is [-r, r-1] where r = rank(data).",
               "Tind", OpSchema::Optional)
        .Input(4, "steps",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          for (size_t axis_index = 0; axis_index < axes_size; ++axis_index) {
            auto axis = axes[axis_index] < 0 ? axes[axis_index] + static_cast<int64_t>(input_rank)
                                             : axes[axis_index];
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "An input tensor.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Output(0, "transposed", "Transposed output.", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir12(),
                        "Constrain input and output types to all tensor types.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(ScatterND, 16,
                         OpSchema()
                             ...
                             .Input(0, "data", "Tensor of rank r >= 1.", "T", OpSchema::Single,
                             ...
                             .Input(2, "updates", "Tensor of rank q + r - indices_shape[-1] - 1.",
                                    "T", OpSchema::Single, true, 1, OpSchema::Differentiable)
                             .Output(0, "output", "Tensor of rank r >= 1.", "T", OpSchema::Single,
                                     true, 1, OpSchema::Differentiable)
                             .TypeConstraint("T", OpSchema::all_tensor_types_ir4(),
...
ONNX_OPERATOR_SET_SCHEMA(ScatterND, 13,
                         OpSchema()
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("reduction",
              ...
              "'mul': reduction using the multiplication operation.",
              AttributeProto::STRING, std::string("none"))
        .Input(0, "data", "Tensor of rank r >= 1.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(1, "indices",
        ...
        .Output(0, "output", "Tensor of rank r >= 1 (same rank as input).", "T", OpSchema::Single,
                true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir4(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          int axis = static_cast<int>(getAttribute(ctx, "axis", 0));
          if (axis < -r || axis >= r) {
            fail_shape_inference("axis=", axis, " must be in [-rank, rank-1] (rank=", r, ") (7).");
          }
          if (axis < 0) {
            axis += r;
          }
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "Tensors with at least max(dims) dimensions.", "T", OpSchema::Single,
               true, 1, OpSchema::Differentiable)
        .Input(1, "axes",
               "1D tensor of integers indicating the dimensions to squeeze. Negative value means "
               "counting dimensions "
               "from the back. Accepted range is [-r, r-1] where r = rank(data).",
               "tensor(int64)", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(0, "squeezed", "Reshaped tensor with same data as input.", "T", OpSchema::Single,
                true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir12(),
          ...
          for (int i = 0; i < input_ndim; ++i) {
            if (axes_not_specified && input_shape.dim(i).dim_value() == 1) {
              ...
              if (input_shape.dim(i).has_dim_value() && input_shape.dim(i).dim_value() != 1) {
                fail_shape_inference("Dimension of input ", i, " must be 1 instead of ",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "data", "Original tensor", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(1, "axes",
        ...
        .Output(0, "expanded", "Reshaped tensor with same data as input.", "T", OpSchema::Single,
                true, 1, OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir12(),
                        "Constrain input and output types to all tensor types up to IRv12.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (nullptr != repeats_inputs && hasNInputShapes(ctx, 2)) {
            ...
            if (repeats_data.size() != static_cast<size_t>(input_rank)) {
              fail_shape_inference("'Repeats' input has incorrect number of values. "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("coordinate_transformation_mode",
              Resize_ver18_attr_coordinate_transformation_mode_doc, AttributeProto::STRING,
        ...
        .Attr("nearest_mode",
              ...
              AttributeProto::STRING, std::string("round_prefer_floor"))
        ...
        .Attr("keep_aspect_ratio_policy", Resize_ver18_attr_keep_aspect_ratio_policy_doc,
              AttributeProto::STRING, std::string("stretch"))
        ...
        .Input(1, "roi",
               ...
               "X or the length of axes, if provided. "
        ...
        .Input(2, "scales",
               ...
               "than 1,"
               " it's sampling down, otherwise, it's upsampling. The number of elements of "
               "'scales' should"
               " be the same as the rank of input 'X' or the length of 'axes', if provided. "
               "One of 'scales' and 'sizes' MUST be specified and it is an error if both are "
               "specified. If 'sizes' is needed, the user can use an empty string as the name of "
               "'scales' in this operator's input list.",
               "tensor(float)", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Input(3, "sizes",
               "Target size of the output tensor. Its interpretation depends on the "
               "'keep_aspect_ratio_policy' value."
               "The number of elements of 'sizes' should be the same as the"
               " rank of input 'X', or the length of 'axes', if provided. Only one of 'scales' and "
               "'sizes' can be specified. ",
               "tensor(int64)", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Output(0, "Y", "N-D tensor after resizing", "T1", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T1", OpSchema::all_tensor_types_ir4(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("exclude_outside",
              ...
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("coordinate_transformation_mode",
              Resize_ver13_attr_coordinate_transformation_mode_doc, AttributeProto::STRING,
              std::string("half_pixel"))
        .Attr("nearest_mode",
              "Four modes: round_prefer_floor (default, as known as round half down), "
        ...
        .Input(
            ...
            "The RoIs' coordinates are normalized in the coordinate system of the input image. It "
            "only takes effect when coordinate_transformation_mode is \"tf_crop_and_resize\"",
            "T2", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Input(2, "scales",
               "The scale array along each dimension. It takes value greater than 0. If it's less "
               "than 1,"
               " it's sampling down, otherwise, it's upsampling. The number of elements of "
               "'scales' should"
               " be the same as the rank of input 'X'. One of 'scales' and 'sizes' MUST be "
               "specified and it is an error if both are specified. If 'sizes' is needed, the user "
               "can use an empty string as the name of 'scales' in this operator's input list.",
               "tensor(float)", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
        .Input(3, "sizes",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("exclude_outside",
              ...
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("coordinate_transformation_mode",
              Resize_attr_coordinate_transformation_mode_ver11_doc, AttributeProto::STRING,
              std::string("half_pixel"))
        .Attr("nearest_mode",
              "Four modes: round_prefer_floor (default, as known as round half down), "
              "round_prefer_ceil (as known as round half up), floor, ceil. Only used by nearest "
        ...
        .Input(2, "scales",
               ...
               " it's sampling down, otherwise, it's upsampling. The number of elements of "
               "'scales' should"
               " be the same as the rank of input 'X'. If 'size' is needed, the user must set "
               "'scales' to an empty tensor.",
               "tensor(float)")
        .Input(3, "sizes",
               "The size of the output tensor. The number of elements of 'sizes' should be the "
               "same as the"
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output", "Tensor to copy input into.", "V", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint(
            "V",
            []() {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output", "Tensor to copy input into.", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir4(),
                        "Constrain input and output types to all tensor types.")
        .TypeAndShapeInferenceFunction(propagateShapeAndTypeFromFirstInput));
...
ONNX_OPERATOR_SET_SCHEMA(IsNaN, 9,
                         OpSchema()
...
ONNX_OPERATOR_SET_SCHEMA(NonZero, 9,
                         OpSchema()
...
ONNX_OPERATOR_SET_SCHEMA(Pad, 24,
...
ONNX_OPERATOR_SET_SCHEMA(Pad, 23,
                         OpSchema().FillUsing(PadDocGenerator(
                             kDoc_Pad_ver24,
                             "Supported modes: `constant`(default), `reflect`, `edge`, `wrap`",
...
ONNX_OPERATOR_SET_SCHEMA(Pad, 21,
                         OpSchema().FillUsing(PadDocGenerator(
                             kDoc_Pad_ver24,
                             "Supported modes: `constant`(default), `reflect`, `edge`, `wrap`",
...
ONNX_OPERATOR_SET_SCHEMA(Pad, 19,
                         OpSchema().FillUsing(PadDocGenerator(
                             kDoc_Pad_ver24,
                             "Supported modes: `constant`(default), `reflect`, `edge`, `wrap`")));
...
ONNX_OPERATOR_SET_SCHEMA(
    Pad, 11,
    OpSchema()
        .Attr("mode", "Supported modes: `constant`(default), `reflect`, `edge`",
        ...
        .Input(1, "pads",
               ...
               "negative) "
               "at the beginning and end of each axis. For 2D input tensor, it is the number of "
               "pixels. "
...
ONNX_OPERATOR_SET_SCHEMA(Concat, 1,
                         OpSchema()
                             .Attr("axis", "Which axis to concat on.  Default value is 1.",
                                   AttributeProto::INT, OPTIONAL_VALUE)
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .Attr("paddings",
              ...
              "`paddings` rank should be double of the input's rank. `paddings` format should be "
              "as follow "
              "[x1_begin, x2_begin...x1_end, x2_end,...], where xi_begin the number of pixels "
              "added at the beginning of axis `i` and xi_end, the number of pixels added at "
              "the end of axis `i`.",
              AttributeProto::INTS)
        .Attr("mode", "Three modes: constant(default), reflect, edge", AttributeProto::STRING,
              std::string("constant"))
        .Attr("value", "One float, indicates the value to be filled, default is 0",
...
ONNX_OPERATOR_SET_SCHEMA(Reshape, 1,
                         OpSchema()
                             ...
                             // old definition.
                             .Attr("consumed_inputs", "legacy optimization attribute.",
                                   AttributeProto::INTS, OPTIONAL_VALUE)
                             .Input(0, "data", "An input tensor.", "T")
                             .Output(0, "reshaped", "Reshaped data.", "T")
                             .TypeConstraint("T",
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("height_scale",
              ...
              AttributeProto::FLOAT)
        .Attr("mode", "Two interpolation modes: nearest(default), bilinear", AttributeProto::STRING,
              std::string("nearest"))
        .Input(0, "X", "4-D tensor, [N,C,H,W]", "T")
        .Output(0, "Y", "4-D tensor after resizing, [N,C,H,W]", "T")
        .TypeConstraint(
            "T",
            {"tensor(bool)", "tensor(int32)", "tensor(int64)", "tensor(float16)", "tensor(float)",
             "tensor(double)"},
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (output_shape->dim_size() > 0) {
            if (output_shape->dim_size() != input_shape.dim_size()) {
              fail_shape_inference("Ranks inferred (", input_shape.dim_size(),
          ...
          if (nullptr != scales) {
            ...
            if (scales->type() == AttributeProto_AttributeType_FLOATS) {
              const std::vector<float> scales_data(scales->floats().begin(),
                                                   scales->floats().end());
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("ends", "Ending indices (exclusive) of corresponding axis in axes`",
              AttributeProto::INTS)
        .Output(0, "output", "Sliced data tensor.", "T")
        .TypeConstraint("T", OpSchema::all_tensor_types(),
                        "Constrain input and output types to all tensor types.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (!getRepeatedAttribute(ctx, "starts", starts) ||
              !getRepeatedAttribute(ctx, "ends", ends) || starts.size() != ends.size()) {
          ...
          if (std::any_of(axes.begin(), axes.end(), is_negative)) {
            ...
            // Hence, they are officially not part of the spec, but some models/runtimes may use
            // them. So we perform simple rank inference in this case.
            for (size_t i = 0;
                 static_cast<int64_t>(i) < ctx.getInputType(0)->tensor_type().shape().dim_size();
                 ++i) {
              ctx.getOutputType(0)->mutable_tensor_type()->mutable_shape()->add_dim();
            }
            return;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(3, "axes", "1-D tensor of axes that `starts` and `ends` apply to.", "Tind",
               OpSchema::Optional)
        .Input(4, "steps",
               "1-D tensor of slice step of corresponding axis in `axes`. Default to 1. ", "Tind",
               OpSchema::Optional)
        .Output(0, "output", "Sliced data tensor.", "T")
        .TypeConstraint("T", OpSchema::all_tensor_types(),
                        "Constrain input and output types to all tensor types.")
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axis",
              ...
              AttributeProto::INT, static_cast<int64_t>(0))
        .Input(0, "data", "Tensor of rank r >= 1.", "T")
        .Input(1, "indices", "Tensor of int32/int64 indices, of r >= 1 (same rank as input).",
               "Tind")
        .Input(2, "updates", "Tensor of rank r >=1 (same rank and shape as indices)", "T")
        .Output(0, "output", "Tensor of rank r >= 1 (same rank as input).", "T")
        .TypeConstraint("T", OpSchema::all_tensor_types(),
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          int axis = static_cast<int>(getAttribute(ctx, "axis", 0));
          if (axis < -r || axis >= r) {
            fail_shape_inference("axis=", axis, " must be in [-rank, rank-1] (rank=", r, ") (8).");
          }
          if (axis < 0) {
            axis += r;
          }
...
ONNX_OPERATOR_SET_SCHEMA(
    Squeeze, 1,
    OpSchema()
        .Attr("axes", "List of non-negative integers, indicate the dimensions to squeeze.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .SetDoc(kDoc_Squeeze_ver11)
        .Input(0, "data", "Tensors with at least max(dims) dimensions.", "T")
        .Output(0, "squeezed", "Reshaped tensor with same data as input.", "T")
...
ONNX_OPERATOR_SET_SCHEMA(
    Unsqueeze, 1,
    OpSchema()
        .Attr("axes", "List of non-negative integers, indicate the dimensions to be inserted",
              AttributeProto::INTS)
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "indices",
               ...
               "T1")
        .Input(1, "depth",
               "Scalar or rank 1 tensor containing exactly one element, specifying the number of "
               "classes "
               "in one-hot tensor. This is also the size of the one-hot dimension (specified by "
               "'axis' attribute) "
               "added on in the output tensor. The values in the 'indices' input tensor are "
               "expected to be "
        ...
        .Output(0, "output",
                ...
                "rank(indices) + 1. "
                "The data type for the elements of the output tensor is the same as the type of "
                "input 'values' "
                "is used.",
                "T3")
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          for (int i = 0; i < input_shape.dim_size(); ++i) {
            ...
            if (ctx.getInputType(0)->tensor_type().shape().dim(i).has_dim_value()) {
              newdim->set_dim_value(checkedAdd(
                  ctx.getInputType(0)->tensor_type().shape().dim(i).dim_value(), total_pad));
            } else if (total_pad == 0) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "condition", "When True (nonzero), yield X, otherwise yield Y", "B",
               OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
        .Input(1, "X", "values selected at indices where condition is True", "T", OpSchema::Single,
               true, 1, OpSchema::Differentiable)
        .Input(2, "Y", "values selected at indices where condition is False", "T", OpSchema::Single,
               true, 1, OpSchema::Differentiable)
        .Output(0, "output",
                "Tensor of shape equal to the broadcasted shape of condition, X, and Y.", "T",
                OpSchema::Single, true, 1, OpSchema::Differentiable)
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "output", "Tensor after padding.", "T", OpSchema::Single, true, 1,
                OpSchema::Differentiable)
        .TypeConstraint("T", OpSchema::all_tensor_types_ir4(),
                        "Constrain input and output types to all tensor types.")
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(Pad, 18,
                         OpSchema().FillUsing(PadDocGenerator(
                             kDoc_Pad_ver18,
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axis",
              "(Optional) Axis along which to take slices. If not specified, "
              "input is flattened before elements being selected. Negative value means counting "
              "dimensions "
              "from the back. Accepted range is [-r, r-1] where r = rank(input).",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "input", "Tensor of rank r >= 1.", "T", OpSchema::Single, true, 1,
               OpSchema::Differentiable)
        .Input(
            1, "condition",
            "Rank 1 tensor of booleans to indicate which slices or data elements to be selected. "
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("axis",
              ...
              "from the back. Accepted range is [-r, r-1] where r = rank(input).",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "X", "A N-D input tensor that is to be processed.", "T", OpSchema::Single, true,
               1, OpSchema::NonDifferentiable)
        .Output(0, "Y",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          // graph-level shape inference should not propagate the shape downstream for empty
          // optional outputs.
