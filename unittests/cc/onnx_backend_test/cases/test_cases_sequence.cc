...
TEST(BackendTestCase, SequenceLengthCaseIsPresent) {
  ...
  EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceConstruct");
  EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceLength");
  ASSERT_EQ(graph.ref_output().size(), 1u);
  const ValueInfoProto &out_vi = graph.ref_output()[0];
...
TEST(BackendTestCase, SequenceEraseCasesAreRegistered) {
  ...
  for (const TestCase *tc : {default_case, pos1_case, neg_case}) {
    ...
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceErase");
    ASSERT_EQ(graph.ref_output().size(), 1u);
...
TEST(BackendTestCase, SequenceInsertCasesAreRegistered) {
  ...
  for (const TestCase *tc : {default_case, pos1_case, neg_case}) {
    ...
    ASSERT_GE(graph.ref_node().size(), 2u) << tc->name;
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceInsert");
    ASSERT_EQ(graph.ref_output().size(), 1u);
    EXPECT_TRUE(graph.ref_output()[0].ref_type().has_sequence_type()) << tc->name;
    ASSERT_EQ(tc->data_sets().size(), 1u);
...
TEST(BackendTestCase, SequenceAtCasesAreRegistered) {
  ...
  for (const TestCase *tc : {pos0_case, pos2_case, neg_case}) {
    ...
    ASSERT_GE(graph.ref_node().size(), 2u) << tc->name;
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceAt");
    ASSERT_EQ(graph.ref_output().size(), 1u);
...
TEST(BackendTestCase, SequenceMapCasesAreRegistered) {
  ...
  for (const TestCase *tc : {float_case, int64_case}) {
    ...
    ASSERT_GE(graph.ref_node().size(), 2u) << tc->name;
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceMap");
    ...
    EXPECT_EQ(map_node.ref_attribute()[0].ref_name(), "body");
  ...
  // identity_2_sequences: 2 SequenceConstruct + 1 SequenceMap → 2 output sequences.
  {
    ...
    EXPECT_EQ(g.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(g.ref_node()[1].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(g.ref_node()[2].ref_op_type(), "SequenceMap");
  ...
  // add_2_sequences: 2 SequenceConstruct + 1 SequenceMap → 1 output sequence.
  {
    ...
    EXPECT_EQ(body.ref_node()[0].ref_op_type(), "Add");
  ...
  // broadcast tensor as the second SequenceMap input.
  {
    ...
    EXPECT_EQ(g.ref_node()[0].ref_op_type(), "SequenceConstruct");
    EXPECT_EQ(g.ref_node()[1].ref_op_type(), "SequenceMap");
  ...
  // and 3 inputs of distinct rank-3 shapes → INT64[3, 3] output.
  {
    ...
    const GraphProto &body = map_node.ref_attribute()[0].ref_g();
    ASSERT_EQ(body.ref_node().size(), 1u);
    EXPECT_EQ(body.ref_node()[0].ref_op_type(), "Shape");
    ASSERT_EQ(g.ref_output().size(), 1u);
    EXPECT_TRUE(g.ref_output()[0].ref_type().has_sequence_type());
...
TEST(BackendTestCase, SequenceEmptyCasesAreRegistered) {
  ...
  for (const TestCase *tc : {default_case, int64_case}) {
    const GraphProto &graph = tc->model().ref_graph();
    ASSERT_EQ(graph.ref_node().size(), 2u) << tc->name;
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SequenceEmpty");
    EXPECT_EQ(graph.ref_node()[1].ref_op_type(), "SequenceLength");
...
TEST(BackendTestCase, SplitToSequenceCasesAreRegistered) {
  ...
  for (const TestCase *tc : {case1, case2, case_nokd}) {
    ...
    EXPECT_EQ(graph.ref_node()[0].ref_op_type(), "SplitToSequence") << tc->name;
    ASSERT_EQ(graph.ref_output().size(), 1u);
    EXPECT_TRUE(graph.ref_output()[0].ref_type().has_sequence_type()) << tc->name;
    ASSERT_EQ(tc->data_sets().size(), 1u);
  }
