...
ONNX_OPERATOR_SET_SCHEMA(StringConcat, 20,
                         OpSchema()
                             .Input(0, "X", "Tensor to prepend in concatenation", "T",
                                    OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
                             .Input(1, "Y", "Tensor to append in concatenation", "T",
                                    OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
                             .Output(0, "Z", "Concatenated string tensor", "T", OpSchema::Single,
...
ONNX_OPERATOR_SET_SCHEMA(
    RegexFullMatch, 20,
    OpSchema()
        .Input(0, "X", "Tensor with strings to match on.", "T1", OpSchema::Single, true, 1,
               OpSchema::NonDifferentiable)
        .Attr("pattern", "Regex pattern to match on. This must be valid RE2 syntax.",
...
ONNX_OPERATOR_SET_SCHEMA(
    StringSplit, 20,
    OpSchema()
        .Input(0, "X", "Tensor of strings to split.", "T1", OpSchema::Single, true, 1,
               OpSchema::NonDifferentiable)
        .Attr("delimiter",
        ...
        .Output(0, "Y",
                ...
                "T2", OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
        .Output(1, "Z", "The number of substrings generated for each input element.", "T3",
                OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          // We produce a string tensor per input element. Therefore we have one additional rank
          // with a runtime dependent number of elements. All except the final dimension of the
          // output shape can be inferred directly from the input.
          propagateElemTypeFromInputToOutput(ctx, 0, 0);
          propagateShapeFromInputToOutput(ctx, 0, 0);
          getOutputShape(ctx, 0)->add_dim();

          // The output tensor containing the number of substrings has identical shape to the input
          // but produces int32 results.
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(std::string("case_change_action"),
        ...
        .Attr(std::string("is_case_sensitive"),
              std::string("Boolean. Whether the identification of stop words in X is "
                          "case-sensitive. Default is false"),
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("stopwords", "List of stop words. If not set, no word would be removed from X.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("locale",
              "Environment dependent string that denotes the locale according to which output "
              "strings needs to be upper/lowercased."
