...
// IEEE-754 binary16 ↔ binary32 conversions are provided by
// ``onnx_kernels/kernels/tensor/cast_helper.h`` as ``kernel::FloatToFloat16Bits``
// / ``kernel::Float16BitsToFloat``.
...
Tensor FloatToFloat16Tensor(const std::string &name, const Tensor &f) {
  ...
  for (int64_t i = 0; i < n; ++i) {
    bits[static_cast<size_t>(i)] = kernel::FloatToFloat16Bits(src[i]);
  }
...
Tensor RoundToFloat16(const Tensor &f) {
  ...
  const float *src = f.AsFloat();
  for (int64_t i = 0; i < n; ++i) {
    rounded[static_cast<size_t>(i)] =
        kernel::Float16BitsToFloat(kernel::FloatToFloat16Bits(src[i]));
  }
