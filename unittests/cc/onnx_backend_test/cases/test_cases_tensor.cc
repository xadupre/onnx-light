...
TEST(BackendTestCase, AffineGridUpstreamCasesArePresent) {
  ...
  const std::vector<Expected> expected{
      ...
      {"test_affine_grid_2d_expanded", {2, 2, 3}, {4}, {2, 5, 6, 2}},
      {"test_affine_grid_2d_align_corners", {2, 2, 3}, {4}, {2, 5, 6, 2}},
      {"test_affine_grid_2d_align_corners_expanded", {2, 2, 3}, {4}, {2, 5, 6, 2}},
      {"test_affine_grid_3d", {2, 3, 4}, {5}, {2, 4, 5, 6, 3}},
      {"test_affine_grid_3d_expanded", {2, 3, 4}, {5}, {2, 4, 5, 6, 3}},
      {"test_affine_grid_3d_align_corners", {2, 3, 4}, {5}, {2, 4, 5, 6, 3}},
      {"test_affine_grid_3d_align_corners_expanded", {2, 3, 4}, {5}, {2, 4, 5, 6, 3}},
  };
...
TEST(BackendTestCase, TriluCasesRegistered) {
  const auto cases = CollectTestCases("Trilu");
  ...
  const std::vector<Expected> expected{
      {"test_cc_trilu_upper_default", {3, 3}, 1},    {"test_cc_trilu_lower", {3, 3}, 1},
      {"test_cc_trilu_upper_k_positive", {3, 4}, 2}, {"test_cc_trilu_lower_k_negative", {3, 3}, 2},
      {"test_cc_trilu_batched_upper", {2, 2, 2}, 1},
  };
...
TEST(BackendTestCase, TensorScatterCasesRegistered) {
  const auto cases = CollectTestCases("TensorScatter");

  struct Expected {
    const char *name;
    std::vector<int64_t> past_cache_shape;
    std::vector<int64_t> update_shape;
  };
  const std::vector<Expected> expected{
      {"test_cc_tensorscatter", {2, 1, 4, 5}, {2, 1, 1, 5}},
      {"test_cc_tensorscatter_circular", {2, 1, 4, 5}, {2, 1, 2, 5}},
      {"test_cc_tensorscatter_3d", {3, 4, 5}, {3, 2, 5}},
  };
  ...
  for (const Expected &exp : expected) {
    ...
    EXPECT_EQ(ds.inputs[1].shape, exp.update_shape);
    EXPECT_EQ(ds.outputs[0].shape, exp.past_cache_shape);
    EXPECT_EQ(ds.outputs[0].data_type, ds.inputs[0].data_type);
  }
}
