...
void KeepAspectRatioHelper(KeepAspectRatioPolicy policy, const TensorShapeProto &input_shape,
  ...
  double scale = policy == KeepAspectRatioPolicy::NOT_LARGER
  ...
  for (size_t i = 0; i < sizes_data.size(); i++) {
    ...
    sizes_data[i] = has_unknown_dim
                        ...
                        : static_cast<int64_t>(std::round(
                              scale * static_cast<double>(input_shape.dim(d).dim_value())));
  }
}
...
void gridSampleShapeInference(InferenceContext &ctx) {
  ...
  if (input_shape.dim_size() != grid_shape.dim_size()) {
    fail_shape_inference(
        ...
        "Got input tensor rank: ", input_shape.dim_size(), ". ",
        "Got grid tensor rank: ", grid_shape.dim_size(), ". ");
  }

  int const num_dims = input_shape.dim_size();

  if (num_dims < 3) {
    fail_shape_inference("The input tensor and grid tensor ranks must be >= 3. ",
                         "Got input tensor and grid tensor ranks: ", num_dims, ". ");
  }
  ...
  if (last_dim.has_dim_value() && (last_dim.dim_value() != num_dims - 2)) {
    fail_shape_inference(
        ...
        "Got grid tensor rank: ", num_dims,
        "Got the last dimension of the grid tensor: ", last_dim.dim_value(), ". ");
  }
...
void resizeShapeInferenceHelper(const TensorShapeProto &input_shape,
                                ...
                                TensorShapeProto *output_shape) {
  // Newer opsets reuse the frozen opset 7-10 helper; give this its own body
  // when they diverge.
...
static void resizeShapeInferenceVersioned(InferenceContext &ctx, int opset_version) {
  ...
  if (keep_aspect_ratio_policy_attr && keep_aspect_ratio_policy_attr->has_s()) {
    auto str = std::string(keep_aspect_ratio_policy_attr->s());
    if (str == "stretch") {
  ...
  if (hasScalesInput && keep_aspect_ratio_policy != KeepAspectRatioPolicy::STRETCH) {
    fail_shape_inference("Providing `scales` is incompatible with a `keep_aspect_ratio_policy` "
                         "other than \"stretch\".");
  }
  ...
  if (output_shape->dim_size() > 0) {
    if (output_shape->dim_size() != input_shape.dim_size()) {
      fail_shape_inference("Ranks inferred (", input_shape.dim_size(),
                           ") is not equal to the existing rank value (", output_shape->dim_size(),
                           ").");
    }
    ...
    if (scales->data_type() == TensorProto::FLOAT) {
      ...
      if (!axes.empty()) {
        ...
        if (scales_data.size() != axes.size()) {
          fail_shape_inference("Number of elements of input 'scales' (", scales_data.size(),
                               ") does not match the number of axes (", axes.size(), ").");
        }
...
void resizeShapeInferenceHelper_opset7_to_10(const TensorShapeProto &input_shape,
                                             const std::vector<float> &scales_data,
                                             TensorShapeProto *output_shape) {
  for (int i = 0; i < input_shape.dim_size(); ++i) {
    auto *dim = output_shape->mutable_dim(i);
    // If input_shape has dim_value, we calculate the scaled result
    // If input_shape doesn't have one, we leave it here
    if (input_shape.dim(i).has_dim_value()) {
      int64_t dim_value =
          static_cast<int64_t>(std::floor(static_cast<double>(input_shape.dim(i).dim_value()) *
                                          static_cast<double>(scales_data[i])));
      // If output_shape has dim_value, we validate the calculated result
      // If output_shape doesn't have one, we set it to the scaled result
      if (dim->has_dim_value()) {
        if (static_cast<int64_t>(dim->dim_value()) != dim_value) {
          fail_shape_inference("Dimension value inferred (", dim_value,
                               ") is not equal to the existing dim value (", dim->dim_value(),
                               ").");
        }
      } else {
...
std::function<void(OpSchema &)> PadDocGenerator(const char *description,
                                                const char *mode_description,
                                                std::vector<std::string> op_schema,
                                                std::string op_schema_description) {
  return [=, op_schema = std::move(op_schema),
          op_schema_description = std::move(op_schema_description)](OpSchema &schema) {
    schema.SetDoc(description);
    schema.Attr("mode", mode_description, AttributeProto::STRING, std::string("constant"));
    schema.Input(0, "data", "Input tensor.", "T", OpSchema::Single, true, 1,
    ...
    schema.Input(
        ...
        "`pads` should be a 1D tensor of shape [2 * num_axes] where `num_axes` refers to the "
        "number "
    ...
    schema.Input(
        ...
        "from the back. Accepted range is [-r, r-1] where r = rank(data). Behavior is undefined if "
        "an "
    ...
    schema.Output(0, "output", "Tensor after padding.", "T", OpSchema::Single, true, 1,
                  OpSchema::Differentiable);
    schema.TypeConstraint("T", op_schema, op_schema_description);
    schema.TypeConstraint("Tind", {"tensor(int32)", "tensor(int64)"},
    ...
    schema.TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
      ...
      if (nullptr != pads_initializer && !axes.empty()) {
        if (pads_initializer->dims_size() != 1 ||
          ...
          fail_shape_inference(
              "'pads' input must be a 1D (shape: [2 * num_axes]) tensor of type int64");
        }

        const auto pads_data = ParseData<int64_t>(pads_initializer);
        if (pads_data.size() != 2 * num_axes) {
          fail_shape_inference("Pads has incorrect number of values. Expected 2 * ", num_axes,
                               " values. Got ", pads_data.size(), " values.");
        }
