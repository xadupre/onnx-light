...
- Extended `tensor_to_numpy(steal=True)` to also transfer ownership of *borrowed* tensors by copying their bytes onto the regular heap (owned by the returned array through a capsule), and exposed `Tensor.has_borrowed_data()`. `ReferenceEvaluator` now routes borrowed graph outputs (e.g. a `Constant` output or an initializer referencing the model proto's `raw_data`) through this path, so the returned arrays own their bytes and stay valid after the model proto is released.
