...
void EmitReduceMeanCase(std::vector<TestCase> &registry, const auto &kernel,
...
void EmitReduceMeanDefaultAxesCase(std::vector<TestCase> &registry, const auto &kernel,
...
void RegisterReduceMeanCases(std::vector<TestCase> &registry, TestMode mode) {
  const auto kernel = MakeReferenceKernel<onnx_kernels::kernel::ReduceMean>(DefaultOpset(18));
