...
constexpr const char *kName = "kernel::Sqrt";
...
Sqrt::Sqrt(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}
...
KernelTuningKey Sqrt::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Sqrt", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Sqrt::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Sqrt", parameters, tuning_, kTuningAbi);
}
...
void Sqrt::operator()(const Tensor &x, Tensor &output) const {
...
void Sqrt::Run(RuntimeContext &rt) {
