...
:: (aidge_core and aidge_onnx) are available on the prefix path pointed at
:: by AIDGE_PREFIX (or CMAKE_PREFIX_PATH), the example is built with the
:: Aidge integration enabled; otherwise the example is built in the
:: onnx-light-only mode.
::
:: Usage (run from the repository root or from this directory):
::   examples\aidge_onnx_light\build.bat [install-prefix] [lib-build-dir] [example-build-dir]
...
if "%~1"=="" (
    set "INSTALL_PREFIX=%REPO_ROOT%\build\install-aidge-onnx-light"
) else (
    set "INSTALL_PREFIX=%~f1"
)

if "%~2"=="" (
    set "LIB_BUILD_DIR=%REPO_ROOT%\build\aidge-onnx-light-lib"
) else (
    set "LIB_BUILD_DIR=%~f2"
)

if "%~3"=="" (
    set "EXAMPLE_BUILD_DIR=%REPO_ROOT%\build\aidge-onnx-light-example"
) else (
...
if not "%AIDGE_PREFIX%"=="" (
    set "PREFIX_PATH=%PREFIX_PATH%;%AIDGE_PREFIX%"
)
...
echo.
echo Example binary:
echo   %EXAMPLE_BUILD_DIR%\%BUILD_TYPE%\aidge_onnx_light.exe
