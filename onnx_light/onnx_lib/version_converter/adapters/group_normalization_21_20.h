...
public:
  ...
  Node *make_int64_constant(const std::shared_ptr<Graph> &graph, Node *before,
  ...
  Value *cast_to(const std::shared_ptr<Graph> &graph, Node *before, Value *input,
                 TensorProto_DataType type) const {
  ...
  Node *adapt_group_normalization_21_20(const std::shared_ptr<Graph> &graph, Node *node) const {
    ...
    ONNX_ASSERTM(input_type == TensorProto_DataType_FLOAT16 ||
                     input_type == TensorProto_DataType_FLOAT ||
                     input_type == TensorProto_DataType_DOUBLE ||
                     input_type == TensorProto_DataType_BFLOAT16,
                 "GroupNormalization input type ", input_type,
                 " is not supported when converting from opset 21 to opset 20.")

    const int64_t stash_type = node->hasAttribute(kstash_type)
                                   ? node->i(kstash_type)
                                   : static_cast<int64_t>(TensorProto_DataType_FLOAT);
    ONNX_ASSERTM(stash_type == TensorProto_DataType_FLOAT16 ||
                     stash_type == TensorProto_DataType_FLOAT ||
                     stash_type == TensorProto_DataType_DOUBLE,
                 "GroupNormalization stash_type ", stash_type,
    ...
    const float epsilon =
        node->hasAttribute(kepsilon) ? static_cast<float>(node->f(kepsilon)) : 1e-5f;
    ...
    Value *one_stashed =
        cast_to(graph, node, one->output(), static_cast<TensorProto_DataType>(stash_type));
    ...
    Value *zero_stashed =
        cast_to(graph, node, zero->output(), static_cast<TensorProto_DataType>(stash_type));
    ...
    Value *normalized = cast_to(graph, node, normalized_shape->output(),
                                static_cast<TensorProto_DataType>(input_type));
