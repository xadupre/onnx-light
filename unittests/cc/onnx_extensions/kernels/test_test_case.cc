...
#include "onnx_extensions/onnx_backend_test/cases/nn/include_nn_cases.h"
...
#include "onnx_extensions/onnx_backend_test/cases/text/include_text_cases.h"
#include "onnx_extensions/onnx_backend_test/cases/traditionalml/include_traditionalml_cases.h"
#include "onnx_extensions/onnx_backend_test/cases/training/include_training_cases.h"
...
TEST(BackendTestCase, TensorTemplatedFromAndAs) {
  ...
  EXPECT_EQ(tf.data_type, static_cast<int32_t>(core::runtime::DataType::FLOAT));
  ...
  EXPECT_EQ(td.data_type, static_cast<int32_t>(core::runtime::DataType::DOUBLE));
  EXPECT_DOUBLE_EQ(td.As<double>()[2], 3.0);

  // Int32
  Tensor ti = Tensor::From<int32_t>("i", {2}, {-7, 8});
  EXPECT_EQ(ti.data_type, static_cast<int32_t>(core::runtime::DataType::INT32));
  EXPECT_EQ(ti.As<int32_t>()[0], -7);

  // Int64
  Tensor tl = Tensor::From<int64_t>("l", {1}, {1234567890123LL});
  EXPECT_EQ(tl.data_type, static_cast<int32_t>(core::runtime::DataType::INT64));
...
TEST(BackendTestCase, PerSubfolderCollectorsAggregateIntoMain) {
  ...
  std::vector<TestCase> shape_tag_only;
  onnx_backend_test::CollectShapeTagTestCases(shape_tag_only);
  EXPECT_FALSE(shape_tag_only.empty());

  std::vector<TestCase> peak_memory_only;
  onnx_backend_test::CollectPeakMemoryTestCases(peak_memory_only);
  EXPECT_FALSE(peak_memory_only.empty());
