...
void RegisterBatchNormalizationCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    RegisterLazyBenchmarkCase(
        registry, std::move(node), "test_cc_batchnorm_example_benchmark", {opset},
  ...
  // (using the default momentum = 0.9).
  {
    ...
    auto [y, output_mean, output_var] = batchnorm_kernel.TrainingForward(x, scale, bias, mean, var);

    Expect(node, {x, scale, bias, mean, var}, {y, output_mean, output_var},
           "test_cc_batchnorm_example_training_mode", {opset}, "backend-test", registry);
  }
  ...
  // case, the input is a deterministic ramp so the case stays reproducible.
  {
    ...
    Expect(node, {x, scale, bias, mean, var}, {y, output_mean, output_var},
           "test_cc_batchnorm_epsilon_training_mode", {opset}, "backend-test", registry);
  }
}
