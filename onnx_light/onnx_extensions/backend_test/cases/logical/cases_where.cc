...
template <typename T>
void RegisterWhereSignedZeroCase(std::vector<TestCase> &registry, const OpsetId &opset,
  ...
  Expect(registry, std::move(node), name, {opset},
         [opset, condition_values, x_shape, x_values, y_shape, y_values]() -> IoData {
...
void RegisterWhereCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    Expect(registry, std::move(node), "test_where_example_benchmark", {opset},
           {count, count, count}, {count}, [shape]() -> IoData {
             const OpsetId opset = DefaultOpset(16);
...

  {
    ...
    NodeProto node = MakeNode("Where", {"condition", "x", "y"}, {"output"});
    Expect(registry, std::move(node), "test_where_bcast", {opset}, {2, 6, 3}, {6}, []() -> IoData {
      const OpsetId opset = DefaultOpset(16);
  ...
  RegisterWhereSignedZeroCase<float>(registry, opset,
  ...
  RegisterWhereSignedZeroCase<double>(registry, opset,
