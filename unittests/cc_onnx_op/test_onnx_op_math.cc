...
TEST(OnnxOpMathRegistrationTest, ReturnsSchemasWithoutShapeInference) {
  ...
  const onnx_op::LightOpSchema *const cos_v7 = FindSchema(schemas, "Cos", 7);
  ...
  const onnx_op::LightOpSchema *const cosh_v9 = FindSchema(schemas, "Cosh", 9);
  ...
  EXPECT_EQ(sin_v7->inputs().size(), 1u);
  EXPECT_EQ(sin_v7->outputs().size(), 1u);
  EXPECT_EQ(sin_v7->inputs()[0].name, "input");
  EXPECT_EQ(sin_v7->outputs()[0].name, "output");
  ...
  EXPECT_EQ(cosh_v9->inputs()[0].description, "Input tensor");
