...
void ComputeShapeSoftmaxCrossEntropyLoss(ShapesContext &ctx, const NodeProto &node,
  ...
  const SymTensor &scores_tensor = ctx.Get(scores);
  const SymShape &scores_shape = scores_tensor.Shape();
  ...
  const SymTensor &labels_tensor = ctx.Get(labels);
  const SymShape &labels_shape = labels_tensor.Shape();
  ...
  if (weights != nullptr && weights[0] != '\0') {
    const SymTensor &weights_tensor = ctx.Get(weights);
  ...
  if (reduction == "none") {
    ...
    ctx.Set(node.output(0), SymTensor(nullptr, scores_tensor.Dtype(), labels_shape));
  } else {
    // Scalar output (rank 0).
    ctx.Set(node.output(0), SymTensor(nullptr, scores_tensor.Dtype(), SymShape()));
  }

  // Optional second output: log_prob has same shape and dtype as scores.
  if (node.output_size() >= 2) {
    const std::string log_prob_name = node.output(1);
    if (!log_prob_name.empty()) {
      ctx.Set(log_prob_name, SymTensor(nullptr, scores_tensor.Dtype(), scores_shape));
    }
  }
}
