...
void RegisterTreeEnsembleCases(std::vector<TestCase> &registry, TestMode mode) {
  ...
  if (mode == TestMode::BENCHMARK) {
    ...
    const auto tree_ens =
        MakeReferenceKernel<onnx_kernels::kernel::TreeEnsemble, std::vector<int64_t>,
                            std::vector<int64_t>, std::vector<double>, std::vector<uint8_t>,
                            std::vector<int64_t>, std::vector<int64_t>, std::vector<int64_t>,
                            std::vector<int64_t>, std::vector<int64_t>, std::vector<int64_t>,
                            std::vector<double>, std::vector<double>>(opset, /*tree_roots=*/{0},
                                                                      /*nodes_featureids=*/{0},
                                                                      /*nodes_splits=*/{0.5},
                                                                      /*nodes_modes=*/{0},
                                                                      /*nodes_truenodeids=*/{0},
    ...
    Expect(registry, std::move(node), "test_cc_treeensemble_single_tree_float_benchmark",
           {default_opset, opset}, {8192}, {8192}, [tree_ens]() -> IoData {
             ...
             Tensor y = tree_ens.Invoke([&](const auto &kernel) {
               return kernel.template operator()<float>(x, /*n_targets=*/1,
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    Expect(registry, std::move(node), "test_ai_onnx_ml_tree_ensemble_single_tree",
           {default_opset, opset}, [=]() -> IoData {
             ...
             Tensor y = tree_ens.Invoke([&](const auto &kernel) {
               return kernel.template operator()<double>(x, /*n_targets=*/2,
                                                         /*aggregate_function=*/1,
                                                         /*post_transform=*/0);
             });
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    Expect(registry, std::move(node), "test_ai_onnx_ml_tree_ensemble_set_membership",
           {default_opset, opset}, [=]() -> IoData {
             ...
             Tensor y = tree_ens.Invoke([&](const auto &kernel) {
               return kernel.template operator()<float>(x, /*n_targets=*/4,
                                                        /*aggregate_function=*/1,
                                                        /*post_transform=*/0);
             });
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    Expect(registry, std::move(node), "test_ai_onnx_ml_tree_ensemble_leaf_like",
           {default_opset, opset}, [=]() -> IoData {
             ...
             Tensor y = tree_ens.Invoke([&](const auto &kernel) {
               return kernel.template operator()<double>(x, /*n_targets=*/1,
                                                         /*aggregate_function=*/1,
                                                         /*post_transform=*/0);
             });
