...
constexpr const char *kName = "kernel::Cosh";
...
Cosh::Cosh(const KernelContext &ctx) : KernelBase(ctx), tuning_(kPortableParallelMinimum) {}

void Cosh::RegisterTuningSchemas() {
  tuning::RegisterParallelTuningSchemas("Cosh", kSupportedElementTypes, kPortableParallelMinimum,
                                        kTuningAbi);
}

KernelTuningKey Cosh::TuningKey(int32_t element_type) const {
  return tuning::IsSupportedElementType(element_type, kSupportedElementTypes)
             ? tuning::MakePortableTuningKey("Cosh", element_type, kTuningAbi)
             : KernelTuningKey{};
}

void Cosh::Configure(const KernelTuningParameters &parameters) {
  tuning::ConfigureParallelTuning("Cosh", parameters, tuning_, kTuningAbi);
}

Tensor Cosh::operator()(const Tensor &x, RuntimeContext *rt) const {
...
void Cosh::operator()(const Tensor &x, Tensor &output) const {
  ...
  case DataType::FLOAT: {
    ...
    ParallelFor(n, tuning_.parallel_minimum_elements, [px, py](int64_t begin, int64_t end) {
      for (int64_t i = begin; i < end; ++i) {
        py[static_cast<size_t>(i)] = std::cosh(px[i]);
      }
    });
    return;
  }
