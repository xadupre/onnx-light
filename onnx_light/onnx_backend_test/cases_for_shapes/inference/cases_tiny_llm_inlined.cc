...
// Tiny "Llama"-style configuration, identical to the high-level
// ``cases_tiny_llm.cc`` companion so the two graphs only differ by whether the
// ``Attention`` / ``RMSNormalization`` operators are expressed as fused ops or
// inlined as primitive subgraphs.
...
// ``cases_tiny_llm.cc`` but with the fused ``RMSNormalization`` and
// ``Attention`` operators **inlined** into their primitive subgraphs. This
// exercises shape inference through the longer chains a real exporter emits
// when those operators are decomposed:
//
...
//
// The case carries no ``DataSet``: it is a shape-inference-only model validated
// by the generic ``BackendTestCaseShapeInference`` gtest, which strips the
// declared ``value_info`` / output shapes and checks that
// :cpp:func:`shape_inference::InferShapes` recovers them.
// ---------------------------------------------------------------------------
