        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (seq_elem_type != tensor_elem_type) {
            fail_type_inference(
                ...
                seq_elem_type, " Tensor=", tensor_elem_type);
          }
...
ONNX_OPERATOR_SET_SCHEMA(SplitToSequence, 24,
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          for (int i = 0; i <= upper_bound; ++i) {
            ...
            if (i != axis) {
              output_shape->mutable_dim(i)->CopyFrom(
                  input_shape.dim((i > axis && new_axis) ? i - 1 : i));
            }
          }
        }));
...
static bool BuildSequenceMapBodyFunc(const FunctionBodyBuildContext &ctx, const OpSchema &schema,
  ...
  *functionProto.add_input() = input_0_name;
  ...
  std::string first_input_name = std::string(functionProto.input(0));
  ...
  for (int outputIndex = 0; outputIndex < noutputs; outputIndex++) {
    std::string output_name = std::string(functionProto.output(outputIndex));
  ...
  nodes.push_back(
      {loop_node_outputs, "Loop", loop_node_inputs, {MakeAttribute("body", loopbody_graph)}});
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input_sequence", "Input sequence.", "S")
        .Input(1, "additional_inputs", "Additional inputs to the graph", "V", OpSchema::Variadic,
               false, 0)
