...
def _format_example_attribute(attr: Any) -> str:
    ...
    if t == int(AttributeProto.STRINGS):
        ...
        return f"{name} = {[_to_text(s) for s in attr.strings]}"
...
def _examples_section_lines(schema: Any, domain: str) -> list[str]:
    ...
    for name in sorted(matches):
        ...
        if target_node is not None:
            ...
            input_list = ", ".join(_fmt_name(n) for n in target_node.input)
            output_list = ", ".join(_fmt_name(n) for n in target_node.output)
...
# ---------------------------------------------------------------------------
# Shape-inference coverage report (for the documentation page driving
# :ref:`l-design-inference-coverage`). Moved from
# ``onnx_light/backend/inference_coverage.py``.
# ---------------------------------------------------------------------------
