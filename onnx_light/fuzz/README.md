...
```bash
python -m onnx_light.fuzz.make_seed_corpus \
    /tmp/vc_seeds.zip \
...
visitor without needing separate fuzzers. Seed inputs generated by
`make_seed_corpus.py` append a trailing `0x00` toggle byte so they take
the raw-bytes path on the first iteration.
