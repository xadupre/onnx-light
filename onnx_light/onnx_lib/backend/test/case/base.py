...
def make_test_coverage(
    ...
    class BackendCoverageTest(ExtTestCase):
        """Represents a dynamically generated test class for backend metadata checks."""
    ...
    for name, test_case in filtered_tests.items():
        ...
        test_func.__name__ = f"test_{name}"
        test_func.__doc__ = f"Coverage test case: {name}"
        setattr(BackendCoverageTest, f"test_{name}", test_func)

    return BackendCoverageTest
