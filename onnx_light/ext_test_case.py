...
def requires_onnxmltools(version: str, msg: str = "") -> Callable:
    ...
    if pv.Version(onnxmltools.__version__) < pv.Version(version):
        msg = f"requires_onnxmltools version {onnxmltools.__version__} < {version}: {msg}"
        return unittest.skip(msg)
    return lambda x: x
...
def measure_time(
    ...
    .. runpython::
        ...
        from onnx_light.ext_test_case import measure_time
    ...
    *number* times the execution of the main statement.
    ...
    if max_time is not None:
        assert div_by_number, "div_by_number must be set to True of max_time is defined."
        i = 1
        total_time = 0.0
        results = []
        while True:
