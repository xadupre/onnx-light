...
class BlackmanWindow(Base):
    ...
    def export() -> None:
        ...
        y = (
            a0
            + a1 * np.cos(2 * np.pi * np.arange(0, size, 1, dtype=np.float32) / size)
            + a2 * np.cos(4 * np.pi * np.arange(0, size, 1, dtype=np.float32) / size)
        )
        ...
        y = (
            a0
            + a1 * np.cos(2 * np.pi * np.arange(0, size, 1, dtype=np.float32) / (size - 1))
            + a2 * np.cos(4 * np.pi * np.arange(0, size, 1, dtype=np.float32) / (size - 1))
        )
        expect(
            node,
            inputs=[size],
