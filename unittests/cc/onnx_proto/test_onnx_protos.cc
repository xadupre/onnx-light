...
// ---------------------------------------------------------------------------
// ByteSpan owned-string container: value() / mutable_value() semantics
// ---------------------------------------------------------------------------

TEST(onnx_proto, ByteSpan_ValueReturnsOwnedStringWithoutCopy) {
  // In plain owned mode value() must return the underlying std::string directly,
  // so no copy is made and the reference is stable across calls.
  utils::ByteSpan span(std::vector<uint8_t>{'a', 'b', 'c', 'd'});
  const std::string &first = span.value();
  const std::string &second = span.value();
  EXPECT_EQ(first, "abcd");
  EXPECT_EQ(&first, &second); // same object: no per-call copy
  // The returned string aliases the owned buffer itself.
  EXPECT_EQ(reinterpret_cast<const void *>(first.data()),
            reinterpret_cast<const void *>(span.data()));
}

TEST(onnx_proto, ByteSpan_MutableValueMutatesOwnedInPlace) {
  // mutable_value() returns a pointer to the real owned string; it does not raise
  // for owned data and mutations are written directly into the ByteSpan.
  utils::ByteSpan span;
  span.assign("hello", 5);
  std::string *mv = span.mutable_value();
  ASSERT_NE(mv, nullptr);
  (*mv)[0] = 'H';
  mv->push_back('!');
  EXPECT_EQ(span.size(), 6u);
  EXPECT_EQ(std::memcmp(span.data(), "Hello!", 6), 0);
  // value() reflects the mutation without any extra sync step.
  EXPECT_EQ(span.value(), "Hello!");
}

TEST(onnx_proto, ByteSpan_MutableValueMaterializesAlignedOwned) {
  // Aligned-owned data is owned, so mutable_value() must not raise; it materializes
  // the bytes into the plain owned string and drops the alignment.
  utils::ByteSpan span;
  span.resize_aligned(5, 64);
  for (int i = 0; i < 5; ++i)
    span.data()[i] = static_cast<uint8_t>(i + 1);
  ASSERT_TRUE(span.is_aligned_owned());
  std::string *mv = span.mutable_value();
  ASSERT_NE(mv, nullptr);
  EXPECT_FALSE(span.is_aligned_owned());
  EXPECT_EQ(mv->size(), 5u);
  EXPECT_EQ(static_cast<uint8_t>((*mv)[4]), uint8_t{5});
}

TEST(onnx_proto, ByteSpan_MutableValueRaisesForBorrowed) {
  // Borrowed data is not owned; mutable_value() must still raise.
  std::vector<uint8_t> buf = {1, 2, 3};
  utils::ByteSpan span;
  span.assign_borrowed(buf.data(), buf.size());
  ASSERT_TRUE(span.is_borrowed());
  EXPECT_ANY_THROW(span.mutable_value());
}

TEST(onnx_proto, ByteSpan_AlignedOwnedMovePreservesAlignedPointer) {
  // Because owned bytes now live in a std::string (which may relocate on move),
  // the aligned pointer must be recomputed so moved-from data stays aligned & intact.
  utils::ByteSpan src;
  src.resize_aligned(9, 64);
  for (int i = 0; i < 9; ++i)
    src.data()[i] = static_cast<uint8_t>(i);
  utils::ByteSpan dst(std::move(src));
  ASSERT_TRUE(dst.is_aligned_owned());
  EXPECT_EQ(reinterpret_cast<uintptr_t>(dst.data()) % 64u, 0u);
  for (int i = 0; i < 9; ++i)
    EXPECT_EQ(dst.data()[i], static_cast<uint8_t>(i));
}
