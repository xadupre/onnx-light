...
constexpr const char *kAttentionDocOpset25 = R"DOC(
...
// LightOpSchema parity test (which compares ``description()`` byte-for-byte
// against ``OpSchema::doc()``) happy.
constexpr const char *kRotaryEmbeddingDocOpset23 = R"DOC(
...
std::string MakeRotaryEmbeddingDoc(int since_version) {
  switch (since_version) {
  case 23:
    return kRotaryEmbeddingDocOpset23;
  default:
    return "";
  }
}
...
constexpr const char *kLayerNormalizationDocOpset17 = R"DOC(
...
std::string MakeLayerNormalizationDoc(int since_version) {
  switch (since_version) {
  case 17:
    return kLayerNormalizationDocOpset17;
  default:
    return "";
  }
}
