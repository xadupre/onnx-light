...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "Y", "The indices, based on 0 as the first index of any dimension.",
               "tensor(int64)")
              ...
              for (int i = 0; i < indices_shape.dim_size(); i++) {
                if (indices_shape.dim(i).has_dim_value()) {
                  ...
                  if (single_symbolic_dim.empty()) {
                    ...
                    // value 1
                    single_symbolic_dim = std::string(indices_shape.dim(i).dim_param());
                  } else {
                    return;
                  }
                } else {
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"tensor(float)", "tensor(double)", "tensor(int64)", "tensor(int32)"},
                        "The input must be a tensor of a numeric type. The output will be of the "
                        "same tensor type.")
        .Attr("threshold", "Values greater than this are mapped to 1, others to 0.",
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "Y",
                "A tensor representing the same data as the input map, ordered by their keys", "T2")
        .TypeConstraint("T1", {"map(int64, string)", "map(int64, float)"},
                        "The input must be an integer map to either string or float.")
        .TypeConstraint("T2", {"tensor(string)", "tensor(float)", "tensor(int64)"},
        ...
        .Attr("cast_to",
              ...
              "'TO_FLOAT', 'TO_STRING', "
              "'TO_INT64'.",
              AttributeProto::STRING, std::string("TO_FLOAT"))
        .Attr("map_form",
              "Indicates whether to only output as many values as are in the input (dense), or "
              "position the input based "
              "on using the key of the map as the index of the output (sparse).<br>One of 'DENSE', "
              "'SPARSE'.",
              AttributeProto::STRING, std::string("DENSE"))
        .Attr("max_map",
              "If the value of map_form is 'SPARSE,' this attribute indicates the total length of "
              "the output tensor.",
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(
            ...
            "T2")
        .TypeConstraint("T1", {"tensor(string)", "tensor(int64)"},
                        "The input must be a tensor of strings or integers, either [N,C] or [C].")
        .TypeConstraint("T2", {"tensor(string)", "tensor(int64)"},
        ...
        .Attr("default_string",
              "A string to use when an input integer value is not found in the map.<br>One and "
              "only one of the "
              "'default_*' attributes must be defined.",
              AttributeProto::STRING, std::string("_Unused"))
        .Attr("default_int64",
              "An integer to use when an input string value is not found in the map.<br>One and "
              "only one of the "
              "'default_*' attributes must be defined.",
              AttributeProto::INT, static_cast<int64_t>(-1))
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (cats_int64s->ints_size() != cats_strings->strings_size()) {
            fail_shape_inference(
                "Attributes 'cats_int64s' and 'cats_strings' are required to be the same length.");
          }
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T1",
                        {"map(string, int64)", "map(int64, string)", "map(int64, float)",
                         "map(int64, double)", "map(string, float)", "map(string, double)"},
                        "The input must be a map from strings or integers to either strings or a "
                        "numeric type. The key and value "
        ...
        .TypeConstraint("T2",
                        ...
                        "shape will be [1,C], where C is the "
                        "length of the input dictionary.")
        .Attr("string_vocabulary",
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          auto input_elem_type =
              ctx.getInputType(0)->map_type().value_type().tensor_type().elem_type();
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X", "An ordered collection of tensors, all with the same element type.", "T1",
               OpSchema::Variadic)
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"tensor(float)", "tensor(double)", "tensor(int64)", "tensor(int32)"},
                        ...
                        "The output type will be of the "
                        "same tensor type and shape.")
        .Attr("imputed_value_floats", "Value(s) to change to", AttributeProto::FLOATS,
              OPTIONAL_VALUE)
        .Attr("replaced_value_float", "A value that needs replacing.", AttributeProto::FLOAT, 0.f)
        .Attr("imputed_value_int64s", "Value(s) to change to.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("replaced_value_int64", "A value that needs replacing.", AttributeProto::INT,
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               "Input data. It must have the same element type as the keys_* attribute set.", "T1")
        .Output(0, "Y",
                "Output data. This tensor's element type is based on the values_* attribute set.",
                "T2")
        .TypeConstraint("T1",
                        {"tensor(string)", "tensor(int64)", "tensor(float)", "tensor(int32)",
                         "tensor(int16)", "tensor(double)"},
        ...
        .Attr("keys_tensor",
              ...
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        ...
        .Attr("default_tensor",
              "A default tensor. {\"_Unused\"} if values_* has string type, {-1} if values_* has "
              "integral type, and "
              "{-0.f} if values_* has float type.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          auto [key_type, key_length] = getAttributeElementTypeAndLength(
              ctx, {"keys_tensor", "keys_strings", "keys_int64s", "keys_floats"});
          if (key_type == TensorProto::UNDEFINED) {
            fail_shape_inference(
                "At least one of keys_tensor, keys_strings, keys_int64s, keys_floats must be set.");
          }
          if (key_type != ctx.getInputType(0)->tensor_type().elem_type()) {
            fail_shape_inference("The input type was ",
                                 ctx.getInputType(0)->tensor_type().elem_type(),
                                 " and the key type ", key_type,
          ...
          if (value_type == TensorProto::UNDEFINED) {
            fail_shape_inference("At least one of values_tensor, values_strings, values_int64s, "
                                 "values_floats must be set.");
          }
          if (value_length != key_length) {
            fail_shape_inference("The number of keys ", key_length, " and the number of values ",
                                 value_length, " must be the same in the LabelEncoder.");
          }

          auto default_attr = ctx.getAttribute("default_tensor");
          if (nullptr != default_attr && default_attr->has_t() &&
              default_attr->t().has_data_type() &&
            ...
            if (default_tensor.data_type() != value_type) {
              fail_shape_inference("The default tensor type ", default_tensor.data_type(),
                                   " and the value type ", value_type,
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(1, "Z", "Classification scores ([N,E] - one score for each class and example",
                "tensor(float)")
        .TypeConstraint("T1", {"tensor(float)", "tensor(double)", "tensor(int64)", "tensor(int32)"},
        ...
        .Attr("multi_class", "Indicates whether to do OvR or multinomial (0=OvR is the default).",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("classlabels_strings",
              "Class labels when using string labels. One and only one 'classlabels' attribute "
              "must be defined.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("classlabels_ints",
              "Class labels when using integer labels. One and only one 'classlabels' attribute "
              "must be defined.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("post_transform",
              "Indicates the transform to apply to the scores vector.<br>One of 'NONE,' 'SOFTMAX,' "
              "'LOGISTIC,' "
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (intercepts.size() == 1 && ((using_strings && label_strs.size() == 2) ||
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("post_transform",
              "Indicates the transform to apply to the regression output vector.<br>One of 'NONE,' "
              "'SOFTMAX,' "
        ...
        .Attr("intercepts", "Weights of the intercepts, if used.", AttributeProto::FLOATS,
              OPTIONAL_VALUE)
        .Attr("targets", "The total number of regression targets, 1 if not defined.",
              AttributeProto::INT, static_cast<int64_t>(1)));
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T",
                        {"tensor(string)", "tensor(int64)", "tensor(int32)", "tensor(float)",
                         "tensor(double)"},
                        "The input must be a tensor of a numeric type.")
        .Attr("cats_int64s",
              "List of categories, ints.<br>One and only one of the 'cats_*' attributes must be "
              "defined.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("cats_strings",
              "List of categories, strings.<br>One and only one of the 'cats_*' attributes must be "
              "defined.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("zeros",
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"tensor(float)", "tensor(double)", "tensor(int64)", "tensor(int32)"},
                        "The input must be a tensor of a numeric type.")
        .Attr("offset",
              "First, offset by this.<br>Can be length of features in an [N,F] tensor or length 1, "
              "in which case it "
              "applies to all features, regardless of dimension count.",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("scale",
              "Second, multiply by this.<br>Can be length of features in an [N,F] tensor or length "
              "1, in which case it "
              "applies to all features, regardless of dimension count.<br>Must be same length as "
              "'offset'",
              AttributeProto::FLOATS, OPTIONAL_VALUE));
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(0, "Y", "Classification outputs (one class per example).", "T2")
        .Output(1, "Z",
                "Class scores (one per class per example), if prob_a and prob_b are provided they "
                "are probabilities for "
        ...
        .TypeConstraint("T2", {"tensor(string)", "tensor(int64)"},
                        ...
                        "which of the classlabels_* "
                        "attributes is used. Its size will match the batch size of the input.")
        .Attr("kernel_type", "The kernel type, one of 'LINEAR,' 'POLY,' 'RBF,' 'SIGMOID'.",
              AttributeProto::STRING, std::string("LINEAR"))
        .Attr("kernel_params",
              "List of 3 elements containing gamma, coef0, and degree, in that order. Zero if "
              "unused for the kernel.",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        ...
        .Attr("prob_b",
              ...
              "prob_a.<br>If these are provided "
              "then output Z are probability estimates, otherwise they are raw scores.",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("rho", "", AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("post_transform",
              "Indicates the transform to apply to the score. <br>One of 'NONE,' 'SOFTMAX,' "
              "'LOGISTIC,' 'SOFTMAX_ZERO,' "
              "or 'PROBIT'",
              AttributeProto::STRING, std::string("NONE"))
        .Attr("classlabels_strings",
              "Class labels if using string labels.<br>One and only one of the 'classlabels_*' "
              "attributes must be "
              "defined.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("classlabels_ints",
              "Class labels if using integer labels.<br>One and only one of the 'classlabels_*' "
              "attributes must be "
              "defined.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("one_class", "Flag indicating whether the regression is a one-class SVM or not.",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("coefficients", "Support vector coefficients.", AttributeProto::FLOATS,
              OPTIONAL_VALUE)
        .Attr("n_supports", "The number of support vectors.", AttributeProto::INT,
              static_cast<int64_t>(0))
        .Attr("post_transform",
              "Indicates the transform to apply to the score. <br>One of 'NONE,' 'SOFTMAX,' "
              "'LOGISTIC,' 'SOFTMAX_ZERO,' "
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Output(1, "Z", "The class score for each class, for each point, a tensor of shape [N,E].",
                "tensor(float)")
        .TypeConstraint("T1", {"tensor(float)", "tensor(double)", "tensor(int64)", "tensor(int32)"},
                        "The input type must be a tensor of a numeric type.")
        .TypeConstraint("T2", {"tensor(string)", "tensor(int64)"},
        ...
        .Attr("nodes_values", "Thresholds to do the splitting on for each node.",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("nodes_values_as_tensor", "Thresholds to do the splitting on for each node.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .Attr("nodes_hitrates", "Popularity of each node, used for performance and may be omitted.",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("nodes_hitrates_as_tensor",
              "Popularity of each node, used for performance and may be omitted.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .Attr("nodes_modes",
              "The node kind, that is, the comparison to make at the node. There is no comparison "
              "to make at a leaf "
              "node.<br>One of 'BRANCH_LEQ', 'BRANCH_LT', 'BRANCH_GTE', 'BRANCH_GT', 'BRANCH_EQ', "
              "'BRANCH_NEQ', 'LEAF'",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("nodes_truenodeids", "Child node if expression is true.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("nodes_falsenodeids", "Child node if expression is false.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("nodes_missing_value_tracks_true",
              "For each node, define what to do in the presence of a missing value: if a value is "
              "missing (NaN), use the "
              "'true' or 'false' branch based on the value in this array.<br>This attribute may be "
              "left undefined, and "
              "the default value is false (0) for all nodes.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("class_treeids", "The id of the tree that this node is in.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("class_nodeids", "node id that this weight is for.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("class_ids", "The index of the class list that each weight is for.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("class_weights", "The weight for the class in class_id.", AttributeProto::FLOATS,
              OPTIONAL_VALUE)
        .Attr("class_weights_as_tensor", "The weight for the class in class_id.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .Attr("classlabels_strings",
              "Class labels if using string labels.<br>One and only one of the 'classlabels_*' "
              "attributes must be "
        ...
        .Attr("post_transform",
              ...
              AttributeProto::STRING, std::string("NONE"))
        .Attr("base_values",
              "Base values for classification, added to final class score; the size must be the "
              "same as the classes or "
              "can be left unassigned (assumed 0)",
              AttributeProto::FLOATS, OPTIONAL_VALUE)
        .Attr("base_values_as_tensor",
              "Base values for classification, added to final class score; the size must be the "
              "same as the classes or "
              "can be left unassigned (assumed 0)",
              AttributeProto::TENSOR, OPTIONAL_VALUE));
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("nodes_falsenodeids", "Child node if expression is false", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("nodes_missing_value_tracks_true",
              "For each node, define what to do in the presence of a NaN: use the 'true' (if the "
              "attribute value is 1) "
              "or 'false' (if the attribute value is 0) branch based on the value in this "
              "array.<br>This attribute may "
              "be left undefined and the default value is false (0) for all nodes.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("target_treeids", "The id of the tree that each node is in.", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("target_nodeids", "The node id of each weight", AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("target_ids", "The index of the target that each weight is for", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("target_weights", "The weight for each target", AttributeProto::FLOATS,
              OPTIONAL_VALUE)
        .Attr("target_weights_as_tensor", "The weight for each target", AttributeProto::TENSOR,
              OPTIONAL_VALUE)
        ...
        .Attr("aggregate_function",
              "Defines how to aggregate leaf values within a target. <br>One of 'AVERAGE,' 'SUM,' "
              "'MIN,' 'MAX.'",
              AttributeProto::STRING, std::string("SUM"))
        .Attr("base_values",
              "Base values for regression, added to final prediction after applying "
              "aggregate_function; the size must be "
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("nodes_splits",
              ...
              "'BRANCH_MEMBER'.",
              AttributeProto::TENSOR, true)
        .Attr("nodes_hitrates", "Popularity of each node, used for performance and may be omitted.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .Attr("nodes_modes",
              "The comparison operation performed by the node. This is encoded as an enumeration "
              "of 0 ('BRANCH_LEQ'), 1 "
              "('BRANCH_LT'), 2 ('BRANCH_GTE'), 3 ('BRANCH_GT'), 4 ('BRANCH_EQ'), 5 "
              "('BRANCH_NEQ'), and 6 "
              "('BRANCH_MEMBER'). Note this is a tensor of type uint8.",
              AttributeProto::TENSOR, true)
        .Attr("nodes_truenodeids",
              "If `nodes_trueleafs` is false at an entry, this represents the position of the true "
              "branch node. This "
              "position can be used to index into a `nodes_*` entry. If `nodes_trueleafs` is "
              "false, it is an index into "
              "the leaf_* attributes.",
              AttributeProto::INTS, true)
        .Attr("nodes_falsenodeids",
              "If `nodes_falseleafs` is false at an entry, this represents the position of the "
              "false branch node. This "
              "position can be used to index into a `nodes_*` entry. If `nodes_falseleafs` is "
              "false, it is an index into "
              "the leaf_* attributes.",
              AttributeProto::INTS, true)
        .Attr("nodes_trueleafs",
              "1 if true branch is leaf for each node and 0 an interior node. To represent a tree "
              "that is a leaf (only "
              "has one node), one can do so by having a single `nodes_*` entry with true and false "
              "branches referencing "
              "the same `leaf_*` entry",
              AttributeProto::INTS, true)
        .Attr("nodes_falseleafs",
              "1 if false branch is leaf for each node and 0 if an interior node. To represent a "
              "tree that is a leaf "
              "(only has one node), one can do so by having a single `nodes_*` entry with true and "
              "false branches "
              "referencing the same `leaf_*` entry",
              AttributeProto::INTS, true)
        .Attr("nodes_missing_value_tracks_true",
              "For each node, define whether to follow the true branch (if attribute value is 1) "
              "or false branch (if "
              "attribute value is 0) in the presence of a NaN input feature. This attribute may be "
              "left undefined and "
              "the default value is false (0) for all nodes.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("tree_roots",
              "Index into `nodes_*` for the root of each tree. The tree structure is derived from "
              "the branching of each "
              "node.",
              AttributeProto::INTS, true)
        .Attr("membership_values",
              "Members to test membership of for each set membership node. List all of the members "
              "to test again in the "
              "order that the 'BRANCH_MEMBER' mode appears in `node_modes`, delimited by `NaN`s. "
              "Will have the same "
              "number "
              "of sets of values as nodes with mode 'BRANCH_MEMBER'. This may be omitted if the "
              "node doesn't contain any "
              "'BRANCH_MEMBER' nodes.",
              AttributeProto::TENSOR, OPTIONAL_VALUE)
        .Attr("leaf_targetids",
              "The index of the target that this leaf contributes to (this must be in range `[0, "
              "n_targets)`).",
              AttributeProto::INTS, true)
        .Attr("leaf_weights", "The weight for each leaf.", AttributeProto::TENSOR, true)
        .Attr("n_targets", "The total number of targets.", AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("post_transform",
              "Indicates the transform to apply to the score. <br>One of 'NONE' (0), 'SOFTMAX' "
              "(1), 'LOGISTIC' (2), "
              "'SOFTMAX_ZERO' (3) or 'PROBIT' (4), defaults to 'NONE' (0)",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("aggregate_function",
              "Defines how to aggregate leaf values within a target. <br>One of 'AVERAGE' (0) "
              "'SUM' (1) 'MIN' (2) 'MAX "
              "(3) defaults to 'SUM' (1)",
              AttributeProto::INT, static_cast<int64_t>(1))
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          if (input_type != nodes_splits->t().data_type()) {
            fail_shape_inference(
                "Attribute 'nodes_splits' must have same type as input. Input type is ", input_type,
                " and attribute type is ", nodes_splits->t().data_type());
          }
          ...
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_featureids"), expected_length,
                                            TensorProto_DataType_INT64, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_hitrates"), expected_length,
                                            TensorProto_DataType_FLOAT, false);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_modes"), expected_length,
                                            TensorProto_DataType_UINT8, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_truenodeids"), expected_length,
                                            TensorProto_DataType_INT64, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_falsenodeids"), expected_length,
                                            TensorProto_DataType_INT64, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_trueleafs"), expected_length,
                                            TensorProto_DataType_INT64, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_falseleafs"), expected_length,
                                            TensorProto_DataType_INT64, true);
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_missing_value_tracks_true"),
                                            expected_length, TensorProto_DataType_INT64, false);
          ...
          if (nullptr != membership_values && membership_values->t().data_type() != input_type) {
            fail_shape_inference(
                ...
                input_type, " and attribute type is ", membership_values->t().data_type());
          }
          AssertAttributeProtoTypeAndLength(ctx.getAttribute("nodes_splits"), expected_length,
                                            static_cast<TensorProto::DataType>(input_type), true);
          ...
          if (nullptr != leaf_targetids && nullptr != leaf_weights) {
            if (leaf_targetids->ints_size() != leaf_weights->t().dims(0)) {
              fail_shape_inference("Attribute 'leaf_targetids' must have same length as attribute "
                                   ...
                                   "length is ",
                                   leaf_targetids->ints_size(), " and 'leaf_weights' length is ",
                                   leaf_weights->t().dims(0));
            }
          } else {
            fail_shape_inference(
                "Attributes 'leaf_targetids' and 'leaf_weights' must both be set.");
          }
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"seq(map(string, float))", "seq(map(int64, float))"},
                        "The output will be a sequence of string or integer maps to float.")
        .Attr("classlabels_strings",
              "The keys when using string keys.<br>One and only one of the 'classlabels_*' "
              "attributes must be defined.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("classlabels_int64s",
              "The keys when using int keys.<br>One and only one of the 'classlabels_*' attributes "
              "must be defined.",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .TypeAndShapeInferenceFunction([](InferenceContext &ctx) {
          ...
          auto output_map_type = ctx.getOutputType(0)
                                     ...
                                     ->mutable_elem_type()
                                     ->mutable_map_type();
          auto output_value_tensor_type =
              output_map_type->mutable_value_type()->mutable_tensor_type();
          output_value_tensor_type->set_elem_type(TensorProto::FLOAT);
          output_value_tensor_type->mutable_shape(); // Initialize to scalar
          if (hasInputShape(ctx, 0) && getInputShape(ctx, 0).dim_size() != 1 &&
              getInputShape(ctx, 0).dim_size() != 2) {
