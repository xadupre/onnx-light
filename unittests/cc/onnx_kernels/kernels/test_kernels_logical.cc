...
using onnx_kernels::kernel::BitwiseNot;
using onnx_kernels::kernel::BitwiseOr;
using onnx_kernels::kernel::BitwiseXor;
using onnx_kernels::kernel::Equal;
using onnx_kernels::kernel::Greater;
using onnx_kernels::kernel::GreaterOrEqual;
using onnx_kernels::kernel::IsInf;
...
TEST(KernelClass, AndClassMatchesReference) {
  ...
  Tensor y("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 1, 0, 0});
  Tensor z = and_kernel(nullptr, x, y);
...
TEST(KernelClass, AndClassBroadcastsScalar) {
  ...
  And and_kernel{ctx};
  Tensor x("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 0, 1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {}, {1});
  Tensor z = and_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data[0], 1);
  EXPECT_EQ(z.data[1], 0);
...
TEST(KernelClass, OrClassMatchesReference) {
  ...
  Tensor x("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 0, 1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 1, 0, 0});
  Tensor z = or_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
  EXPECT_EQ(z.data[0], 1);
...
TEST(KernelClass, XorClassMatchesReference) {
  ...
  Tensor x("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 0, 1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 1, 0, 0});
  Tensor z = xor_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
...
TEST(KernelClass, LogicalRejectsNonBoolTensors) {
  ...
  Tensor y("", onnx_kernels::DataType::BOOL, {2}, {1, 0});
  EXPECT_THROW((void)and_kernel(nullptr, x, y), std::invalid_argument);
}
...
TEST(KernelClass, AndInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor x("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 0, 1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 1, 0, 0});
  Tensor z("", onnx_kernels::DataType::BOOL, {2, 2}, std::vector<uint8_t>(4, 9));
  and_kernel(x, y, z);
  EXPECT_EQ(z.data[0], 1);
  EXPECT_EQ(z.data[1], 0);
  EXPECT_EQ(z.data[2], 0);
...
TEST(KernelClass, OrInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor x("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 0, 1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {2, 2}, {1, 1, 0, 0});
  Tensor z("", onnx_kernels::DataType::BOOL, {2, 2}, std::vector<uint8_t>(4));
  or_kernel(x, y, z);
  EXPECT_EQ(z.data[0], 1);
  EXPECT_EQ(z.data[1], 1);
  EXPECT_EQ(z.data[2], 1);
...
TEST(KernelClass, GreaterClassMatchesReference) {
  ...
  Tensor z = greater_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
...
TEST(KernelClass, GreaterRejectsUnsupportedDtype) {
  ...
  EXPECT_THROW((void)greater_kernel(nullptr, x, y), std::invalid_argument);
}
...
TEST(KernelClass, LessClassMatchesReference) {
  ...
  Tensor x = Tensor::FromFloat("", {2, 2}, {1.0f, 2.0f, 3.0f, 4.0f});
  Tensor y = Tensor::FromFloat("", {2, 2}, {2.0f, 2.0f, 2.0f, 2.0f});
  Tensor z = less_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
...
TEST(KernelClass, LessRejectsUnsupportedDtype) {
  ...
  Tensor x("", onnx_kernels::DataType::BOOL, {2}, {1, 0});
  Tensor y("", onnx_kernels::DataType::BOOL, {2}, {0, 1});
  EXPECT_THROW((void)less_kernel(nullptr, x, y), std::invalid_argument);
}
...
TEST(KernelClass, GreaterOrEqualClassMatchesReference) {
  ...
  Tensor x = Tensor::FromFloat("", {2, 2}, {1.0f, 2.0f, 3.0f, 4.0f});
  Tensor y = Tensor::FromFloat("", {2, 2}, {2.0f, 2.0f, 2.0f, 2.0f});
  Tensor z = ge_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
  EXPECT_EQ(z.data[0], 0); // 1 >= 2 -> false
...
TEST(KernelClass, GreaterOrEqualRejectsUnsupportedDtype) {
  ...
  GreaterOrEqual ge_kernel{ctx};
  EXPECT_THROW({ (void)ge_kernel(nullptr, x, y); }, std::invalid_argument);
}
...
TEST(KernelClass, LessOrEqualClassMatchesReference) {
  ...
  Tensor x = Tensor::FromFloat("", {2, 2}, {1.0f, 2.0f, 3.0f, 4.0f});
  Tensor y = Tensor::FromFloat("", {2, 2}, {2.0f, 2.0f, 2.0f, 2.0f});
  Tensor z = le_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
  EXPECT_EQ(z.data[0], 1); // 1 <= 2 -> true
  EXPECT_EQ(z.data[1], 1); // 2 <= 2 -> true
...
TEST(KernelClass, LessOrEqualRejectsUnsupportedDtype) {
  ...
  Tensor y("", onnx_kernels::DataType::BOOL, {2}, {1, 1});
  LessOrEqual le_kernel{ctx};
  EXPECT_THROW({ (void)le_kernel(nullptr, x, y); }, std::invalid_argument);
}
...
TEST(KernelClass, EqualClassMatchesReference) {
  ...
  Tensor z = equal_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
...
TEST(KernelClass, EqualInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor out("", onnx_kernels::DataType::BOOL, {3}, std::vector<uint8_t>(3));
...
TEST(KernelClass, EqualClassMatchesReferenceBool) {
  ...
  Tensor y("", onnx_kernels::DataType::BOOL, {4}, {1, 1, 0, 0});
  Tensor z = equal_kernel(nullptr, x, y);
  ASSERT_EQ(z.element_count(), 4);
  EXPECT_EQ(z.data[0], 1);
  EXPECT_EQ(z.data[1], 0);
...
TEST(KernelClass, EqualRejectsUnsupportedDtype) {
  ...
  Tensor y("", onnx_kernels::DataType::COMPLEX64, {2}, std::vector<uint8_t>(16));
  EXPECT_THROW((void)equal_kernel(nullptr, x, y), std::invalid_argument);
}
...
TEST(KernelClass, WhereClassSelectsValuesElementwise) {
  ...
  Tensor output = where_kernel(nullptr, condition, x, y);
  ASSERT_EQ(output.data_type, static_cast<int32_t>(onnx_kernels::DataType::FLOAT));
...
TEST(KernelClass, WhereRejectsNonBoolCondition) {
  ...
  EXPECT_THROW((void)where_kernel(nullptr, condition, x, y), std::invalid_argument);
}
...
TEST(KernelClass, BitwiseAndClassMatchesReferenceInt32) {
  ...
  Tensor z = kernel(nullptr, x, y);
  ASSERT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::INT32));
  ASSERT_EQ(z.element_count(), 4);
...
TEST(KernelClass, BitwiseAndInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor z("", onnx_kernels::DataType::INT32, {3}, std::vector<uint8_t>(3 * sizeof(int32_t)));
  kernel(x, y, z);
  const int32_t *p = reinterpret_cast<const int32_t *>(z.data.data());
  EXPECT_EQ(p[0], 0xF0);
  EXPECT_EQ(p[1], 0x0F);
...
TEST(KernelClass, BitwiseAndRejectsFloatTensors) {
  ...
  EXPECT_THROW((void)kernel(nullptr, x, y), std::invalid_argument);
}
...
TEST(KernelClass, BitwiseNotClassMatchesReferenceUint8) {
  ...
  Tensor z = kernel(nullptr, x);
  ASSERT_EQ(z.data_type, static_cast<int32_t>(onnx_kernels::DataType::UINT8));
...
TEST(KernelClass, BitwiseNotInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor z("", onnx_kernels::DataType::INT32, {2}, std::vector<uint8_t>(2 * sizeof(int32_t)));
  kernel(x, z);
  const int32_t *p = reinterpret_cast<const int32_t *>(z.data.data());
  EXPECT_EQ(p[0], ~0);
  EXPECT_EQ(p[1], ~0xFF);
...
TEST(KernelClass, BitwiseNotRejectsBoolTensors) {
  ...
  BitwiseNot kernel{ctx};
  Tensor x("", onnx_kernels::DataType::BOOL, {2}, {1, 0});
  EXPECT_THROW((void)kernel(nullptr, x), std::invalid_argument);
}
...
TEST(KernelClass, NotInPlaceWritesToPreallocatedOutput) {
  ...
  Tensor y("", onnx_kernels::DataType::BOOL, {3}, std::vector<uint8_t>(3));
  not_kernel(x, y);
...
TEST(KernelClass, NotRejectsNonBoolTensors) {
  ...
  EXPECT_THROW((void)not_kernel(nullptr, x), std::invalid_argument);
}
...
TEST(KernelClass, IsNaNClassMatchesReference) {
  ...
  Tensor y = isnan_kernel(nullptr, x);
...
TEST(KernelClass, IsNaNRejectsNonFloatTensors) {
  ...
  IsNaN isnan_kernel{ctx};
  Tensor x = Tensor::FromInt32("", {2}, {0, 1});
  EXPECT_THROW((void)isnan_kernel(nullptr, x), std::invalid_argument);
}
...
TEST(KernelClass, IsInfClassMatchesReference) {
  ...
  Tensor both = isinf_kernel(nullptr, x);
  ASSERT_EQ(both.element_count(), 6);
  EXPECT_EQ(both.data_type, static_cast<int32_t>(onnx_kernels::DataType::BOOL));
  ...
  Tensor only_pos = isinf_kernel(nullptr, x, /*detect_positive=*/1, /*detect_negative=*/0);
  EXPECT_EQ(only_pos.data[2], 1u);
  EXPECT_EQ(only_pos.data[4], 0u);
  EXPECT_EQ(only_pos.data[5], 1u);

  Tensor only_neg = isinf_kernel(nullptr, x, /*detect_positive=*/0, /*detect_negative=*/1);
...
TEST(KernelClass, IsInfRejectsNonFloatTensors) {
  ...
  IsInf isinf_kernel{ctx};
  Tensor x = Tensor::FromInt32("", {2}, {0, 1});
  EXPECT_THROW((void)isinf_kernel(nullptr, x), std::invalid_argument);
}
