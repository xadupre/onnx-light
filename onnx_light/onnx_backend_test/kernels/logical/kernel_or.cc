...
void Or::operator()(const Tensor &x, const Tensor &y, Tensor *output) const {
  ...
  if (output == nullptr) {
    throw std::invalid_argument("kernel::Or requires a non-null preallocated output tensor.");
  }
  if (output->data_type != TensorProto::DataType::BOOL) {
    throw std::invalid_argument("kernel::Or preallocated output must be a BOOL tensor.");
  }
  const int64_t nx = x.element_count();
  const int64_t ny = y.element_count();
  if (!(nx == ny || nx == 1 || ny == 1)) {
  ...
  if (output->shape != expected_shape) {
    throw std::invalid_argument(
        "kernel::Or preallocated output shape must match the broadcasted input shape.");
  }
  const int64_t n = nx >= ny ? nx : ny;
  if (output->data.size() != static_cast<size_t>(n)) {
    throw std::invalid_argument(
        "kernel::Or preallocated output buffer has unexpected size in bytes.");
  }
