...
void ComputeShapeHannWindow(ShapesContext &ctx, const NodeProto &node) {
  CheckNodeOpAndOutput(node, "HannWindow", "ComputeShapeHannWindow");
  EXT_ENFORCE_INVALID(node.input_size() >= 1,
                      "ComputeShapeHannWindow: HannWindow requires one input (size).");
  ...
  if (size_input.HasValueAsShape() && size_input.ValueAsShape().Rank() == 1 &&
    ...
    out_shape.PushBack(SymDim("HannWindow_dim0"));
  }
