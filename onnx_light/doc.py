...
def _index_page_rst(domains: list[str]) -> str:
    ...
    for domain in sorted(domains):
        stem = _domain_file_stem(domain)
        lines.append(f"   {stem}")
    lines.append("   schema_comparison")
    lines.append("")
...
_SCHEMA_COMPARISON_RST = '''\
.. _l-onnx-operators-schema-comparison:
...
def generate_operators_doc(
    ...
    # Write the schema comparison page under ONNX Operators.
    schema_comparison_path = os.path.join(output_dir, "schema_comparison.rst")
    _write_if_missing(schema_comparison_path, lambda: _SCHEMA_COMPARISON_RST)
    _report(
