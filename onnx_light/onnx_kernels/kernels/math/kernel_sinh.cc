...
Tensor Sinh::operator()(RuntimeContext *rt, const Tensor &x) const {
...
void Sinh::operator()(const Tensor &x, Tensor &output) const {
  EXT_ENFORCE_INVALID(x.data_type == DataType::FLOAT, "kernel::Sinh only supports FLOAT tensors.");
  EXT_ENFORCE_INVALID(output.data_type == DataType::FLOAT,
                      "kernel::Sinh preallocated output must be a FLOAT tensor.");
