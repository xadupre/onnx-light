...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::array<int64_t, 4> size{1, 1, 2, 3};
    ...
    const std::vector<int64_t> expected_shape{1, 2, 3, 2};
    ...
    const std::array<float, 12> expected{
        -2.0f / 3.0f, -0.5f, 0.0f, -0.5f, 2.0f / 3.0f, -0.5f,
        -2.0f / 3.0f, 0.5f,  0.0f, 0.5f,  2.0f / 3.0f, 0.5f,
    };
    const float *actual = outputs[0].GetTensorData<float>();
    for (size_t index = 0; index < expected.size(); ++index) {
      if (std::abs(actual[index] - expected[index]) > 1e-6f) {
        std::cerr << "Unexpected value at index " << index << ": " << actual[index] << '\n';
        return 1;
      }
    }

    std::cout << "AffineGrid custom op produced the expected [1, 2, 3, 2] grid.\n";
    return 0;
