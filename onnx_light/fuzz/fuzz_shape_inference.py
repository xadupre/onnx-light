...
"""Atheris fuzz harness for onnx_light.onnx.shape_inference.
...
* Raw bytes  -> onnx_light.onnx.load  -> infer_shapes
...
with atheris.instrument_imports():
    ...
    from onnx_light.onnx import TensorProto, helper, shape_inference

# Cap Python recursion well below CPython's C stack ceiling so a single
# deeply-nested input cannot crash the fuzzer process on every iteration
# that mutates into a deep graph, preventing discovery of unrelated bugs.
...
# shape-inference visitor to descend. Loop/Scan exercise different
# subgraph-context plumbing than If.
...
def TestOneInput(data):
    ...
    # we pass the full `data` to the loader unchanged so seed models (which
    # are complete serialized ModelProtos) are not truncated.
    if len(data) < 2:
        return
    toggles = data[-1]
    use_structured = bool(toggles & 0x04)
    # bits other than 0x04 are reserved for future toggles; mutations
    # against them are harmless until claimed. onnx-light's infer_shapes
    # does not currently expose strict_mode / check_type knobs, so bits
    # 0x01 and 0x02 are intentionally unused here.
    ...
    try:
        ...
        # Bounded by sys.setrecursionlimit above; suppressed so the fuzzer
        # keeps hunting unrelated bugs.
        return
    except Exception:
        # Malformed fuzz inputs raise a broad set of expected exceptions
        # (ValidationError, InferenceError, ValueError, ...). Real bugs
